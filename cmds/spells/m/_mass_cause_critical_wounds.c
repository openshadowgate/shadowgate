...
void spell_effect(int prof) 
{
    ...
    if(sizeof(attackers))
    {
        for(i=0;i<sizeof(attackers);i++)
        {
			...
			tell_object(caster,"%^BOLD%^%^RED%^You send out a very strong wave of magic, "
				"hitting "+attackers[i]->QCN+"!%^RESET%^");
            tell_object(attackers[i],"%^BOLD%^%^RED%^A very strong field radiates out "+
			    "from "+caster->QCN+" stealing a significant portion of strength from you.");
			tell_room(place,"%^BOLD%^%^RED%^"+caster->QCN+" sends out a very strong wave of "
    ...
    else if(objectp(target))
    {
        ...
        tell_object(target,"%^BOLD%^%^RED%^A very strong field radiates out "+
		    "from "+caster->QCN+" stealing a significant portion of strength from you.");
		tell_room(place,"%^BOLD%^%^RED%^"+caster->QCN+" sends out a very strong wave of "
