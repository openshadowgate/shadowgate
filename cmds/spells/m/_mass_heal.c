    ...
    if(sizeof(targets))
    {
        for(i=0;i<sizeof(targets);i++)
        {
            ...
            if((string)targets[i]->query_race() == "undead")
            {
                ...
                set_helpful_spell(0);
                damage_targ(targets[i],targets[i]->return_target_limb(),calculate_healing(targets[i]),"positive energy");
            ...
            else if(targets[i] == caster)
            {
                tell_object(targets[i],"%^BOLD%^%^CYAN%^A brilliant "+
                    "wave moves through you, carrying with it the essence of life.");
                damage_targ(targets[i],targets[i]->return_target_limb(),(-1 * calculate_healing(targets[i])),"positive energy");
            }
            else
            {
                tell_room(place,"%^BOLD%^%^CYAN%^A brillaint wave moves through"+
