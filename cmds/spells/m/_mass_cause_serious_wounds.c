...
void spell_effect(int prof) 
{
    ...
    if(sizeof(attackers))
    {
        for(i=0;i<sizeof(attackers);i++)
        {
			...
			tell_object(caster,"%^BOLD%^%^RED%^You send out a strong wave of magic, "
				"hitting "+attackers[i]->QCN+"!%^RESET%^");
            tell_object(attackers[i],"%^BOLD%^%^RED%^A strong field radiates out "+
			    "from "+caster->QCN+" stealing some strength from you.");
			tell_room(place,"%^BOLD%^%^RED%^"+caster->QCN+" sends out a strong wave of "
    ...
    else if(objectp(target))
    {
        ...
        tell_object(target,"%^BOLD%^%^RED%^A strong field radiates out "+
		    "from "+caster->QCN+" stealing some strength from you.");
		tell_room(place,"%^BOLD%^%^RED%^"+caster->QCN+" sends out a strong wave of "
...
{
    int num, max;
    max = 75; // max amount of damage the spell can heal, for ease of adjustment -Ares
