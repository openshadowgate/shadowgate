...
void spell_effect(int prof) 
{
    ...
    if(sizeof(attackers))
    {
        for(i=0;i<sizeof(attackers);i++)
        {
			...
			tell_object(caster,"%^BOLD%^%^RED%^You send out a faint wave of magic, "
				"hitting "+attackers[i]->QCN+"!%^RESET%^");
            tell_object(attackers[i],"%^BOLD%^%^RED%^A faint field radiates out "+
			    "from "+caster->QCN+" stealing a small bit of strength from you.");
			tell_room(place,"%^BOLD%^%^RED%^"+caster->QCN+" sends out a wave of "
    ...
    else if(objectp(target))
    {
        ...
        tell_object(target,"%^BOLD%^%^RED%^A faint field radiates out "+
		    "from "+caster->QCN+" stealing a small bit of strength from you.");
		tell_room(place,"%^BOLD%^%^RED%^"+caster->QCN+" sends out a wave of "
