...
void spell_effect(int prof) 
{
	...
	if(sizeof(targets))
	{
		for(i=0;i<sizeof(targets);i++)
		{
			...
			{
                tell_room(place,"%^BOLD%^%^BLUE%^A very strong blue field radiates out"+
				    " from "+targets[i]->QCN+" as "+caster->QCN+" releases a wave of "
                    "energy.",({ targets[i],caster }));
                tell_object(caster,"%^BOLD%^%^BLUE%^A very strong blue field radiates"+
				    " out from "+targets[i]->QCN+ " as you release a wave of energy.");
                tell_object(targets[i],"%^BOLD%^%^BLUE%^A very strong blue field "+
				    "radiates out from you as "+caster->QCN+" releases a wave"+
				    " of energy.");	
...
{
    int num, max;
    max = 100; // max amount of damage the spell can heal, for ease of adjustment -Ares
    num = roll_dice(5,8) + (clevel * 2);
    if(num > max) { num = max; }
    return num;
}
