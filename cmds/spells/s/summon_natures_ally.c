...
int check(mixed *args) {
    ...
    if (timer++ < timetowait) call_out("check", 1, ({timer, timetowait, caster, foes}) );
    else dest_effect();
}
...
void defend(object caster) {
    ...
    if(!numfoes) return;
    if (mon && objectp(mon)) {
        for (i=0;i<numfoes;i++) spell_kill(mon, foes[i]);
    }
    return;
}
...
void setup_beastie(object mymon) { } // called from level-specific spells to empower beastie