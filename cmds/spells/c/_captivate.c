...
string query_cast_string() {
    tell_room(place,"%^CYAN%^"+caster+QCN+"%^RESET%^%^CYAN%^ chants sonorously in captivating stones.");    
    tell_object(caster,"%^CYAN%^You chant sonorously in captivating stones");
    return "display";
}
...
void spell_effect(int prof) 
{
    ...
    tell_room(place,"%^CYAN%^"+caster->QCN+"%^RESET%^%^CYAN%^ sings a cong of peace, calming everyone.",caster);
