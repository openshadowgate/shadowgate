...
void create()
{
    ...
    set_description("With this spell a vampire can use the recently deceased to raise a powerful undead vampire spawn to aid them in combat. This power behaves the same way similar spells of creating undead does, but it is unique to vampires. Each undead will use one pool slot, with six pool slot maximum. Undead pool is shared across similar spells." + extra_help());
    evil_spell(1);
    set_helpful_spell(1);
    set_arg_needed();
}
