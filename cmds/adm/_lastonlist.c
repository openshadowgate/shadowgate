...
#define GREPFILE "/adm/save/users/laston.txt"
...
//            if((sscanf(line,"quit_time %d",temp) == 1) && (atoi(pattern) <= temp) )
            if((sscanf(line,"quit_time %d",temp) == 1) && ((time()-(_TWO_WEEKS_IN_SECONDS_*6)) <= temp) )
...
int help(){
   write(
@HELP
   Usage: lastonlist <when> <directory || all>
