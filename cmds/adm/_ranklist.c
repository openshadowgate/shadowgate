...
#define GREPFILE "/adm/save/users/ranklist.grp.txt"
...
seteuid(UID_ROOT);
   ...
   if(!EXCLUDE_WIZ) return notify_fail("WARNING: Exclude file is missing. Search will not run.");
   toberid = ({});
   if(dirs == "all")
...
//               if(member_array(name,EXCLUDE_WIZ) == -1){ 
                  toberid += ({name});
//               }
            i+=2;
         }
...
int help(){
   write(
@HELP
   Usage: ranklist <rank> <directory || all>
