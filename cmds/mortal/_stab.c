...
// turning sheath stabs off for balance-testing 3.5 feats on thieves; stab is way outta line. Nienne, 05/10.
/*        if(sizeof(inv) && objectp(sheath = inv[0]))
        {
            // Has a sheath. Has a dagger.
            // This is Fucking Ugly Code here.
            inven = all_inventory(sheath);
            victim->set_watched(lastwatched);
            // if(!inven[0]->is_weapon()) return 1;
            if(objectp(sheath_dagger=inven[0]))
            {
	        //
            }
            sheath_weight = sheath_dagger->query_weight();
            sheath_dagger->set_weight(0);
            wield = player->query_wielded();
            if(sizeof(wield))
            {
	            if(objectp(orig_dagger = wield[0]))
                {
	                //
	            }
                orig_weight = orig_dagger->query_weight();
                orig_dagger->set_weight(0);
                orig_dagger->move(sheath);
            }

            sheath_dagger->move(player);
            player->force_me("wield "+((string *)sheath_dagger->query_id())[0]);

            v_qcn = victim->query_cap_name();
            p_qcn = player->query_cap_name();
            p_poss = player->query_possessive();
            p_subj = player->query_subjective();

            tell_object(player,"%^BOLD%^%^RED%^As you stab at "+v_qcn+", you quickly release your first weapon!");
            tell_object(victim,"%^BOLD%^%^RED%^As "+p_qcn+" stabs you, "+p_subj+" quickly releases the weapon!");
            tell_room(environment(player),"%^BOLD%^%^RED%^As "+p_qcn+" stabs "+v_qcn+", "+p_subj+" quickly releases the weapon!",({player,victim}));
            tell_object(player,"%^BOLD%^%^RED%^You flick your wrist and another weapon appears in your hand!");
            tell_object(player,"%^BOLD%^%^RED%^With this new weapon you stab quickly again at "+v_qcn+"!");
            tell_room(environment(player),"%^BOLD%^%^RED%^"+p_qcn+" then flicks "+p_poss+" wrists and a new weapon appears"+" in "+p_poss+" hand!",player);
            tell_room(environment(player),"%^BOLD%^%^RED%^"+p_subj+" quickly stabs again at "+v_qcn+"!",({player,victim}));
            tell_object(victim,"%^BOLD%^%^RED%^"+p_subj+" quickly stabs again at you!");
            ...
            //
            if (objectp(sheath_dagger))
            {
	            sheath_dagger->move(sheath);
	            sheath_dagger->set_weight(sheath_weight);
            }

            if(sizeof(wield))
            {
	            orig_dagger->move(player);
	            orig_dagger->set_weight(orig_weight);
	            player->force_me("wield "+((string *)orig_dagger->query_id())[0]);
	            if(!orig_dagger->query_wielded())
	            player->force_me("wield "+((string *)orig_dagger->query_id())[0]+" in right hand and left hand");
            }

            // reset their watch code to something about equal to
            // either what it was or what it got set to during the stabs...

            if ((victim->query_watched() + 10 ) < lastwatched)
	            victim->set_watched(lastwatched);
            else
	            victim->set_watched(victim->query_watched()+ random(5)+5);

        }
        else
        {
            // No sheath or dagger.
            // I made the new sheaths echo the message on check.
