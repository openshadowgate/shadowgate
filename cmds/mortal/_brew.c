...
int cmd_brew(string str){
  ...
  if(!str) return notify_fail("See 'help brew' for syntax.\n");
  ...
  if(mytype == "list") {
    ...
    currentmat = "-simple: ";
    if(!sizeof(matkeys)) currentmat = currentmat + "none.";
    else {
  ...
  datamap = KITS;
  datamapkeys = keys(datamap);
  if(member_array(myname,datamapkeys) != -1) { tracker = 1; mydir = "kits"; }
  if(!tracker) {
    datamap = POTIONS;
    datamapkeys = keys(datamap);
    if(member_array(myname,datamapkeys) != -1) { tracker = 1; mydir = "potions"; }
  }
  if(!tracker) {
    datamap = ELIXIRS;
    datamapkeys = keys(datamap);
    if(member_array(myname,datamapkeys) != -1) { tracker = 1; mydir = "elixirs"; }
  }
  if(!tracker) {
    datamap = OILS;
    datamapkeys = keys(datamap);
    if(member_array(myname,datamapkeys) != -1) { tracker = 1; mydir = "oils"; }
  }
  if(!tracker) {
    datamap = POWDERS;
    datamapkeys = keys(datamap);
    if(member_array(myname,datamapkeys) != -1) { tracker = 1; mydir = "powders"; }
  }
  if(!tracker) {
    datamap = MISC;
    datamapkeys = keys(datamap);
    if(member_array(myname,datamapkeys) != -1) { tracker = 1; mydir = "misc"; }
  }
  if(!tracker) return notify_fail("Sorry, "+myname+" is not a valid alchemical creation. Please see 'help brew' for syntax and lists.\n");

  if(!FEATS_D->usable_feat(TP,"brew potion")) return notify_fail("You need the 'brew potion' feat to do this!\n");
  filename = replace_string(myname," ","_");
  filename = "/d/common/obj/brewing/"+mydir+"/"+filename+".c";
  if(!file_exists(filename)) return notify_fail("File error: does not exist. This brew is probably not installed yet.\n");

  if(mytype == "create") { // creating a potion or mix
    if(!avatarp(TP) && !ETP->is_lab()) return notify_fail("You must be in a laboratory to brew potions and other alchemical mixes.\n");
            ...
            {         
                ...
                if(ob = present(currentmat,TP)) 
                {
                    ...
                    else if((int)ob->is_component() && (string)ob->query_name() == matkeys[i])
                    {
                        ...
                        if((int)ob->query_comp_quantity() < dc) dc = dc - (int)ob->query_comp_quantity();
    ...
    if (expdrain > max_exp_drain) { expdrain = max_exp_drain; }
        ...
        for(i=0;i<sizeof(using);i++) 
        {
            if(objectp(using[i])) 
            {
                if(using[i]->is_component())
                {
                    ...
                    if((int)using[i]->query_comp_quantity() <= 0) using[i]->remove();
                }
                else using[i]->remove();
                continue;
            }
  ...
  if(mytype == "materials") { // list off required materials and exp for a brew
    ...
    materials = datamap[myname]["materials"];
    matkeys = keys(materials);
    for(i=0;i<sizeof(matkeys);i++) tell_object(TP,"%^BOLD%^%^WHITE%^-"+matkeys[i]+": "+materials[matkeys[i]]+"%^RESET%^");
