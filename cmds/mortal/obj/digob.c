...
int get() {return 0;}
int restore_me(string file) {TO->remove();}

int begin_dig(string str, object tp){
  if ((!objectp(tp)) && (!objectp(environment(tp))))  return 0;
...
void move_rubble(string ext, object tp){
  ...
  if(i <= 0)
  {
    ...
    tell_object(tp, "press <RETURN> to continue.");
    tp->remove_paralyzed();
    if(objectp(etp)) { etp->remove_property("wall of stone"); }
    if(objectp(otp = to_object(etp->query_exit(ext)))) { otp->remove_property("wall of stone"); }
    TO->remove();
