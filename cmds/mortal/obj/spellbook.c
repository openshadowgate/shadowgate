...
void flip(string str, int al, int temp);
...
string owner;
int restore, *spells_at_level, *in_mind, descflag;
...
//    add_action("copy", "copy");
  ...
  if (avatarp(TP)) add_action("resetdesc","resetdesc");
}
...
int look(string str) {
    ...
    if ( !(ob == TO) )
        return 0;
    if (/*!interactive(TP) && */(!TP->is_class("mage") && !TP->is_class("bard") && !avatarp(TP))) {
  ...
  if (avatarp(TP)) write( 
@GARRETT
  Avatar functions: resetdesc - allows book desc to be reset.
GARRETT
    );
    return 1;
}
...
void flip(string str, int sl, int temp){
    if (random(8) == 0) {
...
int help(string str) {
    ...
    tell_object(TP, "%^YELLOW%^<setdesc>%^RESET%^ To set a new book description, default is 'A book'");
    ...
    if (WIZ_LEV) {
        line();
        if (TP->is_class("mage"))
            tell_object(TP, "%^MAGENTA%^"+arrange_string("Spell level:", 18)+"Number allowed:      Memorized:     [Your level: "+TP->query_guild_level("mage")+"]");
        else if (TP->is_class("bard"))
            tell_object(TP, "%^MAGENTA%^"+arrange_string("Spell level:", 18)+"Number allowed:      Memorized:     [Your level: "+TP->query_guild_level("bard")+"]");
        line();
        ...
        for (x = 0; x < 9;x++) {
            tmp = in_mind[x];
            if(!tmp) tmp = "0";
            tell_object(TP, "%^CYAN%^"+arrange_string((x+1), 18) + arrange_string(tmp,21) + TP->query_num_spells_level(x+1));
        }
...
void set_owner() {
    ...
    object tp;
    tp = ETO;
    /* had interactive(tp) && in this commented out... */ if ( (TP->is_class("mage") || TP->is_class("bard"))){
...
int mydesc(string str) {
   ...
   if (descflag) {
     tell_object(TP,"You have already set this book's appearance!\n");
     return 1;
   }
   if (!str) {
     TO->set_short("A book");
     ...
     return 1;
      ...
      if(strsrch(str,"book") == -1 && strsrch(str,"tome") == -1) TO->set_short(""+str+" book%^RESET%^");
...
int resetdesc() {
   descflag = 0;
  if (avatarp(TP)) write("Book can be redescribed now.");
return 1;
}
