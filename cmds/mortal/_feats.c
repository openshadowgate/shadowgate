...
int remove_spell_mastery_spell(string str,object ob,string feat,string mytype){
    ...
    for(i = 0;i<sizeof(myclasses);i++) {
        if((int)spell->query_spell_level(myclasses[i]) > 0 && (int)spell->query_spell_level(myclasses[i]) < 3) {
            ...
            if(myclasses[i] == "bard" || myclasses[i] == "sorcerer") { num++; }
        }
    }
...
int spell_mastery_spell(string str,object ob,string feat,string mytype){
    ...
    for(i = 0;i<sizeof(myclasses);i++) 
    {
        ...
        if(myclasses[i] != "mage" && myclasses[i] != "archmage") { continue; }
        if(level < 6)
        {
    ...
    case "list":
      ...
      switch(info[0]) {
        ...
        case "general":
          ...
          return 1;
        case "epic":
          if(!high_mortalp(TP) && !avatarp(TP) && CONFIG_D->check_config("HM") == 0) FEATS_D->display_feats(TP,TP);
          else FEATS_D->display_feats(TP,TP,"epic");
          return 1;
        default:
          FEATS_D->display_feats(TP,TP);
          return 1;
...
// combat feat allocation here
        ...
        for(i=0;i<sizeof(subset);i++) {
          if(member_array(subset[i],CASTERCLASSES) != -1) continue; // caster classes get no bonus melee feats!
          if(member_array(subset[i],HYBRID) != -1) continue; // neither do hybrids!
          if(subset[i] == "fighter") j = (((int)TP->query_class_level(subset[i]) +4) / 5); // fighters @ L1 & every 5 levels thereafter
          else j = (((int)TP->query_class_level(subset[i]) - 16) / 5); // melee classes @ L21 & every 5 levels thereafter
          if(j < 0) j = 0;
          BONUS_ALLOWED += j;
        }
...
//        if(sizeof(info)) { tmp = implode(info," "); }
        ...
        else tell_object(TP,"%^YELLOW%^Are you sure you want to add the feat "+tmp+"?");
...
//        if(sizeof(info)) { tmp = implode(info," "); }
        ...
        for(i=0;i<sizeof(subset);i++) {
          if(member_array(subset[i],CASTERCLASSES) != -1) continue; // caster classes get no bonus melee feats!
          if(member_array(subset[i],HYBRID) != -1) continue; // neither do hybrids!
          if(subset[i] == "fighter") j = (((int)TP->query_class_level(subset[i]) + 4) / 5); // fighters @ L1 & each 5 levels thereafter
          else j = (((int)TP->query_class_level(subset[i]) - 16) / 5); // melee classes @ L21 & each 5 levels thereafter
          if(j < 0) j = 0;
          BONUS_ALLOWED += j;
        }
...
//        if(sizeof(info)) { tmp = implode(info," "); }
        if(!FEATS_D->can_remove_feat(TP,tmp))
        {
            ...
            {
                if(FEATS_D->has_feat(TP,required[i])) { my_required += ({ required[i] }); }
            }
