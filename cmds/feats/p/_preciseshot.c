...
void execute_attack() {
    ...
    if((int)target->query_size() < 3) damage += weapons[0]->query_damage();
    else damage += weapons[0]->query_large_damage();
    ...
    if(target->query_property("weapon resistance"))
      if((int)weapons[0]->query_property("enchantment") < (int)target->query_property("weapon resistance")) damage = 0;
