...
int protecting_count(object obj)
{
    ...
    if(!objectp(obj) || !objectp(environment(obj))) { return 0; }
    
    inven = all_living(environment(obj));
    for(i=0;i<sizeof(inven);i++)
    {
        if(!objectp(inven[i])) { continue; }
        if(member_array(obj, inven[i]->query_protectors()) != -1) { count++; }
    }
...
void execute_attack()
{
    ...
    for(i=0;i<sizeof(party);i++)
    {
        ...
        if(!sizeof(caster->query_attackers())) { continue; }
        ...
        if(party[i]->query_hp_percent() < 100)
        {
            ...
            party[i]->cause_typed_damage(party[i],party[i]->query_target_limb(), -1* (roll_dice(1,10) + 10), "positive energy");
            if(party[i] != caster) 
            { 
