...
void execute_attack()
{
    ...
    if(in_shapeshift) type = "blunt";
    else type = weapons[0]->query_type();
    if(!stringp(type) || !type || type == "") type = "sharp";
    if(strsrch(type,"slash") == -1 && strsrch(type,"pierc" == -1)) type = "blunt";
    else type = "sharp";

    if(sizeof(weapons)) enchant = (int)weapons[0]->query_property("enchantment");
    else enchant = 0;
    ...
    if(sizeof(attackers)) {
        for(i=0;i<sizeof(attackers);i++) {
            ...
            if(attackers[i] == target) continue;
            if(tempmap[attackers[i]] > time()) continue;
            okattackers += ({ attackers[i] });
            break;
        }
    }
    if(sizeof(okattackers)) target_two = okattackers[0]; // pick up only attackers who have not been impaled. -N, 9/10.
...
// picking up 12 as a benchmark for druid shift, two-hand sword equiv
    if(sizeof(weapons)) dam = weapons[0]->query_large_wc();
    else dam = 12;
    dam = ((clevel-1)/10 + 1) * (dam/2); //let it scale properly in 10-level blocks. -N, 9/10
    if(sizeof(weapons)) dam += roll_dice(mult,dam) + weapons[0]->query_large_wc();
    else dam += roll_dice(mult,dam) + 12;
    dam += "/daemon/bonus_d"->damage_bonus(caster->query_stats("strength"));
    dam += (int)caster->query_damage_bonus();
    mod = dam * -1;

    if(!in_shapeshift) theweapon = weapons[0]->query_short();
    else theweapon = "body";
