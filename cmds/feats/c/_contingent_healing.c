...
void execute_feat()
{
    ...
    {
        tell_object(caster,"Contingent healing is already active.");
        dest_effect();
        return;
    }
    ::execute_feat(); 
...
void execute_attack()
{
    ...
    for(i=0;i<sizeof(party);i++)
    {
        ...
        if(caster->query_unconscious() || caster->query_paralyzed() || (caster->query_blind() && !FEATS_D->usable_feat(caster,"blindfight"))) { continue; }
        if(!objectp(party[i])) { continue; }
        if(environment(party[i]) != environment(caster)) { continue; }
        if(party[i]->query_hp_percent() > 60) { continue; }
        if(percent && party[i]->query_hp_percent() > percent) { continue; }
...
void dest_effect()
{
    if(objectp(caster)) 
    {
        tell_object(caster,"%^B_BLUE%^Your contingent healing deactivates.");
