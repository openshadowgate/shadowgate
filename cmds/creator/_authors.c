...
int cmd_authors(string str) {
    string tmp;
    string *domains;
    mapping map_ent;
    int i, total, max;
   
    i =sizeof(domains = keys(domain = author_stats()));
    while(i--) if(file_size(REALMS_DIRS+"/"+lower_case(domains[i])) != -2)
    ...
    if(!total) total = 1;
    tmp = "%^BOLD%^%^GREEN%^"+mud_name()+" %^YELLOW%^Offical Author Statistics:\n\n";
