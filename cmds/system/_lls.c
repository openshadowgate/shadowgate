...
varargs int do_lls(mixed path, object act_ob, int show_dots, string real_path)
{
  string *files;
  string *Files,*Dirs;
...
int cmd_lls(string path)
{
  ...
  if(num == 1)
    {
      do_lls(wcfiles[0], act_ob, show_dots);
      return 1;
    }
  ...
  if(Files)
    do_lls(path_file(Files), act_ob, show_dots);
  if(Dirs)
    {
      for(i = 0; i < sizeof(Dirs); i++)
	{
	  message("system", Dirs[i]+":", this_player());
	  do_lls(Dirs[i],act_ob, show_dots);
	}
    }
  return 1;
}
