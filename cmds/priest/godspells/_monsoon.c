//Monsoon - Istishian firestorm alternative, coded by Nienne//
#include <std.h>
#include <spell.h>
#include <magic.h>
#include <daemons.h>
...
void spell_effect(int prof) {
    ...
    tell_room(place,"%^BOLD%^%^CYAN%^"+caster->query_cap_name()+" waves "+caster->query_possessive()+" hands around summoning the fury of a watery storm from the skies above!",caster);
...
Deity  : Istishia
Syntax : chant monsoon

When the monsoon spell is cast, the surrounding area is
engulfed in a raging storm of water.  Any target in the 
area of effect takes damage every round while the spell 
is in effect, but does not damage the caster.
HELP
);
    return;
}
