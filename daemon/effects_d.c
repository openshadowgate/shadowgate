...
int spell_effect(object caster, object device, string command){
    ...
    if (command) {
        if (sscanf(command, "on %s", who) != 1) return notify_fail("use <object> on <target>\n");
    }
    else who = caster->query_name();

    if (!(target = present(who,environment(caster)))) return notify_fail("No such target.\n");
    obj = new(cspell);
    caster->set_property("spell_casting",obj);
    obj->use_spell(caster, target, level, 100, stype);
    return 1;
}
