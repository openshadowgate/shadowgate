...
//	else return ERROR;
      if (member_array(room, keys(NeededRooms)) != -1)
	  return ERROR;
      else
	  NeededRooms[room] = ""+player+"&"+dest+"";
...
mapping query_whatWhere(){  //added by Lujke to aid with bug checking
  return whatWhere;
}
	...
	for(x = 0;x < sizeof(AreaSaves);x++)
	{
		...
		for(i=0;i<j;i++)
		{
			...
			if(dest != ([]))
			{
				...
				if(!ExitName) ExitName = "out";
				if(!EnterName) EnterName = "enter";
