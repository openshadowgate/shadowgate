...
{
	int x;
	object *__PCItems, __PCKiller;
	if(!objectp(__PC)) return;
	__PCKiller = __PC->query("killedBy");
	__PC->delete("killedBy");
	if(!objectp(__PCKiller)) return;
	__PCItems = deep_inventory(__PC);
	for(x = 0;x < sizeof(__PCItems);x++)
	{
		if(!objectp(__PCItems[x])) continue;
		if(undefinedp(__PCItems[x]->query_property("death notify"))) continue;
		__PCItems[x]->death_notify(__PC, __PCKiller);
		continue;
	}
	return;
}
