...
/*int check_avoidance(object who, object victim, object *weapons)
{
    ...
    temp1 = (int)who->query_character_level();
    temp1 += (int)who->query_skill("athletics");
    temp2 = (int)victim->query_character_level();
    ...
    //basically shouldn't be parrying/scrambling, etc, if you are paralyzed
    ...
    if((sizeof(weapons) && !weapons[0]->is_lrweapon()) || !FEATS_D->usable_feat(who, "point blank shot"))
    {
        ...
        {
            if(roll_dice(1, temp2) > roll_dice(1, temp1))
            {
        ...
        if(victim->is_animal() && !random(5))
        {
            ...
            if(objectp(rider))
            {
                if(FEATS_D->usable_feat(rider,"mounted combat") && (roll_dice(1, temp2) > roll_dice(1, temp1)))
                {
    ...
    if(sizeof(weapons) || FEATS_D->usable_feat(victim, "unarmed parry"))
    {
        ...
        {
            if(victim->query_property("shield_of_whirling_steel")) { mod += victim->query_property("shield_of_whirling_steel"); }
            if(victim->query_scrambling() && !victim->query_property("shield_of_whirling_steel")) { mod = -roll_dice(1,10); }
            //else mod = roll_dice(1,8);
            if((roll_dice(1, temp2)+mod) > roll_dice(1, temp1))
            {
                if(FEATS_D->usable_feat(victim,"masters parry") && victim->is_wielding("dual wielding"))
                {
                    if(random(4)) { counter_attack(victim); } // should happen frequently as it's a level 47 ability
                }
