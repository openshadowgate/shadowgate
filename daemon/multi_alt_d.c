// /daemon/multi_alt_d.c
// Pator@ShadowGate August 19 2003
// A nice rip-off from multi_d to keep track of multiple chars whitout login restrictions
// used by /cmds/avatar/_alt.c and /cmds/avatar/_unalt.c
...
void list_links() {
   ...
   names = keys( links );
  names = sort_array(names,"inverse_caseless_alphabetical_sort","/daemon/filters_d");
   names -= non_voting;
   loop = sizeof( names );
   if( !loop ) {write( "No multiple characters yet."); return;}
   write( "The following multiple characters are registered:");
   while( loop-- )
     write( "%^BOLD%^%^YELLOW%^"+ capitalize(names[loop]) + "%^RESET%^ is also: " +
