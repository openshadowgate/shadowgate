...
void old_bonuses(object item, object tp)
{
    ...
    file = base_name(item);
    if(!file_exists(file)) { file = file+".c"; }
...
string bonus_level(object item, object tp)
{
    ...
    if( (item->query_property("magic") * 5) > flat_level) { flat_level = item->query_property("magic") * 5; }
    ...
    default:
        ...
        for(i=0;i<sizeof(bonus_names);i++)
        {
            ...
            if(level > (flat_level * 0.75)) { count++; }
        }
...
varargs void item_check(object item, object tp)
{
    ...
    if(item->is_armor()) { valid_limb_check(item, tp); }
...
int validate(object ob) {
    ...
    if(sizeof(bonus_names) >= 3) {
      ...
      if(remaining > 0) {
        ...
        timestamp = (t % 60)+"s";
        t /= 60;
        if(t % 60 >0) timestamp = t % 60+"m "+timestamp;
        t /= 60;
        if(t % 24 > 0) timestamp = t % 24+"h "+timestamp;
