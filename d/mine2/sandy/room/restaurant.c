...
"");
	...
	(["waitress" : "The waitress is nicely dressed.",
	  ...
	  "menu" : "Try reading it!"]) );
    set_exits( 
	      (["out" : "/d/mine2/sandy/room/sandy18"]) );
}

void reset() {
  ::reset();
    if(!present("waitress")) 
      new("/d/mine2/sandy/mon/waitress")->move(this_object());
}
...
int read(string str) {
    ...
    write("%^BOLD%^Here is a list of the most popular foods in Sandy's Restaurant.");
