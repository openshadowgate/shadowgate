...
void create() 
{
    ::create();
    set_name("Lezulfin");
    set_id( ({ "Lezulfin","vendor", "gnome", "shopkeeper","lezulfin"}) );
    set_short("%^RESET%^%^CYAN%^Lezulfin - a small %^YELLOW%^gold skinned %^RESET%^%^CYAN%^gnome%^RESET%^");
    set_long("%^RESET%^%^CYAN%^Lezulfin is little gnome at three feet and change with "
    ...
    set_storage_room(INV_PATH"low_store");
    set_store_type("low");
    fill_storage_room();
}
