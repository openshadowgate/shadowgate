...
void create() 
{
    ::create();
    set_name("Cezulfin");
    set_id( ({ "Cezulfin","vendor", "gnome", "shopkeeper","cezulfin"}) );
    set_short("%^RESET%^%^CYAN%^Cezulfin - a small %^YELLOW%^gold skinned %^RESET%^%^CYAN%^gnome%^RESET%^");
    set_long("%^RESET%^%^CYAN%^Cezulfin is little gnome at three feet and change with "
    ...
    set_storage_room(INV_PATH"mid_store");
    set_store_type("mid");
    fill_storage_room();
}
