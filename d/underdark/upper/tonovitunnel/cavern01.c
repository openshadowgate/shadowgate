...
void create() {
    ...
    set_long("%^RESET%^%^BLUE%^A vast natural cavern opens around "
"you.  Formed centuries ago, this cave has likely been sealed away for "
"ages only recently having been breeched by the tunnel system that passes "
...
"faint light.%^RESET%^\n");
    ...
    set_items(([
     ({"cavern","cave"}) : "%^BOLD%^%^BLACK%^Naturally formed by "
"time and the forces of nature, this vast cavern stretches away into the "
...
"filling all that you can see with their massive forms.%^RESET%^",
     ({"tunnel","tunnels","tunnel system"}) : "%^BOLD%^%^BLACK%^To "
"the north and south of this immense cavern can be seen the entrances and "
...
"some of the tunnel system leading into this cavern.%^RESET%^",
    ...
    set_exits(([
      "southeast" : TUNNEL"tunnel16",
      "north" : TUNNEL"cavern02",
    ]));
    issearched = 0;
}
