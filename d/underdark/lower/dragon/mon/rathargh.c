...
void bladefan(object target)
{
    ...
    for (i = 0; i < 5; i++) {
        ...
        target->cause_typed_damage(target, target->return_target_limb(), 50 + random(50), "slashing");
    }
    return;
}
...
void stabfun(object target)
{
    ...
    target->cause_typed_damage(target, target->return_target_limb(), 150 + random(75), "slashing");
}
...
void silencedust(object target)
{
    ...
    if (dustobj = present("choking-dust-obj", target)) {
        dustobj->remove();
    }
