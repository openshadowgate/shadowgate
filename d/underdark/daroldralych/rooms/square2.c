...
"faint light around it.  Directly south of you is the centre of the square, where stands a particularly %^RED%^ex"
"%^MAGENTA%^q%^BOLD%^u%^WHITE%^i%^MAGENTA%^s%^RESET%^%^MAGENTA%^i%^RED%^te %^BLUE%^statue.  Looming over you to the north "
"is the imposing outline of the temple.%^RESET%^\n");

   add_item(({"statue","elaborate statue"}),"%^BLUE%^To the south, in the centre of the square, you can see the outline "
"of the %^RED%^ex%^MAGENTA%^q%^BOLD%^u%^WHITE%^i%^MAGENTA%^s%^RESET%^%^MAGENTA%^i%^RED%^te %^BLUE%^statue that marks the "
"city's centre.%^RESET%^");
   set_exits(([
     "temple":ROOMS"temple1",
     "east":ROOMS"square3",
     "west":ROOMS"square1",
     "south":ROOMS"square5",
   ]));
