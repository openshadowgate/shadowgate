...
"%^BLUE%^You are standing on the third floor inside the massive %^RESET%^stone %^BLUE%^stalactite that serves as the "
...
"Aside from the stairs, there is only one exit to this floor, a single archway that leads through to a small alcove "
"to the north.%^RESET%^\n");
...
"%^BOLD%^%^BLACK%^an %^RESET%^%^BLUE%^leads through to a small alcove to the north.%^RESET%^");
   set_exits(([
     "down":ROOMS"tower4",
     "north":ROOMS"tower7", // carver
     "up":ROOMS"tower8",
   ]));
}
