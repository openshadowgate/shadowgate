//pathway10.c - Path to Lothwaite.  Circe 1/9/04
#include <std.h>
#include "../lothwaite.h"
inherit VAULT;
...
void create(){
   ...
   set_items(([
      ...
      ({"wall","rampart","gate","earthen rampart"}) : (: TO, "wall_desc" :),
   ...
   set_exits(([
      "south" : PATHEXIT"pathway9",
      "east" : PATHEXIT"pathway11"
   ]));
   set_door("gate",PATHEXIT"pathway11","east","giant_key");
...
string ldesc(){
   if(TP->query_size() < 2){
      return("This %^ORANGE%^footpath %^RESET%^is incredibly wide, stretching out "+
         ...
         "forest, making it impossible for you to leave the trail.  Just "+
         "to the east, an iron-bound gate blocks the path.  A tall "+
         "earthen rampart covered with vibrant %^GREEN%^green "+
         "grass %^RESET%^runs both north and south as far as "+
         "you can see before being dwarfed by the forest once "+
         "more.\n");
   }
   if(TP->query_size() > 1 && TP->query_size() < 3){
      return("This %^ORANGE%^footpath %^RESET%^is rather wide, about eight "+
   ...
   return("This %^ORANGE%^trail %^RESET%^winding up the mountainside "+
      ...
      "make leaving the trail all but impossible.  Just "+
      "to the east, an iron-bound gate blocks the path.  A tall "+
      "earthen rampart covered with vibrant %^GREEN%^green "+
      "grass %^RESET%^runs both north and south as far as "+
      "you can see before being dwarfed by the forest once "+
...
string ndesc(){
   if(TP->query_size() < 2){
      return("%^BLUE%^The wide, empty %^ORANGE%^trail %^BLUE%^seems eerily open at night, "+
         ...
         "The shadowy outline of an earthen rampart runs both north "+
         "and south, while lantern light just to the east reveals "+
         "a large, iron-bound gate blocking the path.\n");
   }
   if(TP->query_size() > 1 && TP->query_size() < 3){
      return("%^BLUE%^The wide %^ORANGE%^path %^BLUE%^provides little "+
         ...
         "%^RESET%^%^BLUE%^that reach well over your head.  The shadowy "+
         "outline of an earthen rampart runs both north "+
         "and south, while lantern light just to the east reveals "+
         "a large, iron-bound gate blocking the path.\n");
   } else {
   return("%^BLUE%^The trail seems pleasant even at night, with the "+
      ...
      "comforting somehow.  The shadowy outline of an earthen rampart "+
      "runs both north "+
      "and south, while lantern light just to the east reveals "+
      "a large, iron-bound gate blocking the path.\n");
   }
...
string path_desc(){
   if(TP->query_size() < 2){
      return("The pathway is huge!  It winds its way up the mountainside "+
         ...
         "become less used over the years.  The path seems smoother "+
         "right here at the gate than it does further down on the "+
         "trail.");
   }
   if(TP->query_size() > 1 && TP->query_size() < 3){
      return("This pathway is clearly wider than it would need to be for most "+
         ...
         "wagon ruts whatsoever.  The path seems smoother "+
         "right here at the gate than it does further down on the "+
         "trail.");
   } else {
...
string rocks_desc(){
   ...
   return("Large boulders scattered here and there on the trail "+
      ...
      "that they provide directions.  An arrow pointing eastward "+
...
string wall_desc(){
   if(TP->query_size() < 2){
      return("%^GREEN%^The intimidating earthen rampart rises well "+
         ...
         "south and seems to be encircling whatever it might be "+
         "on the otherside of this massive gate.");
   }
   if(TP->query_size() > 1 && TP->query_size() < 3){
      return("%^GREEN%^An amazing earthen rampart rises well over "+
         ...
         "the iron-bound gate to the east that runs through it.");
   } else {
   return("%^GREEN%^An earthen rampart about 30 feet high seems "+
      "to protect whatever is on the other side of this gate.  For "+
