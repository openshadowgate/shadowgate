...
void create(){
   set_repop(40);
   set_monsters ( ({MON"firbolg_citizen"}),({1}) );
   ::create();
...
/*   } else {
   tmp = "%^GREEN%^Low, one-story %^ORANGE%^buildings %^GREEN%^are "+
      ...
      "all around the hamlet, each one bearing a small %^BOLD%^green "+
...
/*
   if(TP->query_size() < 2){
      tmp = "%^BLUE%^Massive buildings rise up all around you, their "+
...
/*
   ...
   tmp = "%^BLUE%^Buildings rise up all around you, their "+
