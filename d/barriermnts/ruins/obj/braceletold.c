...
void create(){
        ...
        set_obvious_short("gem-set platinum bracelet");
...
"eye.  Upon the underside of the bracelet is a written engraving.%^RESET%^\n");
        ...
        set_type("ring");
        set_limbs(({"left arm"}));
        set_lore("%^WHITE%^%^BOLD%^This bracelet once belonged to the Kelemvorite priestess, Sionne of the Ashes.  Many rumours exist concerning how she came to possess such an item, some that claim she was granted it by the Lord of the Dead himself, as a gift for her unwavering faith.%^RESET%^\n");
        set_property("lore difficulty",15);
        set_read("%^BOLD%^%^CYAN%^Instill%^WHITE%^ within me a spell to mind\n"
"%^BOLD%^%^WHITE%^And tell me %^CYAN%^now%^WHITE%^ to use it\n"
...
void init() {
   ...
   add_action("now_fun","now");
}

int wear_fun(){
   if(TP->is_class("cleric") && sizeof(TP->query_classes()) == 1) {
     tell_room(EETO,"%^WHITE%^%^BOLD%^A soft glow appears around the bracelet as "+ETO->QP+" slides it around "+ETO->QP+" wrist.%^RESET%^",ETO);
     tell_object(ETO,"%^WHITE%^%^BOLD%^A feeling of reassurance fills your mind as you slide the bracelet around your wrist.%^RESET%^");
     return 1;
   }
   else {
     tell_room(EETO,"%^WHITE%^%^BOLD%^The bracelet seems forcibly repelled away from "+ETO->QCN+"'s arm.%^RESET%^",ETO);
     tell_object(ETO,"%^WHITE%^%^BOLD%^No matter how you try to force it, the bracelet seems repelled from your arm.%^RESET%^");
   return 0;
   }
}
...
int remove_fun(){
    ...
    tell_room(environment(ETO),"%^WHITE%^%^BOLD%^The bracelet's glow fades as "+ETOQCN+" slips it from "+ETO->QP+" wrist.%^RESET%^",ETO);
    tell_object(ETO,"%^WHITE%^%^BOLD%^The feeling of confidence leaves you as you remove the bracelet.%^RESET%^");
    return 1;
}
...
int instill_fun(string str){
   ...
   if(!TP->query_cl_memorized(str)){
     notify_fail("You don't have that spell memorized!\n");
     return 0;
   }
   TO->set_property("instilled",str);
   TP->set_cl_memorized(str, -1);
   write("%^WHITE%^%^BOLD%^You instill the spell %^CYAN%^"+str+"%^WHITE%^ into the bracelet!%^RESET%^\n");
   tell_room(EETO,"%^WHITE%^%^BOLD%^"+ETO->QCN+" speaks a quiet prayer over the bracelet on "+ETO->QP+" wrist.%^RESET%^\n",ETO);
   return 1;
}
...
int ask_fun(string str){
   ...
   write("%^WHITE%^%^BOLD%^The bracelet is holding the spell %^CYAN%^"+TO->query_property("instilled")+"%^WHITE%^.%^RESET%^\n");
   return 1;
}
...
int now_fun(string str){
   ...
   if (str){
     write("%^WHITE%^%^BOLD%^Concentrate more clearly: just %^CYAN%^now%^WHITE%^ will activate the spell.");
     return 1;
   }
