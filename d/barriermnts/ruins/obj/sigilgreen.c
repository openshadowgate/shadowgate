...
void create(){
        ::create();
        set_name("green sigil");
        set_id(({"sigil","Sigil","Green Sigil","green sigil"}));
        set_short("%^GREEN%^%^BOLD%^glowing green sigil%^RESET%^");
        set_long("%^BLACK%^%^BOLD%^This sigil is formed of stone, though strangely it seems to weigh "
"nearly nothing, and glows with its own inner %^GREEN%^green %^BLACK%^light.  It is carved into an "
