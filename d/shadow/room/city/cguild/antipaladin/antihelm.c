...
void create() {
   ...
   set_short("Helmet of the Order of Darkness");
   ...
   " the insignia of the Order of Darkness.");
    set_weight(10);
   set_value(30);
    set_type("armour");
    set_limbs( ({ "head" }) );
...
int wearit() {
	...
	return 0;
      ...
      tell_object(ETO,"%^RED%^You feel more protected by the helmet.%^RESET%^");
      tell_room(environment(ETO),"%^RED%^"+ETO->query_cap_name()+" glows with an inner light!%^RESET%^",ETO);
        return 1;
    }
}
int removeit() {
