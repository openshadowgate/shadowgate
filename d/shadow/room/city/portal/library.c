...
void create(){
   ...
   set_short("%^RESET%^%^MAGENTA%^A peaceful library%^RESET%^");
   set_long("%^RESET%^%^MAGENTA%^A peaceful library%^RESET%^\n"+
      ...
      "%^CYAN%^birds %^RESET%^to %^GREEN%^plants%^RESET%^ to "+
      ...
      "in various colors, and this is the only room in the enclave "+
   ...
   set_exits(([
      "north" : "/d/shadow/room/city/portal/fountain"
   ]));
}