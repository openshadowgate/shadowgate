...
void create(){
   ...
   set_long(
   	"%^BOLD%^%^BLACK%^"+
   	"You are in a tunnel that leads east and ends in a bare wall to the west."+
   	...
   	"look deliberate.  There is a hole in the ceiling leading "+
   ...
   set_items(([
   	...
   	({"chisel marks", "chisel mark", "marks", "mark"}) : "The marks "+
   		...
   		"last ale.  This section of wall has one mark.",
   	...
   	"hole" : "Hard to see much from down here, but with the right "+
   		"skill and/or the correct tools one could climb up."
  	]));
   set_exits(([
   	"east" : STEFANO+"tunnel19",
	]));

        set_climb_exits((["climb":({STEFANO+"sm_cave",20,6,100})]));
