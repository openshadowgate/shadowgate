...
void create() 
{
    ...
    "runs the length of the east wall. An opening to the "+
    "northwest leads into darkness.%^RESET%^");

    set_exits( (["northwest" : FFTR"26" ]) ); 	
    TO->add_item(({"crack", "east crack", 
...
int do_peer_stuff(string where) 
{
    ...
    if(where == "east crack") target_room = FFTR"28";
...
int squeeze_function(string str, string what, string where)
{
    ...
    if(sscanf(str, "%s %s", what, where)) 
    {
        ...
        if(where != "east crack") return 0;	
        targ = explode(where, " ")[0];
        if(where == "east crack") targ_room = FFTR"28";
        if(TP->query_bound() || TP->query_unconcious() || TP->query_paralyzed()
        || TP->query_tripped()) 
        {
            TP->send_paralyzed_message("info",TP);
