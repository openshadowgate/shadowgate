...
void create() 
{
    ...
    "length of the west wall. %^BOLD%^%^YELLOW%^The tunnel continues "+
    "into darkness toward the northeast and the south.%^RESET%^");

    set_exits( ([ "northeast" : FFTR"2", "south" : FFTR"9" ]) ); 
    TO->add_item(({"crack", "west crack", "large crack"}), "%^RESET%^%^MAGENTA%^This crack, upon closer "+
...
int do_peer_stuff(string where) 
{
    ...
    if(where == "west crack") target_room = FFCR"9";
...
int squeeze_function(string str, string what, string where)
{
    ...
    if(sscanf(str, "%s %s", what, where)) 
    {
        ...
        if(where != "west crack") return 0;	
        targ = explode(where, " ")[0];
        if(where == "west crack") targ_room = FFCR"9";
        if(TP->query_bound() || TP->query_unconcious() || TP->query_paralyzed()
        || TP->query_tripped()) 
        {
            TP->send_paralyzed_message("info",TP);
