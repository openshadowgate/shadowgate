...
void create() 
{
    ...
    "end here. A %^BOLD%^%^MAGENTA%^large crack%^BOLD%^%^YELLOW%^ "+
    "runs the length of the west wall. An opening to the "+
    "northeast leads into darkness.%^RESET%^");
    set_exits( (["northeast" : FFTR"14" ]) ); 	
    TO->add_item(({"crack", "west crack", 
    "large crack"}), "%^RESET%^%^MAGENTA%^This crack, upon closer "+
    "inspection, appears to continue into the earth. You could "+
    "%^RESET%^squeeze%^MAGENTA%^ into it "+
    "if you were bold enough to do so. You could also "+
    "possible %^RESET%^peer%^MAGENTA%^"+
...
int do_peer_stuff(string where) 
{
    ...
    if(where == "west crack") target_room = FFTR"31";
...
int squeeze_function(string str, string what, string where)
{
    ...
    if(sscanf(str, "%s %s", what, where)) 
    {
        ...
        if(where == "large crack" || where == "crack") where = "west crack";
        if(where != "west crack") return 0;	
        targ = explode(where, " ")[0];
        if(where == "west crack") targ_room = FFTR"31";
        if(TP->query_bound() || TP->query_unconcious() || TP->query_paralyzed()
        || TP->query_tripped()) 
        {
            TP->send_paralyzed_message("info",TP);
...
int peer_function(string str,string what,string where)
{
    ...
    if(sscanf(str,"%s %s",what,where))
    {
        if(what != "into") return 0;
        where = lower_case(where);
        if(where == "west crack" || where == "large crack" ||
        where == "crack") 
        {
            where = "west crack";
            do_peer_stuff(where);
