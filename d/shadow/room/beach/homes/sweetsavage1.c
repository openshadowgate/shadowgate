...
if (locked==1 && (string)this_player()->query_name() != "sweetsavage" &&!archp(this_player()))
   {
...
int lock_room()
{
  if ((string)this_player()->query_name() != "sweetsavage") 
    return 0;
  if(locked)
   {
...
int eject_player(string str) 
{
  ...
  tell_object(player, "Sweetsavage removes you "
                      "from the house.\n");
