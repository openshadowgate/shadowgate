...
void init() 
{
  ...
  if(locked==1  && (string)this_player()->query_name() == "zeek") 
   {
     write("You the back door.\n");
     return;
   }
      if (locked==1 && (string)this_player()->query_name() != "tuco" &&!archp(this_player()))
   {
...
int lock_room()
{
  if ((string)this_player()->query_name() != "tuco") 
    return 0;
  if(locked)
   {
...
int eject_player(string str) 
{
  ...
  tell_object(player, "Tuco removes you "
                      "from the house.\n");
