...
creating a sense of being in the open air despite the drab walls of the tunnels.
CIRCE
...
void reset() {
  int r;
  ::reset();
/* changing to have CROOM control these for wandering over longer reboots *Styx*  11/29/02
  if(!present("worker") && !random(2)) {
    new(MOBS"worker")->move(TO);
    new(MOBS"worker")->move(TO);
  }
*/
  if(!present("chest")){
    r = random(2);
    if(r==0) new("/d/hm_quest/chests/chest16")->move(TO);
    else new("/d/hm_quest/chests/chest23")->move(TO);
  }
}
