...
"%^ORANGE%^The pathway opens up here to a clearing of sorts, with downtrodden dirt covering "
...
", you can hear the roar of tumbling %^CYAN%^water%^ORANGE%^, towards which the rockier limits "
"of the path seem to lead.  Further north lies the rest of the clearing.%^RESET%^\n");
...
"and the dirt upon which you stand.%^RESET%^",
   ...
   set_exits(([ 
     "southwest":ROOMS"valley04",
     "north":ROOMS"valley06",
   ])); 
}
...
void reset() {
   ...
   if(!present("chakit")) find_object_or_load(MON"chakit")->move(TO);
}