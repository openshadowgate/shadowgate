...
void reset(){
   if(!present("uugaruf"))
     find_object_or_load(MON"uugaruf")->move(TO);
   switch(random(6)) {
     case 0:
...
"goes.  He disappears into the cavern beyond before you can even react.%^RESET%^");
     break;
     case 1:
     tell_room(TO,"%^RED%^Uugaruf watches a passing kobold with a baleful glare.%^RESET%^");
     break;
     case 2..3:
     tell_room(TO,"%^BLACK%^%^BOLD%^Uugaruf runs his gnarled hands over a crystalline shard, and then places "
