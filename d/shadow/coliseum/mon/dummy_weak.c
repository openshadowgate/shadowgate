...
void create()
{
    ...
    set_race("test dummy");
    set_body_type("human");
    set_short("%^BOLD%^%^BLACK%^An %^RESET%^weak %^BOLD%^%^BLACK%^looking %^RESET%^%^ORANGE%^dummy%^BOLD%^%^BLACK%^, %^RESET%^wo%^BOLD%^r%^RESET%^n %^BOLD%^%^BLACK%^and %^RESET%^%^RED%^b%^BOLD%^a%^RESET%^%^RED%^t%^BOLD%^te%^RESET%^%^RED%^r%^BOLD%^e%^RESET%^%^RED%^d%^WHITE%^");
