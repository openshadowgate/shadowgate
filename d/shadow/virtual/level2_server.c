...
inherit "/d/shadow/virtual/dinherit2";
inherit "/std/virtual/compile";

/* define a few macros... */
#define BASE            "/d/shadow/virtual/level2/"
#define _DAEMON   "/d/shadow/virtual/level2_daemon"
#define SUFFIX          ".level2"
...
/*  moved to the inherit to make it consistent with level 1 & making it track the total number so it doesn't fill up entirely over a course of a reboot.
...
void reset(){
   ...
   if(!present("spectre")){
	 ...
	 ob->move(TO);
	 ob->set_speed(50);
	 ob->set_property("swarm",1);
	 ob1 = new("/d/shadow/virtual/mon/smallu");
	 ob1->move(TO);
	 ob->add_follower(ob1);
         ob1 = new("/d/shadow/virtual/mon/smallu");
	 ob1->move(TO);
