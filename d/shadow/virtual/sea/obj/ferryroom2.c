...
Travel(){
    object loc;
  	remove_exit("out");
  	ob = find_object_or_load("d/shadow/virtual/sea/obj/ferryboat2");
...
{
    	in_dock = 1;
	//adding this because of 
	//Bad argument 1 to environment()
	//program: d/shadow/virtual/sea/obj/ferryroom2.c, object: 	//d/shadow/virtual/sea/obj/ferryroom2 line 103
	//'        Docking' in 'd/shadow/virtual/sea/obj/ferryroom2.c' 	
	//('d/shadow/virtual/sea/obj/ferryroom2')line 103
	//Error in call out.
	//in the debug.log - guessing it's getting started, then cleaned up 
	//sometimes right after this call out is made?  - Saide
	if(!objectp(ob)) 
	{
		ob = find_object_or_load("d/shadow/virtual/sea/obj/ferryboat2");
	}
      ...
      switch (dir)
	{
   	...
   	tell_room(environment(ob),"%^BOLD%^%^YELLOW%^A ferry boat enters the area.");
    	tell_room(TO,(string)environment(ob)->query_long()+"%^GREEN%^"+
	environment(ob)->query_smell("default")+"\n%^YELLOW%^"+
