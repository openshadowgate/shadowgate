//changed to match other archers, ignore invis. & fix other bugs - Styx 1/13/01
...
create() {    
	::create();
	    set_name("archer");
	        ...
	        set_gender("male");
	        set("short", "An archer");
                set("long", "An elite archer assigned to guard the city walls.  Dressed in chain mail, he's constantly alert for intruders into the city walls.\n");
	        set_level(14);  
	        set_hd(14,0);       
	        set_exp(900);    
	        set_body_type("human");    
	        ...
	        new(WEAPONDIR+"longsword")->move(TO);    
...
//so if there is an elf 2 or it will have the target id right - Styx 12/29/01
