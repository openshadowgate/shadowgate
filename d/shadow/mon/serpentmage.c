...
void catch_tell(string str){
	if((strsrch(str,"tell") == -1)) return;
	if((strsrch(str,"hello") != -1) && (step == 0)){ 
		call_out("reply_back",2,"Who is there?"); 
		call_out("reply_back",4,"What are you doing here?"); 
		call_out("reply_back",6,"You are invading my privacy!"); 
		call_out("reply_back",8,"Why are you bothering me?"); 
		call_out("reply_back",10,"Don't bother answering. Just go away."); 
		step = 1;
		return;
	}
	if((strsrch(str,"case") != -1) && (step == 1)){ 
	...
	if((strsrch(str,"open") != -1) && (step == 3)){ 
		...
		call_out("reply_back",8,"Do you think you will be able to wield that staff?");
...
void reply_back(string str){
	tell_room(environment(TO),"%^MAGENTA%^The Serpentmage Says:%^WHITE%^ "+str,TO); 
}
void re_close_it(){
