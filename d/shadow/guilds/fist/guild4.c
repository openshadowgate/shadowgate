...
set_smell("default","You smell the odor of the old blood that has long since dried into the fine brown patina on the floor.");
    set_exits( ([
"south":"/d/shadow/guilds/fist/guild2.c",
  "east":"/d/shadow/guilds/fist/guild5.c",
  "north":"/d/shadow/guilds/fist/guild7.c"
    ]) );
}
...
int no_vials(string str){
  if(!str || (str!="potion" && str!="vial" &&  str!="blue" && str!="kit"))
    ...
    tell_room(ETP,"%^BOLD%^%^BLUE%^"+TPQCN+" just tried to drink a healing potion. What a wimp!\n",TP);
    return 1;
  }
}
