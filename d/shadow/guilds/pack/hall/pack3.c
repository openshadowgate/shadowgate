...
"couch" : "This couch is a deep %^RED%^red%^RESET%^ colour. There is the symbol of the %^RED%^Shades of Death%^RESET%^ woven into the fabric. The symbol depicts two women with crossed swords standing by eachother, coloured black and gold. ",
"sign" : "The sign reads: %^BOLD%^%^BLUE%^Onyx quarry and ring creation room. NO ADMITTANCE!%^RESET%^",
]));
	set_exits(([
...
object ring;
    tell_object(this_player(),
"%^BOLD%^%^BLUE%^A graceful elf enters and offers you a new Onyx ring on a velvet cushion.%^RESET%^ ");
    tell_room(environment(this_player()),
"%^BOLD%^%^BLUE%^The female elf enters and gives "+
(string)this_player()->query_cap_name()+" a new black onyx ring.",
        ({ this_player() }) );
ring = new("/d/guilds/pack/obj/pack_ring.c");
