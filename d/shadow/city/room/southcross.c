...
void create(){
    ...
    set_name("Intersection of Melistel Avenue and West Wall Road");
    set_short("%^BOLD%^Intersection of %^RESET%^%^CYAN%^Melistel "+
        "Avenue %^BOLD%^%^WHITE%^and %^BOLD%^%^BLACK%^West Wall "+
        "Road%^WHITE%^, Shadow%^RESET%^");
    set_long("%^BOLD%^Intersection of %^RESET%^%^CYAN%^Melistel "+
        ...
        "An open door to the east leads to the %^BOLD%^%^CYAN%^War "+
        "Museum%^RESET%^. The %^BOLD%^%^BLACK%^city gates %^RESET%^lie "+
        "to the south.\n");
    ...
    set_exits(([
        "north" : ROOMS"melistelf",
        "south" : ROOMS"southgate",
        "east" : "/d/guilds/fist/hall/guild1",
        "northwest" : ROOMS"westwall6",
    ]));   
}      
