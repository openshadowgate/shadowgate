 	...
 	if(!str) {
  	...
  	if(str == "grate"){
  		if(member_array("up",TO->query_exits()) != -1) {
			...
			remove_exit("up");
			if(member_array("down",ROOMS"bardguild"->query_exits()) != -1) {
     		tell_room(ROOMS"bardguild","Someone closes the grate from down below.\n",TP);
     		ROOMS"bardguild"->remove_exit("down");
     		return 1;
     	}
...
int open(string str) {
    ...
    tell_room(ETP,"%^BOLD%^%^WHITE%^Merciful fresh air rushes in as "+TPQCN+" slides open the grate leading out of the sewers.%^RESET%^",TP);
     add_exit(ROOMS"bardguild","up");
       tell_room(ROOMS"bardguild","BOLD%^%^WHITE%^The grate in the street slowly slides open, filling the room with the putried stench of the sewers.RESET%^");
       ROOMS"bardguild"->add_exit(ROOMS"sewer1","down");
	   return 1;
     }
   }