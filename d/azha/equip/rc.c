...
int wear_func() {
  ...
  write("%^BOLD%^BLACK%^You draw the robe around yourself, concealing your inventory!");
  tell_room(ETP, "%^BOLD%^BLACK%^"+TP->query_cap_name()+
    " pulls on the robe!", ({ TP }));  
  return 1;
}
...
int pull(string str) {
	...
	if(str != "on hood") return 0;
	if(hood_on) {
		write("You're already disguised.");
		return 1;
	}
	if(!disguised = TP->query_disguised()) title = TP->query_env("TITLE");
	else title = TP->query_disguise();
	...
	write("%^BOLD%^BLACK%^You pull the hood over your head!");
	tell_room(ETP, "%^BOLD%^BLACK%^"+TP->query_cap_name()+
		" pulls the hood of the cloak on!", ({ TP }));
	TP->set("say string", "say");
	...
	TP->set_description("is completely shrouded in a dark robe that covers every inch of "+
		...
		TP->query_possessive()+" face is disguised beneath the voluminous hood of the robe."
	);
	hood_on = 1;
	return 1;
}
...
int reveal(string str) {
	...
	if(str != "myself") return 0;
	if(!hood_on) {
		write("You're already revealed.");
		return 1;
	}
	TP->set("say string", say);
	TP->set("describe string", sdesc); 
	...
	TP->setenv("MIN", enter);
	TP->setenv("MOUT", exit);
	write("%^BOLD%^BLACK%^You pull back your hood and reveal your features!");
	tell_room(ETP, "%^BOLD%^BLACK%^"+TP->query_cap_name()+
		" pulls off the robe and suddenly looks very different!", ({ TP }));
	hood_on = 0;
	return 1;
