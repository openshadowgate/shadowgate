/** -*-C++-*- **
 * fort2_daemon.c -- daemon that handles the exits, descriptions, etc for
 *  Dark Tower Random Level 2
 *
 * thorn@shadowgate -- 25 June 2000 -- creation
 *****/
#include <std.h>
#include <daemons.h>
...
/* define some macros... */
#define FILE_TERRAIN   "/d/azha/virtual/fort2/fort2.terrain"
#define FILE_DESC      "/d/azha/virtual/fort2/fort2.desc"
#define FILE_EXITS     "/d/azha/virtual/fort2/fort2.exits"
#define NUMDESCS 2
...
void load_terrain()
{
  ...
  /* the first line in the terrain file should be the x & y size of the
area */
  tell_object(find_player("thorn"), "Terrain size explode\n");
  s = explode( file[0], " " );

  if ( sizeof( s ) != 2 )
    error( "fort1_daemon::load_terrain() -- parse error #1\n" );

  if ( sscanf( s[0], "%d", max_x ) != 1 )
    error( "fort1_daemon::load_terrain() -- parse error #2\n" );

  if ( sscanf( s[1], "%d", max_y ) != 1 )
    error( "fort1_daemon::load_terrain() -- parse error #3\n" );
  ...
  for ( a = 0; a < sizeof( file ) - 1; ++a ) {
    tell_object(find_player("thorn"), "Terrain explode 2\n");
    s = explode( file[a + 1], " " );

    if ( sizeof( s ) < max_x )
      error( "fort1_daemon::load_terrain() -- parse error #4\n" );
    ...
    for ( b = 0; b < max_x; ++b ) {
      ...
      if ( s[b] == ".." )
        ...
        error( "fort1_daemon::load_terrain() -- parse error #5\n" );
...
/*****
 * void load_desc();
 *  -- fills in the 'lDesc' and 'sDesc' arrays with the list of
descriptions
 *****/
void load_desc()
{
   ...
   for (i=0 ; i<NUMDESCS ; i++) {
                ...
                tell_object(find_player("thorn"), "Desc explode\n");
...
*/
  tell_object(find_player("thorn"), "Exits explode\n");
  s = explode( file[0], " " );

  if ( sizeof( s ) != 2 )
    error( "fort1_daemon::load_exits() -- parse error #1\n" );

  if ( sscanf( s[0], "%d", max_x ) != 1 )
    error( "fort1_daemon::load_exits() -- parse error #2\n" );

  if ( sscanf( s[1], "%d", max_y ) != 1 )
    error( "fort1_daemon::load_exits() -- parse error #3\n" );
  ...
  for ( a = 0; a < sizeof( file ) - 1; ++a ) {
    tell_object(find_player("thorn"), "Exits explode 2\n");
    s = explode( file[a + 1], " " );
    if ( sizeof( s ) < max_x )
      error( "fort1_daemon::load_exits() -- parse error #4\n" );
    ...
    for ( b = 0; b < max_x; ++b ) {
      ...
      if ( strlen( s[b] ) != 2 )
        error( "fort1_daemon::load_exits() -- parse error #5\n" );
      ...
      if ( ( hb = strsrch( "0123456789ABCDEF", s[b][0..0] ) ) == -1 )
        error( "fort1_daemon::load_exits() -- parse error #6\n" );

      if ( ( lb = strsrch( "0123456789ABCDEF", s[b][1..1] ) ) == -1 )
        error( "fort1_daemon::load_exits() -- parse error #7\n" );
