...
void create(){
      ::create();
      set_name("Star Maze");
      set_short("%^ORANGE%^Star Maze");
      set_terrain(BARREN);
      set_travel(FRESH_BLAZE);
      set_property("indoors",1);
      set_property("light",1);
      set_long(
	     "%^ORANGE%^You are in a starry maze.  You are surrounded by deep darkness.  "
	     ...
	     "faint shimmering ORANGE lines leading off into the darkness.  "
       );
	...
	"plaque":"This is a small black plaque with shimmering, starry letters.  ",
      ...
      set_exits(([
         "northwest": MAZEPATH "cass4",
         "northeast": MAZEPATH "cass6"
       ]));
}
...
int read_plaque(string str){
        ...
        if (str=="plaque") {
          if (present("mundanoid",ETP)){
            ...
            tell_object(TP,"The plaque is inscribed with the word: CASSIOPEIA!\n");
          }
        }
        return 1;
      }
