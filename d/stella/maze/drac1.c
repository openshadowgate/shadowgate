...
void create(){
      ::create();
      set_name("Star Maze");
      set_short("%^GREEN%^Star Maze");
      set_terrain(BARREN);
      set_travel(FRESH_BLAZE);
      set_property("indoors",1);
      set_property("light",1);
      set_long(
	     "%^GREEN%^You are in a starry maze.  You are surrounded by deep darkness.  "
	     ...
	     "faint shimmering GREEN lines leading off into the darkness.  "
       );
...
int read_plaque(string str){
        ...
        if (str=="plaque") {
          if (present("mundanoid",ETP)){
            ...
            tell_object(TP,"The plaque is inscribed with the word: DRACONIS!\n");
          }
        }
        return 1;
      }
