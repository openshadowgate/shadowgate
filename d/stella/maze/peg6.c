...
void create(){
      ::create();
      set_name("Star Maze");
      set_short("%^WHITE%^Star Maze");
      set_terrain(BARREN);
      set_travel(FRESH_BLAZE);
      set_property("indoors",1);
      set_property("light",1);
      set_long(
	     "%^WHITE%^You are in a starry maze.  You are surrounded by deep darkness.  "
	     ...
	     "faint shimmering WHITE lines leading off into the darkness.  "
       );
	...
	"plaque":"This is a small black plaque with shimmering, starry letters.  ",
      ...
      set_exits(([
         "southwest": MAZEPATH "peg7",
         "southeast": MAZEPATH "peg5"
       ]));
}
...
int read_plaque(string str){
        ...
        if (str=="plaque") {
          if (present("mundanoid",ETP)){
            ...
            tell_object(TP,"The plaque is inscribed with the word: PEGASUS!\n");
          }
        }
        return 1;
      }
