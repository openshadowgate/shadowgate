...
void create()
{
	::create();
	set_exits( ([
	"northeast" : ROOMS+"e1807",
	"southeast" : ROOMS+"e1607",
	"southwest" : ROOMS+"e1605",
	]) );
	set_invis_exits(({"northeast","southeast","southwest"}));
...
//adding druids since it seems logical ~Circe~ 4/7/13
	...
	if(member_array(TP,global_array) != -1) {
		...
		switch(i) {
			case 0:
				TP->move_player(ROOMS+"e2006");
				break;
			case 1:
				TP->move_player(ROOMS+"e1511");
				break;
			case 2:
				TP->move_player(ROOMS+"e1803");
				break;
		}
