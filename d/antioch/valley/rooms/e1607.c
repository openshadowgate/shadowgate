...
void create()
{
	::create();
	set_exits( ([
	"northeast" : ROOMS+"e1708",
	"northwest" : ROOMS+"e1706",
	]) );
	set_invis_exits(({"northeast","northwest"}));
...
//adding druids since it seems logical ~Circe~ 4/7/13
	...
	if(member_array(TP,global_array) != -1) {
		...
		switch(i) {
			case 0:
				TP->move_player(ROOMS+"e1808");
				break;
			case 1:
				TP->move_player(ROOMS+"e1709");
				break;
			case 2:
				TP->move_player(ROOMS+"e1906");
				break;
		}
