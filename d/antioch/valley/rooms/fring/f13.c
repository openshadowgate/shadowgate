...
void create()
{
   ...
   set_travel(FOOT_PATH);
	...
	set_long(
	"%^BOLD%^%^GREEN%^You've gone three times around the fairy ring.\n"+
...
int go_northwest()
{
   write("%^BOLD%^%^WHITE%^The glow of the toadstools seems to fade.");
   return 1;
}
void reset(){
   ::reset();
   if(!present("peasblossom")){
      new(MONS"faerie")->move(TO);
   }
}
