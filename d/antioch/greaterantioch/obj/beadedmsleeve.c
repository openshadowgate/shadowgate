...
void create()
{
   ...
   set_name("medium staff sleeve");
   set_short("%^RESET%^%^ORANGE%^A medium %^BOLD%^%^YELLOW%^be%^BOLD%^%^CYAN%^ad%^BOLD%^%^BLUE%^ed %^RESET%^%^ORANGE%^leather sleeve%^RESET%^");
   set_id(({"sleeve","medium sleeve","medium sleeves","weapsleeve","leather sleeve","medium leather sleeve"}));
   set_long("%^RESET%^This is a weapons sheath meant for single medium-sized poled"+
