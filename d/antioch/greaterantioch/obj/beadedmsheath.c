...
void create()
{
   ...
   set_short("%^RESET%^%^ORANGE%^A medium %^BOLD%^%^YELLOW%^be%^BOLD%^%^CYAN%^ad%^BOLD%^%^BLUE%^ed %^RESET%^%^ORANGE%^leather sheath%^RESET%^");
   set_id(({"sheath","medium sheath","medium sheaths","medium Sheaths","medium Sheath","weapsheath","leather sheath","medium leather sheath"}));
   set_long("%^RESET%^This is a weapons sheath meant for single medium bladed"+
