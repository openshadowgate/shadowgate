...
void create()
{
   ...
   set_short("%^RESET%^%^ORANGE%^A large %^BOLD%^%^YELLOW%^be%^BOLD%^%^CYAN%^ad%^BOLD%^%^BLUE%^ed %^RESET%^%^ORANGE%^leather sheath%^RESET%^");
   set_id(({"sheath","large sheath","large sheaths","large Sheaths","large Sheath","weapsheath","leather sheath","large leather sheath"}));
   set_long("%^RESET%^This is a weapons sheath meant for single large bladed"+
