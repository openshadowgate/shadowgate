...
void create()
{
    ...
    set_long(
        "%^GREEN%^You are on %^GREEN%^Pine street%^RESET%^^RESET%^\nThis is close to the intersection of Pine and Crescent. To the west there is a sign hanging over the entrance of a house. The sign is engraved with the symbol of a leather worker. The smell of leather is thick in the air here. The street continues up to the northwest. To the southeast you can see the intersection of Pine and Crescent. There is a Healer and the guard %^ORANGE%^tower%^WHITE%^ that way.
"
        );
