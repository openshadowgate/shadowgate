...
%^CYAN%^This is the jailers' room where the criminals are checked in before being put into a cell. A cluttered desk sits in one corner and several well-worn chairs are scattered around the room. From the boot heel prints on the desk and the position of a couple of the chairs, it looks like the guards put their feet up and sleep in the chairs from time to time. The cell door look sturdy enough they probably don't need to worry about escapes though. There is a grate down by it. A few sooty oil lamps light the room barely enough to create shadows that just add to the oppressiveness. There is a book on the desk with a list of names scratched in it. A doorway south leads back to the barracks.");
    set_items(([
