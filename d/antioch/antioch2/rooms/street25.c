...
void create()
{
    ...
    set_long(
        ...
        "%^WHITE%^Right in front of you is a large palace. It has been built from large dun bricks that are sealed together with some mortar to keep the wind out. The entrance of the palace is here, and a large archway has been built. You can see the sharp spikes of a metal grate sticking out from the bottom that can be lowered at a moments notice to keep intruders out and the palace secure. Back to the south you can see the %^BOLD%^%^CYAN%^b%^RESET%^%^CYAN%^ar%^WHITE%^ and inn, and a crossroads where Oak street joins with Apple and Cedar. To the west is a large %^CYAN%^gate%^WHITE%^ that leads to Holly Lane and the residential section.
"
        );
    if (query_night() == 1) {
