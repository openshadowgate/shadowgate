...
void create(){
	::create();
	set_name("copper head");
	set_id(({"copperhead","Copperhead","snake"}));
	set_short("A copperhead snake");
	set_long(
@OLI
	This snake has a triangular head and a greenish body. It has long
fangs protruding from it's mouth. Small in size you are sure it 
can still hurt you.
OLI
	);
	set_hd(2,1);
