//anger_dark.c - inherit for bard rewards in Nereid's Riddle Quest.  Circe 12/10/03
#include <std.h>
#include <daemons.h>
...
int play(string str) {
   ...
   if(TP->query_bound() || TP->query_unconscious() || TP->query_paralyzed()) {
    ...
    new("/cmds/spells/d/_darkness")->use_spell(TP,TP,10,100,"bard");
    TP->set_paralyzed(1,"You are busy removing the light from the room.");
    return 1;
}
