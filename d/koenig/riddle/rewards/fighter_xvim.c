//fighter_xvim.c - Reward for the Riddle Quest.  Circe 1/27/04
#include <std.h>
#include "../../nereid.h"
...
CIRCE
   ...
   set_lore(
@CIRCE
This spiked gauntlet is sacred to warriors who follow the ways of Xvim.  Though many warriors may join the faith, only those who prove themselves truly dedicated are rewarded with a weapon such as this.  Those who wield it consider themselves blessed.
CIRCE
   );
   set_property("lore difficulty",12);
   set_wield((:TO,"wield_me":));
   set_unwield("You feel the strength of Xvim leave you as you sheath your weapon.");
   set_hit((:TO,"hit_me":));
}
...
int wield_me(string str){
        if((string)TPQN != owner) {
                ...
                " in pain as "+ETO->query_subjective()+" tries wield the spiked gauntlet.",ETO);
        ...
        tell_object(ETO,"%^BOLD%^You feel the blessings of Xvim as you wield the spiked gauntlet.");
        return 1;
}
...
int hit_me(object targ){
   ...
   if(!random(8)) {
      ...
      tell_object(targ,"%^BOLD%^%^GREEN%^"+ETOQCN+" hisses and backhands you with a spiked gauntlet!");
