...
void create()
{
  ...
  set_short("You are traveling deeper into the bowels of the earth.");
  set_long("%^RED%^
As you travel deeper into the earth below the crypt and graveyard you
begin to question your own sanity.  You even begin to wonder what in the
devils name you are doing down here.%^RESET%^%^BOLD%^");
  set_exits( ([
...
int do_keeper(int mystate) {
   ...
   switch(mystate) {
     ...
     case 2: tell_room(TO,"%^RESET%^%^RED%^Or maybe even BASH his skull in so you wouldn't have to hear his mind, anymore...%^RESET%^"); break;
