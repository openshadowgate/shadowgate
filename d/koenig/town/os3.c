...
"them, or maybe even try to %^BOLD%^pick%^RESET%^%^GREEN%^ one.%^RESET%^\n");
  ...
  set_items( ([
     "bushes":"The dominating color of rose here is amethyst.",
     "rose":"You take a closer look and notice that there are tints of red in these amethyst colored roses."
]) );
  set_exits( ([
     "east":"/d/koenig/town/os2",
     "west":"/d/koenig/town/os4"
]) );
  bloodrose_wait = 0;
}
void init() {
  ...
  if(present("bloodrose", TP))
{
  ...
  tell_room(environment(TP),TPQCN+" looks for a rose to pick but cant find one.",TP);
  return 1;
}
  bloodrose_wait = 1;
  new("/d/koenig/town/mon/atbloodrose.c")->move(this_object());
