...
int say_stuff(string str) {
   if( (strsrch(str,"yes") != -1 || strsrch(str,"Yes") != -1) && (step == 1)) {
      tell_room(ETO,"The Gatekeeper opens the gate and allows "+TPQCN+" to "
         "pass on into the town of Muileann.",TP);
      tell_object(TP,"The Gatekeeper opens the gate and allows you to pass on into"+
         " the town of Muileann.");
      TP->move_player("/d/koenig/town/room1");
      tell_room(ETO,"The Gatekeeper then turns on his "
                "heels and goes back inside the gatehouse to conclude unfinished "
                "business.");
      move("/d/shadowgate/void");
      remove();
      return 1;
   ...
   if((strsrch(str,"no") != -1) && (step == 1)) {
      reply_snarl("Then begone and leave me in peace!");
      tell_room(ETO,"The Gatekeeper whirls around and "
                "stamps back off into the gatehouse cursing those who would "
                "waste his time.");
      move("/d/shadowgate/void");
      remove();
      return 1;
