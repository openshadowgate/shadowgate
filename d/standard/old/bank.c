...
void create() {
    ...
    set("short", "The Merchant's Bank of Praxis");
    set("long",
	"Welcome to the Merchant's Bank of Praxis!\n"
	"The newly opened bank now keeps all its money in a nice vault "
	"through the door north, to insure extra security for its depositor's "
	"lifetime savings.  A sign by the teller details all commands.");
    set_items(
    ...
    set_exits( 
	      (["north":"/d/standard/bank_vault",
		"south":"/d/standard/alley1"]) );
    set_invis_exits( ({ "north" }) );
    set_door("vault", "/d/standard/bank_vault", "north", "bank key");
...
void reset() {
	...
	mon->set("long", "A big, ugly ogre hired to guard the "
		 "newly open bank.\n");
	mon->set_level(14);
