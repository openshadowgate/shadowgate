...
void create() {
    ...
    set("short", "Along West Road");
    set("long", (: this_object(), "is_dug" :));
    set_no_clean(1);
    set_items(
	(["road" : "There is a mound of loose dirt piled by its side.",
      ...
      set_exits( 
		(["north" : "/d/standard/west_road3",
		  "south" : "/d/standard/west_road1",
		  "east" : "/d/standard/medium"]) );
    dug = 0;
}
...
void reset() {
    ...
    remove_exit("down");
    call_other("/d/standard/hole", "fixing_a_hole");
}
...
int dig() {
    ...
    dug = 1;
    add_item_description("hole", "A hole where someone just dug into the ground by the mound by the road.");
    add_exit("/d/standard/hole", "down");
    call_other("/d/standard/hole", "digging");
    return 1;
}
