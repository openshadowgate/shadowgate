...
void init() {
    ...
    if(!living(this_player()) || !interactive(this_player())) 
      this_player()->move_player("d/standard/outlands/dust2", "north");
    add_action("mail", "mail");
}

void create() {
  ::create();
    set("short", "Outlands Post Office");
    set("long",
        "The Outlands Post Office is grimy and poorly kept. "
        "Outlaws come here to read and post mails.  Type <mail> to access "
        "your mailbox, or <mail name> to send mail to player name. "
        "The Outlands post office runs on the intermud system, meaning "
    ...
    set_exits( (["north" : "/d/standard/outlands/dust2"]) );
