...
int hitme(object targ){
   ...
   if(random(2)) return roll_dice(1,4);
   switch(random(10)){
   case 0..1:
      ...
      targ->set_paralyzed(roll_dice(2,6),"%^RED%^You are still reeling from the blow!%^RESET%^");
   ...
   case 9:
      if(targ->query_stoneSkinned()){
         ...
         targ->set_paralyzed(roll_dice(1,6),"%^BOLD%^%^WHITE%^You're still trying to regain control of your limbs!%^RESET%^");
      }
   }
   return 0;
}
