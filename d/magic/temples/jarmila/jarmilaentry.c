...
//Added a small garden following plot events and changed exit to south to improve mapping. Kismet.
#include <std.h>
...
void create()
{
    ...
    set_long("%^YELLOW%^The temple gardens%^RESET%^\n"
             ...
             "%^BOLD%^%^RED%^e%^RESET%^%^RED%^s %^BOLD%^%^WHITE%^and %^YELLOW%^su%^RESET%^%^ORANGE%^n%^YELLOW%^jew%^BOLD%^%^WHITE%^e%^BOLD%^%^RED%^l%^YELLOW%^s%^BOLD%^%^WHITE%^.  A small stone path leads deeper into the garden to the northeast. A tidy %^RESET%^"
    ...
    set_exits(([
                   "south" : "/d/antioch/antioch2/rooms/trail6",
                   "temple" : "/d/magic/temples/jarmila/jarmila",
                   "northeast" : "/d/magic/temples/jarmila/jarmilagarden",
               ]));
}
