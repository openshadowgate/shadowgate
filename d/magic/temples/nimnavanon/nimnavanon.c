...
void create(){
   ...
   set_long("%^BOLD%^%^GREEN%^The Temple of Nim'navanon%^RESET%^\n%^BOLD%^%^WHITE%^You have entered a large clearing deep in the heart of the forest, where a single ancient %^RESET%^%^ORANGE%^oak tree %^BOLD%^%^WHITE%^rises to incredible heights at its heart, its gnarled limbs clawing eternally skyward.  %^BOLD%^%^CYAN%^L%^RESET%^%^CYAN%^ig%^BOLD%^%^CYAN%^ht %^BOLD%^%^WHITE%^dapples the clearing, cast between its leaves as they wave overhead in the breeze.  At the base of the tree stands a rough-hewn altar, a single piece of jade with %^RESET%^%^ORANGE%^du%^RED%^l%^ORANGE%^l %^BOLD%^%^WHITE%^and rugged sides.  Its surface has been polished smooth, displaying the letters of Nim'navanon's charge in g%^YELLOW%^l%^BOLD%^%^WHITE%^ow%^YELLOW%^i%^BOLD%^%^WHITE%^ng amber against the %^RESET%^%^GREEN%^rich mossy hue %^BOLD%^%^WHITE%^of the stone.  Around the oak, much of the rest of this area looks to be new growth, with %^RESET%^%^GREEN%^young grass %^BOLD%^%^WHITE%^underfoot that is relatively clear of brush and leaves.  A circle of new %^BOLD%^%^GREEN%^sa%^YELLOW%^p%^BOLD%^%^GREEN%^lings %^BOLD%^%^WHITE%^questing upwards from the soil at the clearing's edges, beneath the eaves of the older trees that lie beyond.  Occasionally, a hint of company is suggested by the %^RESET%^%^MAGENTA%^rustling %^BOLD%^%^WHITE%^of undergrowth within their depths, a t%^BOLD%^%^CYAN%^r%^BOLD%^%^WHITE%^ill of birdsong, or a distant %^RESET%^%^BLUE%^howl %^BOLD%^%^WHITE%^carried upon the wind.\n%^RESET%^");
...
%^RESET%^%^CYAN%^Allies: %^RESET%^Ryorik
%^CYAN%^Enemies: %^RESET%^Nilith
