...
void create(){
   ...
   set_long(
   	...
   	"hall ends in an wooden door to the north."+
   	"%^RESET%^\n"
   );
   ...
   set_door("wooden door",GAB+"vestry","north","loviatar_holy_symbol");
   set_door_description("wooden door","The door is made of stout oak "+
   	"and is banded in iron.\n");
   set_string("wooden door","open","The door opens with a creak into the vestry.\n");
   set_locked("wooden door",0,"lock");
	(GAB+"vestry")->set_locked("wooden door",0,"lock");
}
void reset(){
	::reset();
	switch(random(10)){
