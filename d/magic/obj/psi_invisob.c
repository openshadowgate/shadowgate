...
void init() {
   ...
   if(living(environment(TO)) && TP==environment(TO) ) {
      if(TO->query_property("spell")) {
         tell_object(TP,"%^BOLD%^%^BLUE%^You feel secure, hidden from "+
            "prying eyes.  Type <appear> to become visible again. ");
         if(TP->query_invis()) {
            TO->remove();
            return;
         }
         tell_room(ETP,"%^BOLD%^%^BLUE%^Your eyes glance over the "+
...
void show_up() {
   ...
   tell_object(cloaker,"%^CYAN%^Suddenly, you are in full view of "+
      "everyone around you, their befuddled minds made clear.");
   tell_room(environment(cloaker),"%^CYAN%^"+cloaker->QCN+" suddenly "+
