// Note:  If something is changed here, please review if the following files also need the same fix or change since they are nearly identical: celemholder.c, cgenieholder.c, csalholder.c, ctreeholder.c, cplanarholder.c, ctreantholder.c  
...
void set_spell(object the_spell) { spell = the_spell; }
...
void heart_beat()
{
    ...
    if(freed){
        count++;
        if(count > 5){
            tell_room(environment(mon),"%^BOLD%^With the magic holding it fading away, the treant dissipates, leaving behind a single acorn!%^RESET%^");
            remove();
        }
    }
}
...
int cmd(string str){
    ...
    if(sscanf(str, "%s to %s", who, what) != 2)
        return notify_fail("Syntax: command treant to <jump up and down>\n");
    ...
    if(what[0..3] == "wear")
	   return notify_fail("Treants cannot wear anything.");
    if(what == "follow")
    {
