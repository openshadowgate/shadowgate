...
// added the caster check & return for bug prevention/fix  *Styx*  5/17/03
   ...
   for(num =0;num < numfoes;num++){
      ...
      tell_room(environment(mon),"%^BOLD%^%^RED%^The genie attacks "+tmp[num]->query_cap_name()+"!");
   }
   return;
}
...
void heart_beat(){
   ...
   if(!objectp(caster))
      remove();
   if(!freed && sizeof(caster->query_attackers()) ) // changed from object casting - Ares
      defend();
   if(freed){
      count++;
      if(count > 5){
          tell_room(environment(mon),"%^BOLD%^The pain being too great on this plane, the genie returns to its plane!");
         remove();
      }
   }
}
...
// was str[0..3], changed to what so this check will work *Styx*  5/17/03
   ...
   if(!mon->force_me(what))
      return notify_fail("You fail to command the genie to "+what+"!\n");
   return 1;
}
...
int dismiss(string str){
   ...
   tell_room(environment(caster),"%^BOLD%^"+caster->query_cap_name()+" dismisses the genie!",caster);
   tell_object(caster,"%^BOLD%^You dismiss the genie!\n");
   call_out("timed",1);
   return 1;
}
