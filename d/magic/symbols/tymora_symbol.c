...
#define GEMS ({"%^BOLD%^%^WHITE%^al%^BLUE%^ex%^WHITE%^an%^BLUE%^dr%^WHITE%^ite%^RESET%^","%^BOLD%^%^MAGENTA%^o%^BLUE%^r%^MAGENTA%^p%^WHITE%^a%^BLUE%^s%^MAGENTA%^e%^RESET%^","%^BOLD%^%^BLUE%^sapphire%^RESET%^"})
...
string metal;
int isset;
string GOD = "%^BOLD%^%^MAGENTA%^Tymora%^RESET%^";
...
void create(){
   ...
   set_id(({"holy symbol","symbol","tymora_holy_symbol",jewelry,gem+" "+jewelry}));
