...
#define GEMS ({"%^MAGENTA%^f%^BOLD%^%^WHITE%^luo%^GREEN%^r%^WHITE%^spa%^MAGENTA%^r%^RESET%^","%^GREEN%^jade%^RESET%^","%^BOLD%^%^WHITE%^white sardonyx%^RESET%^"})
...
string metal;
int isset;
string GOD = "%^RESET%^%^MAGENTA%^Oghma%^RESET%^";
...
void create(){
   ...
   set_id(({"holy symbol","symbol","oghma_holy_symbol",jewelry,gem+" "+jewelry}));
