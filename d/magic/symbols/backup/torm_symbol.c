...
#define GEMS ({"%^BLUE%^lapis lazuli%^RESET%^","%^WHITE%^%^BOLD%^diamond%^RESET%^","%^BOLD%^%^WHITE%^hy%^YELLOW%^a%^WHITE%^line%^RESET%^"})
...
void create(){
   ...
   set_id(({"holy symbol","symbol","torm_holy_symbol",jewelry,gem+" "+jewelry}));
