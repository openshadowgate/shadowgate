...
void screw_up(object targ);

void create(){
	object ob;
	::create();
	set_name("satyr");
	set_id(({"satyr","forest creature","Satyr"}));
	set_short("A satyr");
	set("long",
@OLI
	This little creature is half goat, half human.  Small horns protrude 
from its head.  Having a human torso is sharply contrasted by the thick 
coat of brown hair covering its goat-like body. 
OLI
);
	set_body_type("human");
...
void screw_up(object targ){
	tell_room(ETO,"%^BOLD%^%^CYAN%^The satyr plays a small tune on "+query_possessive()+" pipes!",targ);
	tell_room(targ,"%^BOLD%^%^CYAN%^You feel a pull on your mind from the music but you push it from your mind!");
}
...
void fear(object targ){
	...
	tell_room(ETO,"%^BOLD%^%^CYAN%^The satyr plays a small tune on "+query_possessive()+" pipes!");
	tell_room(ETO,"%^BOLD%^%^CYAN%^You feel a pull on your mind from the music but you push it from your mind!",targ);
	tell_object(targ,"%^BOLD%^%^YELLOW%^You feel your mind being over come by immense fear!!");
