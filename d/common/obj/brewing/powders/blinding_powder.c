...
int toss(string str)
{
    ...
    if (sscanf(str, "powder at %s", who) != 1) {
        return notify_fail("Syntax: toss powder at <target>\n");
    }
    ...
    if (TP->query_polymorphed()) {
        return notify_fail("Not in your current polymorphed form.\n");
    }
    if (TP->id("summoned monster") || TP->id("elemental")) {
        return notify_fail("Not you!\n");
    }
    ...
    if (ob->query_property("powdered")) {
        return notify_fail("Jeez, isn't once enough for a little while?\n");
    }
    ...
    if (!ob->reflex_save(10 + (TP->query_skill("thievery") * 4 / 5))) {
        ...
        perception_bane = TP->query_skill("thievery") / 5;
        ob->add_skill_bonus("perception", -perception_bane);
