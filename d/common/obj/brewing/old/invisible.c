...
int do_effect() {
  object spellob;
    	set ("effect_time", ((random(4)+1) * 60)) ;
//	drinker->set_invis();
   spellob=new("/cmds/spells/i/_invisibility");
   spellob->use_spell(drinker,drinker->query_name(),random(2)+random(2)+1,100,"mage");
   spellob->potion_dest_effect((random(4)+2) * 30);
    	return 1 ;
}
