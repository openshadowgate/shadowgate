...
int potion_effect(object drinker,int mystrength,string str) {
    ...
    tell_object(drinker,"%^GREEN%^Your limbs seem to grow more fluid and graceful.%^RESET%^");
    drinker->set_property("morale-boost",1); // prevent stacking with heroism, as this is a lesser vers.
    drinker->add_skill_bonus("athletics",2);
    drinker->add_skill_bonus("stealth",2);
...
int potion_backfire(object drinker,int mystrength,string str) {
    ...
    tell_object(drinker,"%^MAGENTA%^Your limbs seem to grow more heavy and cumbersome.%^RESET%^");
    drinker->set_property("morale-boost",1); // prevent stacking with heroism, as this is a lesser vers.
    drinker->add_skill_bonus("athletics",-2);
    drinker->add_skill_bonus("stealth",-2);
