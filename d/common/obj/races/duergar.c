...
"%^RED%^You can %^YELLOW%^<racial invisibility>%^RESET%^%^RED%^ once per "
"SG day (8 hours realtime).%^RESET%^\n");
   return 1;
}
...
int myracial(string str){
   ...
   if(!str || str != "invisibility") {
     tell_object(TP,"You can only use <racial invisibility> as a duergar.");
     return 1;
   }
   if(tracker > time()) {
   ...
   tracker = time() + 28800;
   new("/cmds/spells/i/_invisibility")->use_spell(TP,TP,(int)TP->query_level(),100,"mage");
   return 1;
}
