...
"%^RED%^You can %^YELLOW%^<racial step>%^RESET%^%^RED%^ once per SG "
...
int myracial(string str){
   ...
   if(!str || str != "step") {
     tell_object(TP,"You can only use <racial step> as an earth genasi.");
     return 1;
   }
   if(tracker > time()) {
   ...
   tracker = time() + 28800;
   new("/cmds/priest/_ghost_step")->use_spell(TP,TP,(int)TP->query_highest_level());
   return 1;
}
