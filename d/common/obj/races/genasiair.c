...
"%^RED%^You can %^YELLOW%^<racial fly>%^RESET%^%^RED%^ once per SG "
...
int myracial(string str){
   ...
   if(!str || str != "fly") {
     tell_object(TP,"You can only use <racial fly> as an air genasi.");
     return 1;
   }
   if(tracker > time()) {
   ...
   tracker = time() + 28800;
   new("/cmds/spells/f/_fly")->use_spell(TP,TP,(int)TP->query_level(),100,"mage");
   return 1;
}
