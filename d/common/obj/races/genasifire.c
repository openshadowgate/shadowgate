...
"%^RED%^You can %^YELLOW%^<racial shield>%^RESET%^%^RED%^ once per SG "
...
int myracial(string str){
   ...
   if(!str || str != "shield") {
     tell_object(TP,"You can only use <racial shield> as a fire genasi.");
     return 1;
   }
   if(tracker > time()) {
   ...
   tracker = time() + 28800;
   new("/cmds/spells/f/_fire_shield")->use_spell(TP,TP,(int)TP->query_level(),100,"cleric");
   return 1;
}
