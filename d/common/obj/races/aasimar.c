...
void create() {
   ::create();
   set_name("mark");
   set_id(({"mark","marking","racial mark","pale mark","pale marking"}));
...
"%^RED%^You can %^YELLOW%^<racial light>%^RESET%^%^RED%^ once per SG "
...
int myracial(string str){
   ...
   if(!str || str != "light") {
     tell_object(TP,"You can only use <racial light> as an aasimar.");
     return 1;
   }
   if(tracker > time()) {
   ...
   tracker = time() + 28800;
   new("/cmds/spells/l/_light")->use_spell(TP,TP,(int)TP->query_level(),100,"mage");
   return 1;
}
