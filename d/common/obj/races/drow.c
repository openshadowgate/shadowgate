...
"%^RED%^You can %^YELLOW%^<racial fire target>%^RESET%^%^RED%^ and "
"%^YELLOW%^<racial darkness>%^RESET%^%^RED%^ once per SG day (8 hours "
"realtime).%^RESET%^\n");
   return 1;
}
...
int myracial(string str){
   ...
   if(!str) {
     tell_object(TP,"You can only use <racial darkness> or <racial fire target> as a drow.");
     return 1;
   }
   if(str == "darkness") {
     if(tracker > time()) {
       tell_object(TP,"You've already used your innate darkness ability today.");
       return 1;
     }
     tracker = time() + 28800;
   ...
   if(sscanf(str,"fire %s",mytarg) == 1) {
     if(tracker2 > time()) {
       tell_object(TP,"You've already used your innate faerie fire ability today.");
       return 1;
     }
     ...
     tracker = time() + 28800;
     new("/cmds/spells/f/_faerie_fire")->use_spell(TP,present(mytarg,ETP),(int)TP->query_level(),100,"cleric");
     return 1;
   }
   tell_object(TP,"You can only use <racial darkness> or <racial fire target> as a drow.");
   return 1;
