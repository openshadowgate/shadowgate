...
void place_board()
{
  string which, desc;
  object obj;
  if(!present("wizboard", TO))
  {
	obj = new("std/bboard");
	obj->set_name("wiz");
	obj->set_id(({"board", "bulletin board","wizboard"}));
	obj->set_board_id("wizboard");
	obj->set_short("OOC wiz board");
	...
	obj->set_location("/d/common/boards/wizboard.c");
    obj->set_ooc_board(1);
