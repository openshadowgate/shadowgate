...
void place_board()
{
  string which, desc;
  object obj;
  if(!present("typosboard", TO))
  {
	obj = new("std/bboard");
	obj->set_name("typos");
	obj->set_id(({"board", "bulletin board","typosboard"}));
	obj->set_board_id("typoboard");
	obj->set_short("OOC typos board");
	...
	obj->set_location("/d/common/boards/typosboard.c");
    obj->set_ooc_board(1);
