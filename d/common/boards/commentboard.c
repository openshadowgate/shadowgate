...
void place_board()
{
  ...
  if (!objectp(present("board", TO))) {
    ...
    ob->set("long", "OOC "+desc+" board"+".  Please be constructive and "
       "offer meaningful suggestions/solutions to any complaints.");
    ob->set_max_posts(50);
    ob->set_location("/d/common/boards/commentboard");
//    ob->set_restricted_read();  // available to all
    ob->set_ooc_board(1);
