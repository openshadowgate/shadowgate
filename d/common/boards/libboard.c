...
void place_board()
{
  string which, desc;
  object obj;
  if(!present("libboard", TO))
  {
	obj = new("std/bboard");
	obj->set_name("wiz");
	obj->set_id(({"board", "bulletin board","libboard"}));
	obj->set_board_id("lib_board");
	obj->set_short("OOC lib board");
	...
	obj->set_location("/d/common/boards/libboard.c");
    obj->set_ooc_board(1);
