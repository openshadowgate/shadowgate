...
#define STEFHS "/d/player_houses/stefano/"
#define MAX_ITEMS 50

inherit "/std/vendor_pstore";
void make_me();
// inherit "/realms/styx/vendor_pstore"; // corrected to use the /std/ one *Styx* 6/05
object imp;
   ...
   switch(step) {
	...
	case 1:
	   tell_room(ETO, "%^BOLD%^%^BLUE%^A door in the side of the desk "
		...
		"%^BOLD%^%^BLUE%^.\n");
	   imp = new("/d/player_houses/mons/stefano_imp");
	   imp->move(ETO);
...
void pickup_actions(object cust, object *what, int step) {
   switch(step) {
	...
	break;
     case 3:
	tell_room(ETO, "%^BOLD%^%^BLUE%^"+TOQCN+" takes the cylinder, retrieves "
	   "some rolled-up paper from it, and hands it across to "
	   +cust->query_cap_name()+".", cust);
	tell_object(cust, "%^BOLD%^%^BLUE%^"+TOQCN+" takes the cylinder, retrieves "
	   "some rolled-up paper from it, and hands it across to you.");
