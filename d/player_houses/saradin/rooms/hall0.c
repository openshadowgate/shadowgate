...
void create(){
   ...
   set_long(
   	"%^BOLD%^%^BLACK%^"+
   	"This is a secret hallway leading to the southwest.  "+
   ...
   set_exits(([
		"southwest" : SROOMS+"hall",
		"northeast" : SROOMS+"living_area"
  	]));
...
void draining(object tp,int num){
         ...
         case(2):
        	...
        	case(9):
        		...
        		tell_room((SROOMS+"hall1"),"%^BOLD%^%^CYAN%^The grinding of stone against stone "+
      	...
      	case(10):
        	...
        	case(14):
				tell_room((SROOMS+"hall"),"%^BOLD%^%^CYAN%^The walls blocking the passages "+
					"to the northeast and the west slide slowly back up into the ceiling.\n");
				tell_room((SROOMS+"hall1"),"%^BOLD%^%^CYAN%^The wall blocking the passage "+
					"to the east slide slowly back up into the ceiling.\n");
         	...
         	(SROOMS+"hall")->remove_property("underwater");
   			...
   			add_exit(SROOMS+"hall","southwest");
            return;
    	}
    	num++;
