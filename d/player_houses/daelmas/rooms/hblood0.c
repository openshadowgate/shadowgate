...
void create(){
   ...
   set_exits(([
		"north" : DROOMS+"torture",
		"south" : DROOMS+"hblood1"
	]));
}
void init() {
  	::init();
  	add_action("mask_em","snap");
}
int mask_em(string str) {
   if(!str) {
    	tell_object(TP,"Snap what?");
     	return 1;
   }
  	if(str == "fingers"){
  		tell_object(TP,"%^BOLD%^You snap your fingers loudly");
		tell_room(ETP,"%^BOLD%^"+TPQCN+" snaps "+TP->query_possessive()+" fingers loudly.",TP);
		(DROOMS+"torture")->remove_exit("south");
		return 1;
 	}
}