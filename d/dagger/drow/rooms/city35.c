...
void create() {
   ...
   set_travel(PAVED_ROAD);
	...
	"stands a regal looking building. "
	"The %^RED%^red banners%^BLUE%^ of Lloth hang from the building "
	"displaying her %^BOLD%^%^CYAN%^spider symbol%^RESET%^%^BLUE%^ "
	"proudly. An open doorway to the north leads into the building. "
	"To the west lies the %^GREEN%^temple%^BLUE%^ "
	"walls which tower twenty feet above you.\n%^YELLOW%^You feel "
	"sleepy and detect the presence of an entity tapping "
	"into your mind.%^RESET%^"
    );
    set_listen("default",
	...
	"building":"%^BLUE%^You suspect this building houses some form of "
	"elite Drow forces.%^RESET%^",
...
void reset() {
    ::reset();
    if(!present("blood")) {
        new( OPATH "blood" )->move(TO);
        new( MPATH "drowp" )->move(TO);
    new("/d/dagger/drow/temple/mon/drider")->move(TO);
	new( MPATH "drowp" )->move(TO);
	new( MPATH "drowp" )->move(TO);
	new( MPATH "drowp" )->move(TO);
    }
