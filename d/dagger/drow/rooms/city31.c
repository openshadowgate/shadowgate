...
void create() {
   ...
   set_travel(PAVED_ROAD);
	...
	"the %^BOLD%^lake%^RESET%^%^BLUE%^ which "
	"surrounds the city. To the south lies the %^CYAN%^temple "
	"wall%^BLUE%^.%^RESET%^"
    );
    set_listen("default",
	...
	"walls.",
	"lake":"The lake is deep, black and dark. You decide not to "
	"take a dip.",
	"wall":"%^RED%^%^BOLD%^Upon the wall "
	"here is a single giant %^YELLOW%^golden rune%^RED%^.%^RESET%^",
	"rune":"%^BOLD%^%^YELLOW%^As you gaze upon the "
	"rune you gasp in horror. It is clearly the rune of "
	"invulnerability inscryable by only one "
	"being...%^RED%^Grazzt.\n%^BLUE%^Horror sinks in as you "
	"realize that the Drow have the aid of this God.%^RESET%^",
    ...
    set_exits(([
	"west":"/d/dagger/drow/rooms/city30",
	"east":"/d/dagger/drow/rooms/city32"
    ]));
}
