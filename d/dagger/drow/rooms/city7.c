...
void create() {
   ...
   set_travel(PAVED_ROAD);
	...
	" A simple soldier's barracks lies to the west. The road to the "
	"east leads along the south side of the temple."
    );
    set_listen("default",
	...
	"doorway":"The doorway lies to the west and hangs open leading into "
	"the barracks.",
    ...
    set_pre_exit_functions(({"west"}),({"scythe"}));
    set_exits(([
	"north":"/d/dagger/drow/rooms/city8",
	"south":"/d/dagger/drow/rooms/city6",
	"west":"/d/dagger/drow/rooms/brk6",
	"east":"/d/dagger/drow/rooms/city17"
    ]));
}
int scythe() {
    message("my_action",
	...
	"A %^RED%^%^BOLD%^scythe%^RESET%^ swings down on "+TPQCN+" as "
	"they pass through the doorway!"
    ,environment(TP),TP);
    TP->do_damage("head",(25));
    return 1;
}
