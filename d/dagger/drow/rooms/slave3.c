...
void create() {
   ...
   set_travel(FOOT_PATH);
    ...
    set_long(
	"%^BLACK%^%^BLUE%^You stand within the elven slave "
	"compound. The compound is surrounded by chain link fence. "
	"The fence is covered with barbed wire. Wandering around "
	"various areas of the compound elven men, women and children "
	"wander about with sad misery in their eyes. "
	...
	"in your head speaks: %^BLACK%^Seek the old man for the key "
	"of entrance.%^RESET%^"
    ]));
    set_exits(([
	"south":"/d/dagger/drow/rooms/slave2",
	"north":"/d/dagger/drow/rooms/slave4",
    ]));
}
void reset() {
    ::reset();
