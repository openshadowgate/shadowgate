...
void create() {
   ...
   "mysticism. Unlike drow mages, who still draw their energies from the "
    ...
    set_alignment(9);
    add_money("gold",750);
	//Commenting this out and making the priest have
	//the updated one - in azha - has an obvious short, 
	//otherwise is apparently the exact same file - same file 
	//name and everything - Saide
    //new ( OPATH "gemsee.c" )->move(TO);
	new("/d/azha/obj/gemsee.c")->move(TO);
    set_funcs(({"moves"}));
    set_func_chance(30);
    set_emotes(2, ({
...
int steal_func(string str) {
    write("%^RED%^A mystical force prevents your action!");
    return 1;
}
int moves() {
