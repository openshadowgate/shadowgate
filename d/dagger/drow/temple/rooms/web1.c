...
void create() {
   ...
   set_travel(LEDGE);
	...
	"not to touch them.",
        "glyphs":"%^RED%^The glyphs of chanting hang overhead of you.%^RESET%^"
    ]));
    set_exits(([
	"east": RPATH "web2",
	"southeast": RPATH "web3",
	"south": RPATH "web4",
	"southwest": RPATH "web5",
	"west": RPATH "web6",
	"northwest": RPATH "web7",
	"north": RPATH "web8",
	"northeast": RPATH "web9"
    ]));
...
string lng_fun(string str) {
	...
	str = 
	    ...
	    "Above you, hanging motionless in the air, are some mysterious "
	    "glowing glyphs."
	    "%^RED%^ ";
    } else {
	str =
...
int chant_func(string str) {
    ...
    if(present("syculess")) {
        notify_fail("The Syculess won't let you chant anything!");
        return 0;
    }
    if(str == "zifnia") {
        write(
        ...
        TP->move_player( RPATH "first20.c" );
        say(
    ...
    if(str == "ainfiz") {
        ...
        TP->move_player( RPATH "first21.c" );
        say(
	    "%^CYAN%^A strange moan comes to your ears!%^RESET%^"
        );	
        return 1;
