...
void create() {
   ...
   set_property("no teleport",1);
    ...
    set_long( (:TO,"lng_fun" :) );
    set_smell("default",
	"The smell of burning insence surrounds you."
    );
    set_listen("default","Evil chants come from the north.");
	...
	"light":"Towards the north is a flickering light.",
...
string lng_fun(string str) {
	...
	str = 
            "%^RED%^You stand within a hallway "
	    "which leads deeper into "
	    "the %^MAGENTA%^Drow temple of Lloth%^RED%^. The walls, ceiling "
	    "and floor are made of blackened stone slabs. The surface of the "
	    "slabs seems to absorb all forms of light. Ahead to the north"
	    " a flickering light can be seen. The sound of unintelligeble "
	    "chanting fills the hallway and seems to grow louder as you "
	    "move towards the light. The shadows here seem to reach out "
...
int exit_fun() {
	...
	return 0;
        ...
        write("%^BLUE%^A feeling of sickness passes through you."
	    "%^RESET%^"
        );
