...
void create() {
   ...
   set_short("%^BLUE%^Giant gar%^RESET%^");
	...
	"creature with long flexible jaws outlined with multiple "
   "rows of razor sharp teeth. Despite its large size, it "
	"moves at lightning fast speeds through the water "
   "and is always looking for a tasty meal to satisfy its hunger."
    );
    set_gender("neuter");
    set_hd(8,2);
    set_size(3);
   ...
   "%^BLUE%^The Giant Gar brushes its scaly skin against "
	"you as it swims by.%^RESET%^",
	}),0);
}
