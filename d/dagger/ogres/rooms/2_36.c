...
int break_func(string str){
    ...
    if(!str) {
        ...
        if(roll > smarts) {
            ...
            tell_room(ETP,""+TPQCN+" looks over the mechanisms... Then moves one of the smaller parts. \n\n",TP);
            tell_object(TP,"You find the perfect part to move to break the mechanism.\n");
            mechanism = 0;
            pow_rm->remove_power();
            return 1;
}
...
int fix_func(string str){
    ...
    if(!str) {
        ...
        if(roll > smarts) {
            ...
            tell_room(ETP,""+TPQCN+" looks over the mechanisms... Then moves one of the smaller parts. \n\n",TP);
            tell_object(TP,"You find the perfect part to move to start the flow of water.\n");
            mechanism = 1;
            pow_rm->add_power();
            return 1;
}
