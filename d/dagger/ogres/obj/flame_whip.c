...
" whip by an invisible force.%^RESET%^\n");
    set_lore("%^RED%^%^BOLD%^The Whip of Flames carries some conflicting and contradictory"+
" stories with it.  Some claim that the whip is forged in the unholy fires of evil planes"+
...
int wieldme()
{
    if((string)ETO->query_diety() == "istishia" || (string)ETO->query_diety() == "auril") {
        tell_object(ETO,"%^RED%^Wielding a whip such as this would most certainly offend your Goddess.");
      return 0;
   	}
   	if(((string)ETO->query_diety() == "kossuth") || ((string)ETO->query_diety() == "sune") || ((string)ETO->query_diety() == "loviatar")) {
        ...
        tell_room(environment(ETO),"%^RED%^"+ETO->QCN+" grasps the whip with ease.",ETO);
    return 1;
    }
   else{
