...
int do_stealing()
{
    ...
    for(i=0;i<sizeof(people);i++)
    {
        ...
        if(people[i] == TO || avatarp(people[i])) continue;
        else list += ({people[i]});
        continue;
    }

	if(!pointerp(list) || !sizeof(list)) return 2;
	person = list[random(sizeof(list))];
	if(!present(person, ETO)) return 1;
	inven = ({});
	inven = all_inventory(person);
	list = ({});
    if(!pointerp(inven) || !sizeof(inven)) return 1;
    for(i=0;i<sizeof(inven);i++)
    {
        if(!objectp(inven[i]) || !stringp(inven[i]->query_short())) continue;
        if(inven[i]->query_worn() || inven[i]->query_wielded()) continue;
        list += ({inven[i]});
        continue;    
	}
...
if (objectp(targ))	tell_room(ETO, "Thorby suddenly reaches up and grabs "+targ->query_cap_name()+" by the throat!", ({targ}));
if (objectp(targ))	tell_object(targ, "Thorby reaches up and grabs you by the throat!");
if (objectp(targ))	command("whisper "+targ->query_name()+" you have won, but we shall meet again!!!");
