...
%^BOLD%^You can see a bit of light shining thru the grate above your head.
GIL
);
...
set_listen("default", "%^BOLD%^%^BLUE%^You can hear the sounds of rats squeeking.");
  set_items(([
"grate" : "It's a metal sewer grate just over your head, it appears loose, perhaps you can open it and exit.",
...
"bricks" : "Granite bricks...tho theres so much muck and filth on them you couldn't hardly tell!",
  ...
  set_exits(([
     "east" : "/d/dagger/Daggerdale/sewers/level1/sewer1",
     "west" : "/d/dagger/Daggerdale/sewers/level1/sewer3",
]));
}
...
int open(string str){
if (str == "grate") {
  tell_object(this_player(),"The grate slowly creaks open, allowing you to squeeze out of the sewers.");
  say ("The grate slowly creaks open, allowing you to squeeze out of the sewers.");
  add_exit("/d/dagger/Daggerdale/streets/street41", "out");
}
return 1;
}
...
{
  ::reset();
  if(query_exit("out")) {remove_exit("out");
}
}
