...
set ("day long", "%^ORANGE%^Then suddenly, so suddenly that it is as if someone had removed a veil from before their eyes, the trees of the wood split apart, the slope drops away, and the great bowl of the Hollows spreads out before you. However, there is no way to get past the chasm so you must return from whence you came.%^RESET%^");
set ("night long", "%^CYAN%^Then suddenly, so suddenly that it is as if someone had removed a veil from before their eyes, the trees of the wood split apart, the slope drops away, and the great bowl of the Hollows spreads out before you.  However, there is no way to get past the chasm, so you must return from whence you came.%^RESET%^");
...
set_exits (([
"up" : "/d/dagger/Elvanta/forest/room89"
]));
set_items
(([
...
::reset();
if(!present("glaysa"))
new("/d/dagger/Elvanta/forest/mon/glaysa")->move(TO);
}
