...
set ("day long", "%^ORANGE%^As you walk along the way you see that the woods are shot through with slanting rays of clear light, asmoke with drifting dust motes, striking down from half-bared treetops, to cast pools of glowing gold on the leaf strewn floor.  Light and colour create a hazy texture, imparting an oddly dreamlike atmosphere to the scene.%^RESET%^");
set ("night long", "%^CYAN%^As you walk along the way you see that the woods are shot through with slanting rays of moonlight.  <Globes> show dust motes drifting through the trees, and cast light upon the leaf strewn floor.  Light from the <globes> and muted colours create a hazy texture, imparting an oddly dreamlike atmosphere to the scene.%^RESET%^");
...
set_exits (([
"northeast" : "/d/dagger/Elvanta/forest/room59",
"southwest" : "/d/dagger/Elvanta/forest/room61"
]));
set_items
(([
...
::reset();
if(!present("psdragon"))
new("/d/tharis/monsters/psdragon")->move(this_object());
}
