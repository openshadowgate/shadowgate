...
set ("day long", "%^ORANGE%^The pathway you have travelled upon has now disappeared within the <trees>.  You are now walking along an exquisite <walkway> that seems to connect the trees within this place.  Perhaps you have now found the city of Elvanta.  You notice ahead of you a small <building>, only one floor, but it is WITHIN the <tree> itself.  How is that possible?%^RESET%^");
set ("night long", "%^CYAN%^The pathway you have travelled upon has now disappeared within the <trees>.  You are now walking along an exquisite <walkway> that seems to connect the trees within this place.  Perhaps you have now found the city of Elvanta.  You notice the light coming from a small <building> ahead of you, only one floor, but it is WITHIN the <tree> itself.  How is that possible?%^RESET%^");
...
set_exits (([
"north" : "/d/dagger/Elvanta/forest/room11.c",
"southwest" : "/d/dagger/Elvanta/forest/room9"
]));
set_items
(([
...
::reset();
if(!present("bee"));
new("/d/dagger/Elvanta/forest/mon/bee")->move(TO);
