...
set ("day long", "%^ORANGE%^At the edge of you vision, the shadows return, furtive, cautious, a gathering of quick and formless ghosts that are there until the instant you look for them and then they are gone.%^RESET%^");
set ("night long", "%^CYAN%^At the edge of your vision, the shadows return, furtive, cautious, a gathering of quick and formless ghosts that are there until the instant you look for them and then they are gone.%^RESET%^");
...
set_exits (([
"north" : "/d/dagger/Elvanta/forest/room98",
"south" : "/d/dagger/Elvanta/forest/room96"
]));
set_items
(([
...
void reset(){
::reset();
if(!present("white demon"))
new("/d/dagger/Elvanta/forest/mon/wdemon")->move(TO);
if(!present("skeleton"))
new("/d/dagger/Elvanta/forest/mon/skeleton")->move(TO);
}
