...
void create(){
    ...
    set_door("ebony door", ROOMS "hallway2", "east", 0);
    ...
    set_search("chest",(:TO,"search_pile":));
    set_search("drawers",(:TO,"search_pile":));
}

void search_pile(){
    if(searched){
        tell_object(TP,"The drawers are empty and bare.");
        tell_room(ETP,"%^BOLD%^%^BLACK%^"+TP->QCN+" pokes around in the chest of "+
            "drawers.",TP);
        return;
    }
    else{
        tell_object(TP,"%^BOLD%^%^BLACK%^You find a small leather case left behind in "+
            "one of the drawers.%^RESET%^");
        tell_room(ETP,"%^BOLD%^%^BLACK%^"+TP->QCN+" pokes around in the chest of "+
            "drawers.",TP);
