...
void create() {
    ...
    set_short("%^MAGENTA%^A %^BOLD%^half-elven servant%^RESET%^%^MAGENTA%^, pushing a "+
    ...
    set_long("%^RESET%^%^CYAN%^Garbed in a tunic and leggings in the %^YELLOW%^T"+
        ...
        "%^RESET%^%^ORANGE%^m%^CYAN%^, this woman stands at attention with downcast "+
        "eyes. While at a glance she appears human, there are slender points to her "+
        "ears that bely a half-elven heritage. Occasionally, she tugs lightly at a "+
        "%^BOLD%^%^BLACK%^s%^RESET%^i%^BOLD%^lv%^RESET%^e%^BOLD%^%^BLACK%^r c%^RESET%^"+
        "h%^BOLD%^ok%^RESET%^e%^BOLD%^%^BLACK%^r %^RESET%^%^CYAN%^at her neck, "+
        "featuring a single beveled %^BOLD%^%^RED%^r%^RESET%^%^RED%^u%^BOLD%^by"+
        "%^RESET%^%^CYAN%^. She stands near a large %^BOLD%^%^BLACK%^se%^RESET%^r"+
        "%^BOLD%^v%^RESET%^i%^BOLD%^%^BLACK%^ng %^RESET%^c%^BOLD%^a%^RESET%^r%^BOLD%^"+
        "%^BLACK%^t%^RESET%^%^CYAN%^, laden with various covered dishes.\n\n%^BOLD%^"+
        "%^BLACK%^They will patiently wait on you until you %^WHITE%^dismiss room "+
        "service%^BLACK%^.%^RESET%^");
    set_gender("female");
    set_alignment(6);
    set_race("half-elf");
