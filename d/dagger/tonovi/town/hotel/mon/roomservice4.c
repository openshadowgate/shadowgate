...
void create() {
    ...
    set_short("%^MAGENTA%^An %^BOLD%^elven servant%^RESET%^%^MAGENTA%^, pushing a "+
    ...
    set_long("%^RESET%^%^CYAN%^Garbed in a tunic and leggings in the %^YELLOW%^T"+
        ...
        "%^RESET%^%^ORANGE%^m%^CYAN%^, this elven woman stands at attention with "+
        "downcast eyes. Such a servant is reserved for the prestigious guests of the "+
        "Duchal Suite, as a further indulgence in opulence. Occasionally, she tugs "+
        "lightly at a %^BOLD%^%^BLACK%^s%^RESET%^i%^BOLD%^lv%^RESET%^e%^BOLD%^"+
        "%^BLACK%^r c%^RESET%^h%^BOLD%^ok%^RESET%^e%^BOLD%^%^BLACK%^r %^RESET%^"+
        "%^CYAN%^at her neck, featuring a single beveled %^BOLD%^%^RED%^r%^RESET%^"+
        "%^RED%^u%^BOLD%^by%^RESET%^%^CYAN%^. She stands near a large %^BOLD%^"+
        "%^BLACK%^se%^RESET%^r%^BOLD%^v%^RESET%^i%^BOLD%^%^BLACK%^ng %^RESET%^c"+
        "%^BOLD%^a%^RESET%^r%^BOLD%^%^BLACK%^t%^RESET%^%^CYAN%^, laden with various "+
        "covered dishes.\n\n%^BOLD%^%^BLACK%^They will patiently wait on you until "+
        "you %^WHITE%^dismiss room service%^BLACK%^.%^RESET%^");
    set_gender("female");
    set_alignment(6);
    set_race("elf");
    set_hd(10,0);
