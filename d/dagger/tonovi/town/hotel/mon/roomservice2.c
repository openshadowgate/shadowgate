...
void create() {
    ...
    set_long("%^RESET%^%^CYAN%^Garbed in a tunic and leggings in the %^YELLOW%^T"+
        ...
        "%^RESET%^%^ORANGE%^m%^CYAN%^, this human woman stands at attention with "+
        "downcast eyes. Occasionally, she tugs lightly at a %^BOLD%^%^BLACK%^s"+
        "%^RESET%^i%^BOLD%^lv%^RESET%^e%^BOLD%^%^BLACK%^r c%^RESET%^h%^BOLD%^ok"+
        "%^RESET%^e%^BOLD%^%^BLACK%^r %^RESET%^%^CYAN%^at her neck, featuring a "+
        "single beveled %^BOLD%^%^RED%^r%^RESET%^%^RED%^u%^BOLD%^by%^RESET%^%^CYAN%^"+
        ". She stands near a large %^BOLD%^%^BLACK%^se%^RESET%^r%^BOLD%^v%^RESET%^i"+
