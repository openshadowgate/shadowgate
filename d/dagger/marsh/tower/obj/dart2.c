...
void create() {
	...
	"There is a small glyph of explosion on the side of it."
    );
    set_value(500);
    set_weight(4);
}
...
int throw_func(string str) {
  ...
  return 1;
	...
	"As the dart hits "+ob->query_cap_name()+" it "
	"%^YELLOW%^explodes%^RESET%^%^RED%^ with a loud bang!"
	"%^RESET%^"
    );
    tell_object(ob,"%^ORANGE%^"
	+ETO->query_cap_name()+" throws a dart towards you!\n%^RED%^"
	"As the dart hits you it %^YELLOW%^explodes%^RESET%^%^RED%^ "
	"with a loud bang!%^RESET%^"
    );
    tell_room(ETO,"%^ORANGE%^"
	+ETO->query_cap_name()+" throws a dart towards "+ob->query_cap_name()+"!\n%^RED%^"
	"As the dart hits "+ob->query_cap_name()+" it explodes with "
	"a loud bang!%^RESET%^"
    ,({ETO,ob}));
    ETO->set_paralyzed(2,"You are busy watching the impressive dart!");
    dam = (int)ETO->query_level();
    ob->do_damage(ob->return_target_limb(),random(dam)+(dam/3));
    TO->remove();
    return 1;
}
