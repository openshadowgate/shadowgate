...
void create() {
    ...
    set_listen("default",
        "A low cold moan rises from the south."
    );
    set_smell("default",
...
int exita_fun() {
    if(present("morlock")) {
        write("The morlock blocks your way!");
        return 0;
    }
    write("%^BOLD%^%^CYAN%^A long low howl echoes from somewhere above you.%^RESET%^"
    );
    return 1;
}
int exitb_fun() {
...
void reset() {
    ::reset();
    if(!present("morlock")) {
        new( MPATH "morlock.c" )->move(TO);
// changed from say(...) to tell_room() to stop bugging *Styx* 11/1/03, last chg. 7/98	tell_room(TO,"%^RED%^The morlock grins evilly as it enters.%^RESET%^");
    }
    tell_room(TO,"%^RED%^The morlock grins evilly to you.%^RESET%^");
}
