...
string lng_fun(string str) {
   ...
   "The %^GREEN%^land%^BLUE%^ below you is hard to make out clearly, as it is enveloped in %^BOLD%^%^BLUE%^shadows"
	...
	"in substance and some are virtually transparent. The ground below you is atleast a ten foot "
	"clear drop.%^RESET%^";
	return str;
    }
...
int fly_fun(string str) {
    ...
    if(str == "down") {
        write(
	    "%^CYAN%^You slowly fly down the side of the tower and land on the ground.%^RESET%^"
        );
        say(
	    "%^CYAN%^"+TPQCN+" slowly flys down the side of the tower.%^RESET%^"
        ,TP);
        TP->move_player( RPATH "s1.c","flying in the air");
        return 1;
    } else {
        write(
        ...
        ,TP);
        TP->move_player( RPATH "drop3.c","flying in the air");
        return 1;
    }
}
...
if(TP->is_class("thief")) {
	...
	TP->remove_property("falling");
        ...
        if(str == "down") {
	    write("You succeed in climbing to the base of the tower wall.");
	    TP->move_player( RPATH "s1.c","climbing down the tower wall");
	    return 1;
        } else {
	    write("You succeed in climbing ten feet or so up the tower's outer wall.");
	    TP->move_player( RPATH "drop3.c","climbing up the tower wall");
	    return 1;
        }
    }
}
