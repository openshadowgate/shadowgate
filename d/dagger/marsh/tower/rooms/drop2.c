...
string lng_fun(string str) {
   ...
   "The %^GREEN%^land%^BLUE%^ below you is hard to make out clearly, as it is enveloped in %^BOLD%^%^BLUE%^shadows"
	...
	"in substance and some are virtually transparent. The ground below you is atleast a thirty foot "
	"clear drop.%^RESET%^";
	return str;
    }
...
if(TP->is_class("thief")) {
	...
	TP->remove_property("falling");
	    ...
	    TP->move_player( RPATH "drop3.c","climbing down the tower wall");
	    return 1;
        } else {
	    write("You succeed in climbing ten feet or so up the tower's outer wall.");
	    TP->move_player( RPATH "drop1.c","climbing up the tower wall");
	    return 1;
        }
    }
}
