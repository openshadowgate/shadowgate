...
void create() {
	...
	"disappears to the north, up into the darkness."
	"%^RESET%^"
    ); 
	...
	"hook":"A rope hangs from the hook. The rope disappears to the north, up into the darkness.",
	"rope":"The rope appears to be secured to something up and to the north...perhaps it is for swinging on.",
    ]));
    set_exits(([
        "north": RPATH "d6",
        "south": RPATH "d1",
...
int swing_fun(string str) {
    ...
    write(
	"%^ORANGE%^You grab ahold of the rope and take a giant leaping swing to the north!"
        "%^RESET%^"
    );
    say("%^ORANGE%^"
	+TPQCN+" grabs the rope and takes a giant leaping swing to the north!"
	"%^RESET%^"
    ,TP);
    TP->move_player( RPATH "d6","swinging on a rope" );
    write("%^ORANGE%^You go swinging through the room with no floor!%^RESET%^");
    TP->move_player( RPATH "d9","swinging like a monkey on a rope" );
    write("%^ORANGE%^You land safely on the other side of the chute room and let go of the rope!\nThe "
	"rope slides away to the south.%^RESET%^"
    );
