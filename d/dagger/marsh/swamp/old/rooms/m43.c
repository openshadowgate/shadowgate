...
void create() {
   ...
   "To the north lies a large ruined keep. However, the moat surrounding it is "
   "quite full of slime filled water, and the keeps drawbridge is raised."
        "%^RESET%^"
    );
    set_listen("default",
   ...
   set_pre_exit_functions(({"north"}),({"exits_fun"}));
}
int exits_fun() {
    if(FLAG == 0) {
