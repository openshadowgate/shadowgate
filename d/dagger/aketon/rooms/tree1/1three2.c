...
This is a large bedroom with just a few ordinary decorations. Only a few pieces "
      ...
      "down towards the far end of the room. The floor in front of the "
   ...
   set_smell("default","You smell perfume within the room.");
   set_listen("default","It is quiet here.");
   set_exits( (["east":RPATH1+"1three3",
      "south":RPATH1+"1three6"]) );
}

void reset() {
   ::reset();
   if(sizeof(children(MPATH+"chelspan")) > 1)
      return;
   new(MPATH+"chelspan")->move(TO);
}
