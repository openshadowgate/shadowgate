...
You are on the steps around the giant tree. These steps connects the two "
      ...
      "tree trunk. Below you is the second level and more levels can be "
      "seen above.");
   set_items( (["steps":"These steps twist around the giant tree trunk.",
      ({"forest","kilkean"}):"The dense Kilkean forest is all around you.",
      "city":"You can see shadows moving around upon the branches below.",
      "levels":"You can see the second level of the tree branches beneath "
      "you and some more levels above you.",
      "trunk":"This is a party of the huge tree body."]) );
   set_exits( (["up":RPATH1+"1three1",
      "down":RPATH1+"1two1"]) );
...
int up_func() {
   ...
   if( obj = present("standing guard") ) {
      if( (int)SAVE_D->is_member("aketon_list",TPQN) == -1 ) {
         ...
         obj->force_me("say All citizens are welcomed to visit the city "
            "elders.");
         return 1;
      }
   }
