...
void create() {
   ::create();
   set_name("northwest cable");
   set_id( ({"cable","white cable","northwest_cable"}) );
   set("short","%^RESET%^%^WHITE%^white long cable");
   set("long","This cable is made of enchanted steel. It extends down to "
      "a huge tree towards the northwest. It is fixed in place on a large "
...
int slide_func(string str) {
   ...
   if(member_array(str,
      ({"northwest cable","nw cable","white cable","northwest_cable"})) == -1)
   ...
   tell_object(TP,"You wrap the leather around the cable and slide all the "
      "way down to the northwest with extreme speed!");
