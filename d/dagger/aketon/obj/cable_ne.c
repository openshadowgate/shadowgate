...
void create() {
   ::create();
   set_name("northeast cable");
   set_id( ({"cable","red cable","northeast_cable"}) );
   set("short","%^RESET%^%^RED%^red long cable");
   set("long","This cable is made of enchanted steel. It extends down to "
       "a huge tree towards the northeast. It is fixed in place on a large "
...
int slide_func(string str) {
   ...
   if(member_array(str,
      ({"northeast cable","ne cable","red cable","northeast_cable"})) == -1)
   ...
   tell_object(TP,"You wrap the leather around the cable and slide all the "
      "way down to the northeast with extreme speed!");
