#include "/d/dagger/aketon/short.h"
#include <daemons.h>

inherit MONSTER;

object *bad_list;

void create() {
   object obj;
   ::create();
   set_name("sopzil");
   set_id( ({"sopzil","assassin","hunter","elf"}) );
   set("race","elf");
   set_gender("female");
   set_short("%^RESET%^%^RED%^Sopzil the assassin");
   set_long("%^RED%^Sopzil the assassin, the wandering head hunter of "
      "Aketon, Assigned by the Queen herself to wipe out human traces in "
      "Aketon. She travels with swift silence and purpose seeking out "
      "those who need to be dead. She obeys the Queen's order without a "
       "tiny disbelief. Every human to her is a sin. Sins to her are not "
      "allowed to live a single minute more in this world. Not many "
      "people have had the chance to see her face. Many have often wondered "
      "when she passes them by and just grins if they should give thanks "
      "to the god that they are not human. She is reputed for her "
      "lightning deadly stabs and the accuracy in hitting the vital "
      "organs of her victim. When targeting those human beings with a "
      "reputation within the realms, many say she is even more nasty and "
      "dirty in her way of assassination. A strange bastard sword is what "
       "she wields and it never leaves her hands, even as she sleeps, she grabs "
      "the sword tightly within her arms like it is a part of her body. "
       "She wears a strange cloak which is made entirely of serpent scales. "
      "Rumor says this cloak is a royal gift of the Queen Teleris herself "
      "to the noble grey elves of Aketon.");
   set_hd(20,8);
   set_max_hp(400);
   set_hp( query_max_hp() );
   set_class("assassin");
   set_mlevel("assassin",20);
   set_guild_level("assassin",20);
   set_thief_skill("hide in shadows",120);
   set_thief_skill("move silently",120);
   add_search_path("/cmds/assassin");
   set_body_type("human");
   set_alignment(2);
   set_size(2);
   set("aggressive",0);
   set_exp(50000);
   set_property("no bows",1);
   set_property("no death",1);
   set_property("no steal",1);
   set_property("no animate",1);
   set_property("weapon resistance",1);
   set_mob_magic_resistance("average");
   set_stats("intelligence",20);
   set_stats("wisdom",25);
   set_stats("strength",25);
   set_stats("charisma",18);
   set_stats("dexterity",25);
   set_stats("constitution",20);
   set_wielding_limbs( ({"right hand","left hand"}) );
   set_overall_ac(-8);
   obj = new(OPATH+"asssword");
   obj->move(TO);
   obj->set_property("monsterweapon",1);
   command("wield sword");
   new(OPATH+"serpentcloak")->move(TO);
   command("wear cloak");
   new("/d/tharis/obj/bpowder")->move(TO);
   add_money( "electrum",1000 + random(1000) );
   add_money( "silver",1000 + random(1000) );
   add_money( "gold",2000 + random(1000) );
   set_stabbed_func( (: TO,"stabbed_func" :) );
   set_funcs( ({"crit"}) );
   set_func_chance(40);
   remove_property("swarm");
   set_speed(60);
   set_spoken("elven");
   bad_list = ({});
}

int kill_ob(object victim,int which) {
   int hold;
   if(member_array(victim,query_attackers()) == -1 &&
      member_array(victim,bad_list) == -1)
      call_out("do_it",1,victim);
   else if(member_array(victim,query_attackers()) == -1) {
      victim->set_paralyzed(15,"%^RED%^You cringe in terror!");
      call_out("crit_func",2,victim);
   }
   hold = ::kill_ob(victim,which);
   return hold;
}

void do_it(object victim) {
   if( !objectp(victim) )
      return;
   if( present( victim->query_name() ) ) {
      bad_list += ({victim});
      cease_all_attacks();
      victim->cease_all_attacks();
      command("stab "+victim->query_name());
      victim->set_paralyzed((10 + random(10)),"%^RED%^You are still "
         "recovering  from that stab!!");
      if( victim->query_highest_level() >= 30 ) {
         if(!present("blinding powder"))
            new("/d/tharis/obj/bpowder")->move(TO);
         command("throw powder at "+victim->query_name());
         call_out("crit_func",5,victim);
      }
      if( (victim->query_highest_level() >= 25) &&
         (victim->query_highest_level() < 30) ) {
         call_out("crit_func",8,victim);
      }
      if( (victim->query_highest_level() >= 20) &&
         (victim->query_highest_level() < 25) ) {
         if(!present("blinding powder"))
            new("/d/tharis/obj/bpowder")->move(TO);
         command("throw powder at "+victim->query_name());
      }
   }
   return;
}

void crit_func(object victim) {
   object *gutted;
   if( !objectp(victim) )
      return;
   if( query_property("gutted") )
      gutted = query_property("gutted");
   else
      gutted = ({});
   if( member_array(victim,gutted) != -1 )
      return;
   if( present( victim->query_name() ) ) {
      command("say Now you DIE!");
      command("grin");
      command("crit "+victim->query_name());
   }
   else {
      bad_list -= ({victim});
      command("grumble");
      command("say Another wuss.");
   }
   return;
}

void init() {
   ::init();
   if( wizardp(TP) )
      return;
   if( (string)TP->query_race() == "human" ) {
      set_spoken("common");
      if(TP->query_lowest_level() < 10) {
         command("say I have no interest in a human as punny as you!!");
         command("laugh "+TPQN);
         TP->run_away();
      }
      else if(TP->query_lowest_level() < 15) {
         command("say I have no interest such a weak human, "+TPQCN+"!!");
         command("cackle "+TPQN);
      }
      else if(TP->query_lowest_level() < 20) {
         command("say YEAH A HUMAN! I give you 20 seconds to run, "+
            TPQCN+"!!");
         command("cackle "+TPQN);
         call_out("kill_ob",20,TP,1);
      }
      else if(TP->query_lowest_level() < 30) {
         command("say GREAT A POWERFUL HUMAN! You better run as fast as "
            "you can AT ONCE, "+TPQCN+"!!");
         command("poke "+TPQN);
         call_out("kill_ob",10,TP,1);
      }
      else {
         command("say It is time to pay for your sin on my elven family "
            "now, TAKE THIS, "+TPQCN+"!!");
         command("growl "+TPQN);
         command("kill "+TPQN);
      }
      set_spoken("elven");
   }
}

void heart_beat() {
   ::heart_beat();
   if( !objectp(TO) )
      return;
   if( !objectp(ETO) )
      return;
   if(!present("blinding powder"))
      new("/d/tharis/obj/bpowder")->move(TO);
   if( !query_current_attacker() && (query_hp() == query_max_hp()/8) )
      set_hp(query_max_hp()/8);
   return;
}

void crit(object vic) {
   set_property("magic",1);
   tell_object(vic,"%^RED%^Sopzil slices repeatedly across your torso!");
   tell_room(ETO,"%^RED%^Sopzil slices repeatedly across "+
      vic->query_cap_name()+"'s torso!",vic);
   tell_object(vic,"%^RED%^As you feel the great pain in your chest, you "
      "feel a sword in your back!");
   tell_room(ETO,"%^RED%^Within no time, you see a sword being plunged "
      "into "+vic->query_cap_name()+"'s back!",vic);
   if(SAVING_D->saving_throw(vic,"paralyzation_poison_death",-2)) {
      vic->do_damage( "torso",40 + random(20) );
   }
   else {
      vic->do_damage( "torso",60 + random(40) );
      tell_object(vic,"%^RED%^You scream in pain!");
      tell_room(ETO,"%^RED%^"+vic->query_cap_name()+" screams in pain!",
         vic);
      vic->set_paralyzed(20,"%^RED%^You are still recovering from that "
         "stab!");
   }
   remove_property("magic");
}

void stabbed_func() {
   if(query_hp() < (query_max_hp()/8)) {
      tell_object(TP,"%^RED%^Sopzil cackles and moves aside before you "
         "hit his vital organ.");
      set_hp(query_max_hp()/8);
   }
   return;
}

int query_watched() {
   return 60;
}

void set_paralyzed(int time,string message) {
   return 1;
}
