...
void create() {
   ...
   ::create();
   set_name("weyzil");
   set_id( ({"weyzil","assassin","hunter","elf"}) );
   set("race","elf");
   set_gender("male");
   set_short("%^RESET%^%^RED%^Weyzil the assassin");
   set_long("%^RED%^Weyzil the assassin, the wandering head hunter of "
      ...
      "within his arms like it is a part of his body. He wears a strange "
       "cloak which is made entirely of serpent scales. Rumor says this cloak "
      "is a royal gift of the Queen Teleris herself to the noble grey "
      "elves of Aketon.");
   set_hd(20,8);
   set_max_hp(400);
   set_hp( query_max_hp() );
...
void crit(object vic) {
   ...
   tell_object(vic,"%^RED%^Weyzil slices repeatedly across your torso!");
   tell_room(ETO,"%^RED%^Weyzil slices repeatedly across "+
...
void stabbed_func() {
   if(query_hp() < (query_max_hp()/8)) {
      tell_object(TP,"%^RED%^Weyzil cackles and moves aside before you "
         "hit his vital organ.");
      set_hp(query_max_hp()/8);
   }
   return;
