...
int give_quest(object ob){
  ...
  if ((!objectp(ob)) || (!userp(ob))) return 0;
	...
	if(ob->query_lowest_level()<19){
		...
		tell_object(ob,"%^BOLD%^I can give you only this - a note leading to the location of the first piece of the sword.  One warning, my friend: Be sure to keep close watch over the pieces - they will disappear if they leave your possesion.");
