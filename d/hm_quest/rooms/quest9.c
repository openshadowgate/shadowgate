...
"%^RED%^h%^ORANGE%^r%^RED%^e%^RESET%^, perhaps hinting at some metallic residue within the earth itself.  One ancient tree has fallen at an angle, partly obstructing the path ahead.  It is very "
"%^MAGENTA%^quiet %^RESET%^here, though the silence is broken by an occasional rustle, or the eerie wailing call of some distant creature.\n");
    ...
    set_exits( ([
        "east" : "/d/hm_quest/rooms/quest8",
        "northwest" : "/d/hm_quest/rooms/quest10",
    ]) );
    set_items(([
...
"path.  It does fairly well to obstruct the way further northwest, though if you duck low you might be able to make your way beneath.",
    ]));
    add_pre_exit_function("northwest","GoThroughDoor");
}
