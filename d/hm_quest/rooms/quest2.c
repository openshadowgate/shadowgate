...
"%^RESET%^You stand at the far end of a %^YELLOW%^g%^BOLD%^%^WHITE%^o%^RESET%^l%^ORANGE%^d%^YELLOW%^e%^BOLD%^%^WHITE%^n %^RESET%^beach that stretches around the eastern side of "
...
"%^CYAN%^e %^RESET%^serves as a constant backdrop of sound to the otherwise quiet surrounds, waves chasing in eagerly towards the shore only to surrender and slink back again seconds later.  "
"Away to the west, above the %^BOLD%^%^BLACK%^r%^RESET%^oc%^BOLD%^%^BLACK%^k%^RESET%^y c%^ORANGE%^l%^RESET%^if%^BOLD%^%^BLACK%^f%^RESET%^s upon the highest point of this tiny island, the upper "
"reaches of a tall %^BLUE%^building %^RESET%^loom above the treeline.  The beach winds away to the south, serving as a pale border to the %^GREEN%^fo%^BOLD%^r%^YELLOW%^e%^RESET%^%^GREEN%^st"
"%^ORANGE%^e%^GREEN%^d %^RESET%^reaches further inland.  Only a few paces further to the northwest, a rocky outcropping supports the remains of a decayed %^ORANGE%^pi%^BOLD%^%^BLACK%^e%^RESET%^"
"%^ORANGE%^r%^RESET%^.  Occasionally %^BOLD%^%^WHITE%^birds %^RESET%^can be seen passing overhead, one of the few signs of life on this apparently desolate island.\n");
