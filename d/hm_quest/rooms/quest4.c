...
"serves as a constant backdrop of sound to the otherwise quiet surrounds, waves chasing in eagerly towards the shore only to surrender and slink back again seconds later.  Away to the west, "
"above the %^BOLD%^%^BLACK%^r%^RESET%^oc%^BOLD%^%^BLACK%^k%^RESET%^y c%^ORANGE%^l%^RESET%^if%^BOLD%^%^BLACK%^f%^RESET%^s upon the highest point of this tiny island, the upper reaches of a tall "
"%^BLUE%^building %^RESET%^loom above the treeline.  The beach winds away in both directions, serving as a pale border to the %^GREEN%^fo%^BOLD%^r%^YELLOW%^e%^RESET%^%^GREEN%^st%^ORANGE%^e"
"%^GREEN%^d %^RESET%^reaches further inland.  Occasionally %^BOLD%^%^WHITE%^birds %^RESET%^can be seen passing overhead, one of the few signs of life on this apparently desolate island.\n");

    set_smell("default","The fresh air carries the salty tang of the sea.");
    set_listen("default","Relentless waves pound the shoreline.");
    set_exits( ([
        "northeast" : "/d/hm_quest/rooms/quest3",
        "east" : "/d/hm_quest/rooms/quest5",
    ]) );
    set_items(([
...
"its walls cracked and darkened with age, while %^GREEN%^moss %^RESET%^and %^BOLD%^%^GREEN%^v%^YELLOW%^i%^BOLD%^%^GREEN%^nes %^RESET%^climb as they will upon its every surface.",
