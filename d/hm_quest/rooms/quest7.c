...
"%^RESET%^You stand at the far end of a %^YELLOW%^g%^BOLD%^%^WHITE%^o%^RESET%^l%^ORANGE%^d%^YELLOW%^e%^BOLD%^%^WHITE%^n %^RESET%^beach that stretches around the eastern side of the island.  "
...
"winds away to the north, serving as a pale border to the %^GREEN%^fo%^BOLD%^r%^YELLOW%^e%^RESET%^%^GREEN%^st%^ORANGE%^e%^GREEN%^d %^RESET%^reaches further inland, too densely grown in most "
"spots to allow access or any visibility of what might lie beyond.  It does thin a little to the southwest, though, and you wonder if you could make your way into the bushland here.  "
"Occasionally %^BOLD%^%^WHITE%^birds %^RESET%^can be seen passing overhead, one of the few signs of life on this apparently desolate island.\n");
    ...
    set_exits( ([
        ...
        "southwest" : "/d/hm_quest/rooms/quest8",
    ]) );
    set_items(([
...
"inner edge of the beach, giving way to thickly forested %^GREEN%^b%^ORANGE%^u%^GREEN%^sh%^BOLD%^%^GREEN%^la%^RESET%^%^GREEN%^nd %^RESET%^beyond.  It does thin a little to the southwest, "
"leaving you to wonder if you could force your way through.",
...
"constant pounding of the %^BLUE%^w%^BOLD%^%^BLUE%^a%^BOLD%^%^CYAN%^v%^BOLD%^%^WHITE%^e%^RESET%^%^CYAN%^s%^RESET%^.  There doesn't seem to be any other sign of habitation here.",
    ...
    add_pre_exit_function("southwest","GoThroughDoor");
}

int GoThroughDoor() {
    tell_object(TP,"%^BOLD%^%^WHITE%^You push your way up and into the forest where it seems to thin.  After a short struggle with tangled foliage, you find traces of an abandoned trail to "
"follow inland.%^RESET%^");
    if(!TP->query_invis())
      tell_room(TO,"%^BOLD%^%^WHITE%^"+TP->QCN+" presses on into the forest and disappears among the thickly clustered trees.%^RESET%^",TP);
    return 1;
}