...
"worn in several places, it must have been %^YELLOW%^breathtaking %^RESET%^to look upon when first made. The entire outer surface of it is covered in strange %^BOLD%^%^GREEN%^glyphs %^RESET%^"
...
void heart_beat() {
    ...
    if(holder != owner) {
      tell_object(ETO,"The scabbard of glyphs vanishes!");
      TO->remove();
    }
}
...
int sheath_fun(string str) {
    ...
    if(base_name(blade) != "/d/hm_quest/obj/blade1") {
    ...
    SHEATH = 1;
    blade->remove();
    set_short("%^RESET%^%^RED%^Scabbard of glyphs (blade sheathed)%^RESET%^");
    set_long("%^RESET%^This %^CYAN%^metallic %^RESET%^scabbard must have once been home to an impressive blade, for it is incredibly fine in its crafting. Obviously %^RED%^old%^RESET%^, and "
"worn in several places, it must have been %^YELLOW%^breathtaking %^RESET%^to look upon when first made. The entire outer surface of it is covered in strange %^BOLD%^%^GREEN%^glyphs %^RESET%^"
"that still bear a faint glow of power. A %^YELLOW%^golden %^RESET%^sword has been sheathed within, and there is a depression in the center of the protruding hilt that appears to have held a "
...
int insert_fun(string str) {
    ...
    if(str != "ruby into hilt" && str != "ruby into sword" && str != "gem into hilt" && str != "gem into sword") {
    ...
    if(base_name(gem) != "/d/hm_quest/obj/gem1") {
    ...
    tell_object(TP,"%^CYAN%^The magical energy around the gem and the sheathed weapon flares brightly as they are brought together, and the gem settles into place with a satisfying click. Finally the Sword of Destiny is complete!%^RESET%^");
    gem->remove();
