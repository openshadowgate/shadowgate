...
#define VERSION "Mistletoe version 3.0 May 09 1997"
#define AUTHOR  "pator"
...
int query_interval();
int query_worship();
mapping query_levelmap();
int check_known(string spell_name, int spell_level);
...
void init(){	
	...
	if((string)owner->query_class() != "druid")
        ...
        if( is_allowed_wizard(NAME) )
           { LEVELMAP = DRUIDWIZ; 
	     if (TP->query("spells_per_level") != LEVELMAP[lvl])
		{ TP->set("spells_per_level", LEVELMAP[lvl]); }
...
mapping query_levelmap() { return LEVELMAP; }

// Check if a spell is known
int check_known(string spell_name, int spell_level) {
string spell_file_name;
      spell_file_name= replace_string( spell_name, " " , "_");
		// Is it in the known list
	if ( member_array(spell_file_name, keys(known_spells)) != -1 ) 
			// Is it the correct level
//           if ( known_spells[spell_name] == spell_level )
			return 1;
	return 0;
}
