...
void init() {
   ...
   TP->set("subrace","human");
   show_race();
}

void show_hair() {
   write("%^BOLD%^%^GREEN%^You are a pureblood yuan-ti, as this is the only subrace that Shadowgate offers. However your subrace is set to <human> to allow you to pass for one in roleplay situations.");
   write("%^BOLD%^%^WHITE%^Please ensure you roleplay the race appropriately. You are not a human with a tuned-up statline!");
