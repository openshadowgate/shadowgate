...
"itself.");
   ...
   set_hit((:TO,"hit_func":));
   set_prof_type("maelstrom scimitar");
...
"up the blade, but thinks better of it.%^RESET%^",ETO);
   ...
   if(sizeof(ETO->query_wielded())) {
     ...
     for(i = 0; i < sizeof(mywielded); i ++) {
       if((string)mywielded[i]->query_name() == "%^BOLD%^%^BLACK%^darksteel scimitar%^RESET%^") {
         myflag = 1;
         myob = mywielded[i];
       }
     }
     if(myflag) {
       tell_object(ETO,"%^BOLD%^%^BLACK%^You grasp the blade, and a "
"sudden %^RESET%^%^MAGENTA%^shu%^RED%^dd%^MAGENTA%^er %^BOLD%^%^BLACK%^"
"runs down your arm as it reacts with the one you already hold.  An "
"earsplitting %^YELLOW%^c%^WHITE%^ra%^RED%^c%^YELLOW%^k %^BLACK%^echoes "
"forth as both blades shatter!%^RESET%^");
     tell_room(EETO,"%^BOLD%^%^BLACK%^"+ETO->QCN+" grasps the blade, and a "
"sudden %^RESET%^%^MAGENTA%^shu%^RED%^dd%^MAGENTA%^er %^BOLD%^%^BLACK%^"
"runs down "+ETO->QP+" arm as it reacts with the one "+ETO->QS+" already "
"holds.  An earsplitting %^YELLOW%^c%^WHITE%^ra%^RED%^c%^YELLOW%^k "
"%^BLACK%^echoes forth as both blades shatter!%^RESET%^",ETO);
       ETO->do_damage("torso",random(20)+100);
       myob->remove();
       TO->remove();
       return 0;
     }
...
" scimitar.%^RESET%^",ETO);
     break;
     case 3:
       tell_object(ETO,"%^BOLD%^%^BLACK%^You take up the scimitar, and hear a "
"peal of thunder somewhere in the distance.");
