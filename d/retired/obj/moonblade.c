//updated 9/2/2019 to make it an estoc - Odin
//updated 3/30/2020 to include set_special_material_type
#include <std.h>
inherit "/d/common/obj/weapon/estoc.c";
...
{
    ::create();
    set_name("mithril estoc");
    set_id(({ "estoc", "sword", "mithril estoc", "blade", "engraved estoc", "syolkiir", "Syolkiir", "wild star", "moonbladex" }));
    ...
    set_obvious_short("%^RESET%^a %^MAGENTA%^de%^RED%^li%^MAGENTA%^cat%^BOLD%^e%^RESET%^%^MAGENTA%^ly %^RESET%^engraved %^BOLD%^%^WHITE%^mith%^CYAN%^r%^WHITE%^il %^RESET%^estoc");
    set_long("This weapon is simple and efficient in design, a estoc with a straight blade of nearly four "
             ...
             "armored foes.  In from each edge, the blade broadens in a lenticular shape, but is prevented "
    ...
    obvshort = "%^RESET%^a %^MAGENTA%^de%^RED%^li%^MAGENTA%^cat%^BOLD%^e%^RESET%^%^MAGENTA%^ly %^RESET%^engraved %^BOLD%^%^WHITE%^mith%^CYAN%^r%^WHITE%^il %^RESET%^estoc";
    set_heart_beat(1);
...
int wield_func()
{
    ...
    if ((owned && (string)ETO->query_name() != owner)) {
    ...
    if (!owned) {
        ...
        tell_room(EETO, "%^BOLD%^%^WHITE%^The estoc rests in a light and balanced hold in " + ETO->QCN + "'s hand."
                  "%^RESET%^", ETO);
    }
...
int unwield_func()
{
    tell_object(ETO, "%^BOLD%^%^WHITE%^You release your hold on the %^BLACK%^com%^RESET%^%^ORANGE%^fo%^BOLD%^"
                "%^BLACK%^rtab%^RESET%^%^ORANGE%^l%^BOLD%^%^BLACK%^e %^WHITE%^grip of the estoc.%^RESET%^");
    tell_room(EETO, "%^BOLD%^%^WHITE%^" + ETO->QCN + "%^BOLD%^%^WHITE%^ releases hold on the grip of the estoc."
              "%^RESET%^", ETO);
    ETO->set_property("good item", -1);
    ETO->set_property("master weapon", -1);
    return 1;
...
int hit_func(object targ)
{
    ...
    if ((rand > 0 && rand < 5) || (rand == 10 && present("elfshadow", EETO))) {
        tell_object(ETO, "%^BOLD%^%^CYAN%^The first rune upon the estoc's blade flares %^GREEN%^br%^RESET%^"
                    "%^GREEN%^i%^BOLD%^llia%^YELLOW%^n%^GREEN%^tly %^CYAN%^as it draws a line of blood across " + targ->QCN + "'s "
                    "flesh!%^RESET%^");
        tell_object(targ, "%^BOLD%^%^CYAN%^The first rune upon the blade of " + ETO->QCN + "'s estoc flares "
                    "%^GREEN%^br%^RESET%^%^GREEN%^i%^BOLD%^llia%^YELLOW%^n%^GREEN%^tly %^CYAN%^as it draws a line of blood across "
                    "your flesh!%^RESET%^");
        tell_room(EETO, "%^BOLD%^%^CYAN%^The first rune upon the blade of " + ETO->QCN + "'s estoc flares "
                  "%^GREEN%^br%^RESET%^%^GREEN%^i%^BOLD%^llia%^YELLOW%^n%^GREEN%^tly %^CYAN%^as it draws a line of blood across "
    ...
    if (rand > 3 && rand < 7) {
        tell_object(ETO, "%^BOLD%^%^GREEN%^The estoc's second rune emits a sudden flash of %^RESET%^%^GREEN%^"
                    "virid%^YELLOW%^i%^RESET%^%^GREEN%^an %^BOLD%^light.  " + targ->QCN + " struggles to blink away the blinding "
                    "afterimage.%^RESET%^");
        tell_object(targ, "%^BOLD%^%^GREEN%^" + ETO->QCN + "'s estoc emits a sudden flash of %^RESET%^%^GREEN%^"
                    "virid%^YELLOW%^i%^RESET%^%^GREEN%^an %^BOLD%^light, and the world disappears in a blinding white afterimage!"
                    "%^RESET%^");
        tell_room(EETO, "%^BOLD%^%^GREEN%^" + ETO->QCN + "'s estoc emits a sudden flash of %^RESET%^%^GREEN%^"
    ...
    if (rand > 5 && rand < 8) {
        tell_object(ETO, "%^BOLD%^%^BLUE%^The third rune upon the estoc %^GREEN%^c%^YELLOW%^r%^GREEN%^ack"
                    "%^WHITE%^l%^GREEN%^es %^BLUE%^with light, and a sudden burst of energy races down the blade to throw "
                    + targ->QCN + " to the ground!%^RESET%^");
        tell_object(targ, "%^BOLD%^%^BLUE%^The third rune upon " + ETO->QCN + "'s estoc %^GREEN%^c%^YELLOW%^r"
        ...
        tell_room(EETO, "%^BOLD%^%^BLUE%^The third rune upon " + ETO->QCN + "'s estoc %^GREEN%^c%^YELLOW%^r"
                  "%^GREEN%^ack%^WHITE%^l%^GREEN%^es %^BLUE%^with light, and a sudden burst of energy races down the blade to "
    ...
    if (rand > 7 && rand < 10) {
        ...
        tell_object(ETO, "%^YELLOW%^The estoc's fourth rune %^RESET%^%^GREEN%^pul%^BOLD%^s%^RESET%^%^GREEN%^"
    ...
    if (rand == 10 && !present("elfshadow", EETO)) {
        tell_object(ETO, "%^BOLD%^%^WHITE%^The fifth rune upon the estoc's blade lights up with a %^RESET%^"
