...
"the world with a %^RESET%^%^BLUE%^savage gaze%^BOLD%^%^BLACK%^.%^RESET%^\n");
   set_weight(4);
   set_value(4000);
   set_type("leather");
   set_limbs(({"head"}));
...
int wear_fun() {
   ...
   if(ETO->query_highest_level() < 30) {
     tell_object(ETO,"The visor doesn't seem to fit you.");
     tell_room(EETO,"The visor doesn't seem to fit "+ETO->QCN+".",ETO);
     return 0;
   }
   tell_object(ETO,"%^BOLD%^%^BLACK%^A great weight settles on your shoulders, and you "
"feel the grasp of the wilds as you wear the visor.%^RESET%^");
