//	/realms/pator/castle/castleE3.c
...
int mouse_hole() {
	if (present("castle mouse")) {
		...
		message("my_action","When you look at the hole, a little mouse notices you and comes out of the hole. Maybe it is hungry, maybe it is just curious !",this_player());
                new("/d/deku/open/castle/monster/C_mouse")->move(this_object());
	      }
      return 1;}
...
void create() {
   ...
   set_travel(PAVED_ROAD);
    ...
    set_long(
@PATOR
You are in a room near the eastern tower. When you look around you see thick stone walls, further there isn't much to see other than some mouse holes in the corners.
For those who want to climb the castletower there is nothing to be pleased about. The staircase is blocked by a pile of rubble. Also a sign tells you :
' UNDER CONSTRUCTION '.
PATOR
    );
...
set_listen("default","You hear not much, but maybe you notice some mouses behind the walls.");
    set_exits(([
       "north" : CASTLE+"castleE4",
       "west"  : CASTLE+"castleE2",
    ] ));
    set_items(([
       "hole" : (: "mouse_hole" :),
