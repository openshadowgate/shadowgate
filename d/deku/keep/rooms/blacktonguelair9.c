...
void create()
{
	int x;
    	::create();
   	set_long(TO->query_long() + "%^BOLD%^%^WHITE%^  The corridor extends "+
	"to the north and to the south.  You may travel in either direction "+
	...
	for(x = 0;x < sizeof(LITEMS);x++) 
	{
 	...
 	set_exits(([
		"south": KEEPR + "blacktonguelair8","north" : KEEPR + "blacktonguelair10",
	] ));	
}
