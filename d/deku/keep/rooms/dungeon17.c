...
void create()
{
	...
	"here, leaving you with two options.  You may either descend into "+
	"the depths of the darkness or find a way to open the %^RED%^rusty "+
	...
	"low and inset directly into the center of it is a rusty metal door.%^RESET%^");
    	...
    	set_exits(([
		"down" : KEEPR + "dungeon16", "up" : KEEPR + "towerN2",
    	] ));
	set_door("rusty door",KEEPR + "towerN2","up","blacktongue master key","lock");
	...
	"square and made from a solid metal that has rusted over the course "+
	"of time.%^RESET%^");
}
