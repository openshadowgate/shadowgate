...
void create() {
  int x;
  ::create();
 	set_long(TO->query_long() + "%^BOLD%^%^WHITE%^  A %^RESET%^stone wall%^BOLD%^%^WHITE%^ runs "+
			"along the outside of the graveyard here.%^RESET%^");
  ...
  set_exits(([
			"east" : GY_ROOMS +"grave8",
   "south":GY_ROOMS +"grave7-1",
   "west" : GY_ROOMS +"grave8-2"
  ] ));
}
