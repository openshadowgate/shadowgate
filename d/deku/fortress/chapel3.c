...
void create() {
    ...
    "seems to %^RESET%^sparkle%^RED%^.  The western all of this room is littered with %^BLUE%^"+
    ...
    "sacrifice, at least not a living one.  An %^BLUE%^open doorway%^RED%^ in the eastern wall "+
    "of this room leads back to the foyer.%^RESET%^\n");
 
    set_exits(([
    "east" : GY_ROOMS +"chapel1",
    ] ));
    ...
    set_items(([
    ({"ashen altar","ash alatar","altar","stone altar"}) : "%^BOLD%^%^WHITE%^This small altar "+
