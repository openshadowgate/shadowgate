...
void create() {
  ...
  set_exits(([
  "north":GY_ROOMS +"grave4-6",
  "south":GY_ROOMS +"grave2-6",
  "east" : GY_ROOMS +"grave3-5"
  ] ));
  do_tombs();
}
...
void do_tombs() {
 int x;
  ...
  && member_array("tomb",query_exits()) == -1) {
  
    TO->add_exit(GY_ROOMS+"tomb1","tomb");
    set_door("glowing door",GY_ROOMS +"tomb1","tomb");	
    set_string("glowing door", "open", "%^CYAN%^The door swings "+
    "silently outward.%^RESET%^");
    set_door_description("glowing door","%^BOLD%^%^CYAN%^This "+
    "glowing door is made from an unknown substance.  It "+
    "doesn't appear to be solid yet somehow it manages to "+
    "completely obscure any attempt at viewing whatever "+
    "may linger behind it.%^RESET%^");
    tell_room(TO,"%^BOLD%^%^CYAN%^As the soft moonlight "+
    "cascades through the graveyard, a ghostly, glowing "+
    "building appears here amongst the ruins.%^RESET%^");
    TO->set_long(original_long + "%^BOLD%^%^WHITE%^  A "+
    "%^BOLD%^%^CYAN%^ghostly, glowing building%^BOLD%^%^WHITE%^ "+
    "rests undisturbed here amongst the ruins.%^RESET%^");
   for(x = 0;x < sizeof(TOMB_ITEMS);x++) {
     TO->add_item(TOMB_ITEMS[x],"%^BOLD%^%^CYAN%^This glowing "+
     "building is made from an unknown substance.  It doesn't "+
     "appear solid, yet seems to somehow mask whatever may "+
     "rest within it.  It continually gives off a bright, "+
     "ghostly glow.  An eerie feeling begins to eat at you "+
     "as you behold it.%^RESET%^");
    }
  }
