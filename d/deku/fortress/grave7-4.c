...
void create() {
  ...
  set_exits(([
   "east" : GY_ROOMS+"grave7-3",
   "north":GY_ROOMS +"grave8-4",
   "west" : GY_ROOMS +"grave7-5",
   "south" : GY_ROOMS +"grave6-4"
  ] ));
  do_tombs();
}
...
void do_tombs() {
 ...
 if((string)EVENTS_D->query_time_of_day() == "night" && member_array("tomb",query_exits()) == -1) {
   TO->add_exit(GY_ROOMS+"tomb3","tomb");
   set_door("glowing door",GY_ROOMS +"tomb3","tomb");	
