//based on the light gloves - a dark alternative
#include <std.h>
#include <daemons.h>
...
void create(){
    ...
    "gloves of %^BOLD%^%^BLACK%^Dar%^RESET%^k%^BOLD%^%^BLACK%^ness%^RESET%^");
    set_obvious_short("black leather gloves");

    set_lore("These gloves were crafted by a master tailor and "+
    "then imbued with magic by the wizard Ishnor, before being "+
    "altered by some unknown tinkerer. The magic is "+
    "said to darken the world in the wearer's eyes, "+
