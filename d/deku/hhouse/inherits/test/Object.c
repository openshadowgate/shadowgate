...
inherit "/d/deku/hhouse/inherits/test/trap_data";
inherit "/d/deku/hhouse/inherits/test/move";
...
void set_property(string prop, mixed value)
{
    ...
    if(prop == "enchantment"  && !random(20) && ! query("no curse"))
    {
        value = -1* value;
    }
    if(pointerp(value))
...
void create()
{
    ...
    studied = ({});
    studied_failed = ([]);
    log_file("deku_code", "@@START@@\n Object.c before reset call\n");
    CleanCheck = query_no_clean();
    set_no_clean(1);
    if(living(TO)) reset();
    set_no_clean(CleanCheck);
    log_file("deku_code", "@@END@@\n Object.c after reset call\n");
