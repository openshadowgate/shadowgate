...
inherit "/d/deku/hhouse/inherits/test/room";
...
void create()
{
    log_file("deku_code", "@@START@@\n vault.c before ::create\n");
    ::create();
    log_file("deku_code", "@@END@@\n vault.c after ::create\n");    
}

void reset()
{
    log_file("deku_code", "@@@START@RESET@@@\n vault.c before ::reset\n");
    ::reset();
    log_file("deku_code", "@@@END@RESET@@@\n vault.c after ::reset call\n");
    return;
}
...
int OpenDoor(string str) 
{
	...
	{
       	if(TO->execute_trap("open "+str, this_player())) return 1;
    	}
    	if(doors[str]["open fun"]) 
	{
      	if(!call_other(this_object(), doors[str]["open fun"])) return 1;
    	}
    	if(doors[str]["open string"]) write(doors[str]["open string"]);
...
int CloseDoor(string str) 
{
	...
	{
      	if(TO->execute_trap("close "+str, this_player())) return 1;
    	}
    if(doors[str]["close fun"]) {
        if(!call_other(this_object(), doors[str]["close fun"])) return 1;
    }
    if(doors[str]["close string"]) write(doors[str]["close string"]);
...
//    say(this_player()->query_cap_name()+" closes the "+str+".\n");
   if(doors[str]["close string"])
    ...
    else tell_room(ETP,TPQCN+" closes the "+str+".\n",TP);
...
***********/
    ...
    {
        if(TO->execute_trap("unlock "+ident+ " " +lock_id, this_player())) return 1;
    }
    ...
    */
    if(doors[ident]["unlock string"]) write(doors[ident]["unlock string"]);
    else write("You unlock the "+lock_id+" on "+ident+""+((key)?" with the "+key+"":"")+".\n");
    ...
    if(objectp(find_object_or_load(doors[ident]["destination"])))
    {
        ...
        if(userp(TP)) { doors[ident]["destination"]->set_had_players(); }
...
int PickLock(string str){
    ...
    {
        if(TO->execute_trap("pick "+ident +" " +lock_id, TP)) return 1;
    }
...
/*    if(!TP->is_class("thief") && !TP->is_class("assassin")) {
    ...
    modifier = query_mod(ident, lock_id);
    score = TP->query_skill("dungeoneering") + modifier;
    difficulty = 20;
    if(score >= difficulty) {
        if(doors[ident]["unlock fun"])
        if(!call_other(this_object(), doors[ident]["unlock fun"])) return 1;
        if(doors[ident]["unlock string"]) write(doors[ident]["unlock string"]);
        else write("You unlock the "+lock_id+" on the "+ident+" with your thieves' tools.\n");
...
string query_long_exits() 
{
    ...
    for(j = 0; j < sizeof(sorties); j++)
    {
        ...
        {
            if(query_invis_door(id)) continue;
...
string query_short_exits() {
    ...
    if(max == 1){
        ...
        if(id && !query_invis_door(id)){
            if(query_open(id)) return "[1: "+sx[0]+" (op)]";
            else return "[1: "+sx[0]+" (cl)]";
        }
        else return "[1: "+sx[0]+"]";
    }
    s = ("[" + max + ": ");
...
{
    if(sizeof(default_door_state)) { return default_door_state; }
    return 0;
}
...
void sync_outside_doors(mapping map)
{
    ...
    {        
        ...
        if(!objectp(other_room = find_object(door_map["destination"]))) { continue; }
