...
void shadow_me(object targ)
{
    ...
    set_short("%^BOLD%^%^BLACK%^Hulking shadow of "+targ->getWholeDescriptivePhrase() + "%^RESET%^");
    set_long("%^BOLD%^%^BLACK%^This being is the shadowly outline of "+
    targ->query_description()+"%^BOLD%^%^BLACK%^.%^RESET%^");
    set_overall_ac(targ->query_ac());
    set_hp(((int)targ->query_max_hp() * 2));
...
{
    ::create();
    set_name("hulking shadow");
    set_id(({"shadow", "Shadow", "hulking shadow"}) );
    set_short("%^BOLD%^%^BLACK%^hulking shadow%^RESET%^");
    set_race("undead");
...
void heart_beat()
{
    ...
    if(is_voided && random(2))
    {
        if((int)TO->query_hp() < (int)TO->query_max_hp())
        {
            ...
            TO->set_hp((int)TO->query_hp() + 20 + random(50));
        }
    }
    return;
}
...
void complete_void()
{
    ...
    for(x = 0; x < sizeof(vics);x++)
    {
        ...
        if(vics[x]->will_save(22))
        {
            ...
            vics[x]->cause_typed_damage(vics[x], vics[x]->return_target_limb(), roll_dice(2,10), "negative energy");
            continue;
        }
        else
        {
            ...
            vics[x]->cause_typed_damage(vics[x], vics[x]->return_target_limb(), roll_dice(4,10), "negative energy");
            continue;
        }
        continue;
    }
