...
items = 1;
   ...
   set("long",
      ...
      " this town and nothing has remained untouched by the destruction."
   );
...
void searching() {
   ...
   switch(random(23)){
      ...
      case 18..19:
         ...
         break;
      case 20..22:
         write("%^BOLD%^%^WHITE%^You search through the corpses and find a %^BOLD%^%^RED%^ring%^BOLD%^%^WHITE%^ on the finger of one body!%^RESET%^");
         tell_room(TO,"%^BOLD%^%^WHITE%^"+TPQCN+" finds a %^BOLD%^%^RED%^ring%^BOLD%^%^WHITE%^ on the finger of one body.%^RESET%^",TP);
      ob = new(OBJ+"zring")->move(TO);
         break;
      }
      items = 0;
      return;
...
void reset(){
   ::reset();
   if(base_name(TO) == "/d/deku/sanctuary/inherit/troad") return;
   if(!getmobs) return;
