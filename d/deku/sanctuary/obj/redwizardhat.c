...
void create(){
	...
	set_id(({ "hat", "wizard hat" }));
        set_short("%^BOLD%^%^RED%^A red suede wizard's hat%^RESET%^");
	set_long(
@AVATAR
%^BOLD%^%^RED%^This hat is modelled after the classical wizard's hat. The tall conical hat is made out of red suede. The body of the hat itself is rather soft, allowing it to drape in soft folds, though the point at the top still remains very stiff and straight. A wide brim helps to shield the wearer from the elements and bright lighting. A black velvet band with a decorative %^RESET%^%^ORANGE%^brass %^BOLD%^%^RED%^buckle is set in the center of the hat, where the crown meets the brim.
...
int wear_func(){
	...
	tell_room(environment(ETO),"",ETO);
        tell_object(ETO,"%^BOLD%^%^RED%^You place the red suede hat on your head, topping off your wizard's apparel.");
	return 1;
      }
...
int remove_func(){
	tell_room(environment(ETO),"",ETO);
        tell_object(ETO,"%^BOLD%^%^RED%^You remove the red suede hat, and feel less like a wizard.");
	return 1;
}