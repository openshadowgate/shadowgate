...
void create()
{
	::create();
 	set_id(({"antipaladin","human","anti"}));
  	...
  	set_long("%^RESET%^This man is tall and his body is completely decked with a suit of darkened fullplate armor.  His eyes, visible only because the visor of the helm is raised, are a dark and very cold blue.  The visible parts of his flesh are a slightly tanned brown, suggesting that he has saw at least some sunlight recently.  He carries a massive two handed sword, that is always kept clean, at his side.%^RESET%^");
  	set_race("human");
  	set_gender("male");
  	set_alignment(3);
