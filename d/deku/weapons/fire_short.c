...
void create(){
	::create();
	set_name("fire short sword");
	set_id(({ "short sword", "short", "sword", 	
	"fire sword", "sword of fire", "fire short sword" }));
	set_short("%^BOLD%^%^WHITE%^K%^YELLOW%^i%^BOLD%^%^WHITE%^"+
	"n%^YELLOW%^o%^BOLD%^%^WHITE%^rd%^YELLOW%^a%^BOLD%^%^WHITE%^'"+
	"s%^RESET%^ %^RED%^short sword of fl%^YELLOW%^a%^RESET%^"+
	"%^RED%^m%^YELLOW%^e%^RESET%^");
	set_obvious_short("A short sword of flame");
	set_long("%^RED%^This small sword barely manages to "+
   	 ...
   	 "entirely dies out occasionally.  A soft and barely "+
    	"audible %^CYAN%^hum%^RESET%^%^RED%^ is given off from the "+
    	"entire weapon.%^RESET%^");

	set_lore("These swords were said to be made by a "+
...
void heart_beat() {
    ...
    if(!perm && charges < 1) {
        tell_object(ETO,"%^RED%^The fire on your short sword burns out "+
        "and the humming ceases.%^RESET%^");
        tell_room(EETO,"%^RED%^The fire on "+ETOQCN+"%^RED%^'s "+
        "short sword burns out and the humming ceases.%^RESET%^",ETO);
        TO->set_weight(0);
        new("/d/common/obj/weapon/shortsword")->move(ETO);
        TO->remove();
        return;
    }
} 
...
int wield_func()
{
    	...
    	tell_object(ETO,"%^RED%^You slowly take hold of the short sword and "+
	"wince, but no pain comes, the fire feels strangely cool.%^RESET%^");
    
    	if(!ETO->query_invis()) 
	{
      	tell_room(EETO,ETOQCN+"%^RED%^ slowly takes hold of the "+
        	"short sword and winches slightly, but then looks relieved.%^RESET%^",ETO);
    	}
	return 1;
}
	...
	if(!random(4)){
        switch(random(18)) {
            case 0..9:
                ...
                "by the sword as you strike "+
                targ->QCN+"%^CYAN%^!%^RESET%^");
                
                tell_object(targ,ETOQCN+"%^CYAN%^'s sword hums "+
                "a sad tune as it strikes you!%^RESET%^");

                tell_room(EETO,ETOQCN+"%^CYAN%^'s sword hums "+
            ...
            case 17:
                ...
                "sword collides with "+targ->QCN+"%^YELLOW%^!"+
                "%^RESET%^");

                tell_object(targ,"%^YELLOW%^An explosion of "+
                "%^RED%^fire%^RESET%^%^YELLOW%^ from "+ETOQCN+
                "%^YELLOW%^'s sword slams into you, burning "+
                "you horribly!%^RESET%^");
                
                tell_room(EETO,"%^YELLOW%^An explosion of "+
                "%^RED%^fire%^RESET%^%^YELLOW%^ from "+ETOQCN+
                "%^YELLOW%^'s sword collides with "+targ->QCN+
