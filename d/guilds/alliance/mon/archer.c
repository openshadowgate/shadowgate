...
create() {  
  ::create();
  set_name("archer");
  set_id( ({"alliance archer","an archer", "archer" }) );
  set("race", "human");
  set_gender("male");
  set("short", "White Guard Archer");
  set_long(
@TSARVEN
...
int is_hunting_me(object agg) {
	...
	if((int)agg->in_guild("Alliance of Light")) return 0;
	if(present("alliance monster ring", agg) != -1) return 0;
	return 1;
}
