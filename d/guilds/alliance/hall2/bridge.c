...
void create(){
	...
	set_long(
@DESC
%^CYAN%^On the Drawbridge%^RESET%^
%^BOLD%^%^WHITE%^You find yourself walking on a massive drawbridge that leads
across the dark%^RESET%^ %^ORANGE%^moat%^RESET%^%^BOLD%^%^WHITE%^ that surrounds the castle. The drawbridge is 
made of strong oak reinforced with iron. On it you see a large 
symbol that looks like two gauntlets shaking hands.%^RESET%^
DESC
	);
...
int lock_trap(string str) {
	...
	if(!wizardp(TP)) TP->set_paralyzed(stun, "You are stunned.");
	HALL_D->incoming(TO);
	return 1;
}
