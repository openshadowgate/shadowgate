// level 1 thief trooper for the Fist
...
void create(){
	...
	set_class("thief");
	set_alignment(5);
	set_name("Great backstabber");
	set_id(({"trooper","backstabber", "thief","great backstabber","Great backstabber"}));
	set_short("Great backstabber");
	races = RACE_D->query_races();
	races = races -  SHIT;
	races = races - ({"firbolg","half-ogre","ogre","wemic"});
	set_race(races[random(sizeof(races))]);
	set_hd(random(7)+20,8);
	set_max_hp(random(150)+50);
	set_hp(query_max_hp());
...
for a fee, of course. He has sworn to guard the Iron Fist
members and their hall until the day he dies.
DESC
	);
