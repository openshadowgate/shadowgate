// level 2 thief trooper for the Fist
...
void create(){
	...
	set_class("thief");
	set_alignment(5);
	set_name("Master of shadows");
	set_id(({"thief", "trooper","master", "master of shadows","Master of shadows"}));
	set_short("Master of shadows");
	races = RACE_D->query_races();
	races = races -  SHIT;
	races = races - ({"firbolg","half-ogre","ogre","wemic"});
	set_race(races[random(sizeof(races))]);
	set_hd(random(7)+27,8);
