// level 3 mage trooper for the Fist
...
void create(){
 	...
 	set_class("mage");
  	set_alignment(5);
	set_name("Great enchantress");
	set_id(({"trooper","mage", "great enchantress","Great enchantress"}));
	set_short("Great Enchantress");
  	races = ({"elf","half-elf","human","ogre-mage","voadkyn"});
  	set_race(races[random(sizeof(races))]);
  	set_hd(random(10)+40,8);
  	set_max_hp(random(100)+180);
  	set_hp(query_max_hp());
