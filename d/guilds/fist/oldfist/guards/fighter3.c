// level 3 fighter trooper for the Fist
...
void create(){
 	...
 	set_class("fighter");
  	set_alignment(5);
	set_name("Great warrior");
	set_id(({"trooper","fighter", "warrior","great warrior","Great warrior"}));
	set_short("Great Warrior");
  	races = RACE_D->query_races();
 	races = races - SHIT;
  	set_race(races[random(sizeof(races))]);
  	set_hd(random(10)+40,8);
  	set_hp(random(300)+200);
