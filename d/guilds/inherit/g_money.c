...
int query_cash() {
    ...
    guild = guild_name;
    if(!guild) return notify_fail("Sorry you are in a wrong place.\n");
    if(member_array(guild,TP->query_guild()) < 0)
        return notify_fail("This is not your guild.\n");

    amount = GUILDSD->query_guild_account(guild);
    tell_player(TP, capitalize(guild_name)+" has "+amount+" gold in funds available.\n");
    return 1;
}
...
int deposit(string str) {
    ...
    TP->force_me("save");
    write("The bookie takes "+amount+" of your gold with a scowl.\n");
    say(TPQCN+" forks over some of his hard earned cash.\n");
    GUILDSD->add_guild_gold(guild_name, amount);
    return 1;
}
