...
timings, maze_names, north_locations, east_locations, south_locations, west_locations,
...
string * query_river(string path){
  string * ks;
  if (!mapp(saved_rivers) || sizeof(saved_rivers)<1) saved_rivers = ([]);
...
string * right_locations(string path){
  ...
  mapping locations;
  if (!mapp(saved_locations) || sizeof(saved_locations)<1) saved_locations = ([]);
...
string query_river_entry(string path, string name){
  ...
  if (!mapp(river)|| sizeof(river)<1) river = ([]);
  ks = keys(river);
  if (sizeof(ks)<1 ||member_array(name, ks)==-1){
    return "none";
  } else {
...
void add_location(string path, int location, string room){
  string * ks;
  if (!mapp(saved_locations) || sizeof(saved_locations)<1){
    saved_locations = ([]);
    ks = ({});
  } else {
    ks = keys(saved_locations);
...
int add_special_room(string path, string name){
  ...
  int loc, r, i;
  mapping rooms;
  report("%^BOLD%^%^GREEN%^Adding special room. Path: " + path + "  Name: " + name);
  if (!mapp(special_rooms)|| sizeof(special_rooms)<1) special_rooms = ([]);
...
mapping query_room_exits(string path, string room_name){
  ...
  if (!mapp(maze) || sizeof(maze)<1) maze = ([]);
  ks = keys(maze);
...
mapping query_maze(string path){
  string * ks;
  if (!mapp(saved_mazes) || sizeof(saved_mazes)<1) return ([]);
  ks = keys(saved_mazes);
  if (sizeof(ks)<1 || member_array(path, ks)==-1){
...
string query_room(string path, int location){
  string * ks;
  int * locs;
  if (!mapp(exits) || sizeof(exits)<1) exits = ([]);
  ks = keys(saved_locations);
...
int replace_room(string path, string original_room, string new_room){
  ...
  {
    report("%^BOLD%^%^YELLOW%^Those rooms are the SAME ROOM! It would be ridiculous to try to replace one with the other. Aborting." );
    return 0;
  }
  ...
  i = 1;
  foreach (dir in dirs){
    report("%^BOLD%^%^MAGENTA%^Adding exit " + i + " Path: " + path + " new_room: " + new_room + " dir: " + dir);
    add_exit(path, new_room, dir, 1);
    i ++;
    report("Exit added");
  }
  maze = saved_mazes[path];
...
int add_exit(string path, string room_name, string direction, int overwrite){
  string end_room, *ks, destination_room;
  int start_location, end_location;
//Add the location recording
  report ("%^BOLD%^%^YELLOW%^Adding exit. Path: " + path + " Room name: " + room_name + "Direction: " + direction);
  if (!mapp(saved_locations) || sizeof(saved_locations)<1)
  {
    report("saved_locations not a valid mapping");
    saved_locations = ([]);
  }
  ks = keys(saved_locations);
  if (member_array(path, ks)==-1){
  ...
  {
    report ("There are saved locations for path: " + path);
  }
  if (!intp(overwrite)){
  ...
  if (intp(start_location))
  {
    report("Start_location: " + start_location);
  } else
  {
    report("start_location is not a valid integer");
  }
  if (start_location<0){
...
//Add the exit to the start room
  if (!mapp(saved_mazes) || sizeof(saved_mazes)<1) saved_mazes = ([]);
  ks = keys(saved_mazes);
  if (sizeof(ks)<1 || member_array(path, ks)==-1){
    saved_mazes[path] = ([]);
  }
  maze_rooms = saved_mazes[path];
  if (!mapp(maze_rooms) || sizeof(maze_rooms)<1) maze_rooms = ([]);
  ks  = keys (maze_rooms);
  if (sizeof(ks)<1 || member_array(room_name, ks ) == -1){
...
void clear_maze(string path){
  ...
  if (!mapp(timings) || sizeof(timings)<1) timings= ([]);
  timings[path] =0;
  if (!mapp(north_locations) || sizeof(north_locations)<1) north_locations = ([]);
  if (!mapp(east_locations) || sizeof(east_locations)<1) east_locations = ([]);
  if (!mapp(south_locations) || sizeof(south_locations)<1) south_locations = ([]);
  if (!mapp(west_locations) || sizeof(west_locations)<1) west_locations = ([]);
  north_locations[path] = -1;
  east_locations[path] = -1;
  south_locations[path] = -1;
  west_locations[path] = -1;
  if (!mapp(maze_names) || sizeof(maze_names)<1) maze_names = ([]);
...
int build_maze(string path, string name, int size, int max_rooms){
  ...
  if (!mapp(timings)|| sizeof(timings)<1) timings = ([]);
  timings[path] = time();
  report ("%^BOLD%^%^YELLOW%^about to setup special rooms");
  call_out("setup_special_rooms", 1, path, name);
}
...
void setup_special_rooms(string path, string name){
  ...
  object room;
  int i, count;
  report("Setting up special rooms. Path: " + path + " Name: " + name);
  if(catch(room = find_object_or_load(path + name + "0")) )
  {
    report ("%^BOLD%^%^BLUE%^Error loading: " + path + name + "0");
  }
  if (!objectp(room)){
...
mapping get_rooms_at_y(string path, string name, int y){
  ...
  if (!mapp(locations) || sizeof(locations)<1) exits = ([]);
...
void show_map(object ob, string path){
  ...
  tell_object(ob, "About to show map: low_x " + low_x + " high_x " + high_x + " low_y " + low_y + " high_y " + high_y);
  for (i=high_y+1;i>=low_y-1;i--){
    rooms_at_y = get_rooms_at_y(path, name, i);
    if (!mapp(rooms_at_y) || sizeof(rooms_at_y)<1){
      tell_object(ob, "No rooms at line " + i);
      continue;
    }
...
int query_timing(string path){
  string * paths;
  if (!mapp(timings) || sizeof(timings)<1){
    return 0;
  }
  paths = keys(timings);
  if (member_array(path, paths)==-1) return 0;
  return timings[path];
}
...
int place_north_exit(string path, string room_name){
  ...
  report("%^BOLD%^%^MAGENTA%^Placing north exit: " + room_name);
...
int count_saved_mazes(){
  if (!mapp(saved_mazes)) return 0;
  return sizeof(saved_mazes);
}
