...
void create() {
   ...
   set_obvious_short("%^BOLD%^%^WHITE%^a fancy " + code1 + "embr" + code2 + "oi" + code1 + "der" + code2+ "e" + code1 + "d k" + code2+ "i" + code1 + "lt"); 
   switch (which){
   case WARRIOR:
   ...
   case CEREMONIAL:
     ...
     break;
   case TRACKER: 
     set_obvious_short(query_obvious_short() + "%^BOLD%^%^WHITE%^ with a %^BOLD%^%^GREEN%^ju%^RESET%^%^GREEN%^ngl%^RESET%^"
...
int strike_func(int damage, object what, object who)
{
   ...
   code2 = COLOUR_CODES[col2];
    ...
    if(random(3)<1)
    {
        tell_room(EETO, ETOQCN + "'s " + code1 + "kilt%^RESET%^ flickers, and " + ETO->QS + " becomes momentarily insubstantial, allowing " + who->QCN+" 's blow to pass right through " + ETO->QO, ({ETO, who}));
        
        tell_object(ETO, "Your " + code1 + "kilt%^RESET%^ goes %^BOLD%^%^WHITE%^c%^CYAN%^o%^WHITE%^ld%^RESET%^ and you feel a strange shock as " + who->QCN + "'s attack passes right through, leaving you feeling weird but otherwise unharmed.");
                                
        tell_object(who,  ETOQCN + "'s " + code1 + "kilt%^RESET%^ flickers, and " + ETO->QS + " becomes momentarily insubstantial, allowing your blow to pass right through " + ETO->QO);
        return 0;        
    }    
    return damage;
}
