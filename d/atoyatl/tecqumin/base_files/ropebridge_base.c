...
void create() {
  ...
  add_item(({"canyon", "jungle_canyon"}), "The %^ORANGE%^canyon%^RESET%^ is"
    ...
    +" the bottom. A path cut into the side winds its way down to the muddy"
    +" riverbank far below.");
}
...
// going backwards or forwards
  ...
  if (objectp(occupier) && occupier!=ob && !occupier->query_true_invis() 
    ...
    if ((string)occupier->query_race()=="ghost" || (string)ob->query_race() == "ghost"){
      if ((string)occupier->query_race()=="ghost") {
        tell_object(ob, "%^CYAN%^You pass through " + occupier->QCN +"'s"
          +" %^BOLD%^%^WHITE%^insubstantial %^RESET%^%^CYAN%^body");
        tell_room(TO, ob->QCN + " %^RESET%^%^CYAN%^passes through " 
          + occupier->QCN + "'s %^BOLD%^%^WHITE%^insubstantial%^RESET%^"
          +" %^CYAN%^body");
      } else {
        tell_object(ob, "%^CYAN%^You pass %^BOLD%^%^WHITE%^insubtantially"
          +"%^RESET%^%^CYAN%^ through " + occupier->QCN + "'s %^CYAN%^body");
        tell_room(TO, ob->QCN + " %^RESET%^%^CYAN%^passes%^BOLD%^%^WHITE%^"
          +" insubstantially%^RESET%^%^CYAN%^ through " + occupier->QCN 
          + "'s %^RESET%^%^CYAN%^body");
      }
    } else {
      if (fly1 == fly2 && fly1 != 1){
