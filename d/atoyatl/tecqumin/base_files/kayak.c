...
int capsized;     // just a flag, to remember if the kayak has capsized
...
string long_desc(){
  ...
  int i;
  if (capsized ==1){
    tell_object(TP, "%^BOLD%^%^CYAN%^The kayak has been capsized! You"
...
void capsize(){
  ...
  bank_num ++;
  river = find_object_or_load(ROOMS + "river" + bank_num + ".c");
  if (!objectp(river)){
    tell_room(TO, "Error with the river you were falling into ('" + "river" + bank_num + ".c" + "'). Please tell a Wiz, and/or bug this room.");    
    return;
  }
...
void hit_obstruction(string obstruction){
  tell_room(TO, "The kayak hits  " + obstruction + ". You%^BOLD%^"
...
void arrive(){
  tell_room(TO, "%^ORANGE%^With a loud CRUNCH, the kayak grounds"
...
void waterfall(){
  tell_room(TO, "%^BLUE%^The kayak is caught by a strong current and"
...
void waterfall2(){
  tell_room(TO, "%^BLUE%^The kayak accelerates forward in the centre of"
...
void waterfall3(){
  tell_room(TO, "%^BLUE%^The %^ORANGE%^RUMBLING %^BLUE%^sound is near"
               +" deafening now, as the kayak is pulled directly toward"
...
void waterfall4(){
  tell_room(TO, "%^ORANGE%^With a terrifying inevitability, the kayak"
...
void waterfall10(){
  ...
  if (num>0){
    for (i=0;i<num;i++){
      critters[i]->move(JUNG_ROOM + "shore.c");
    }
  }
}
  ...
  if (relative_direction ==12 ||relative_direction ==4){
    if (river_speed>=3){
      tell_room(TO, "%^BOLD%^%^CYAN%^The swift flowing water roars over"
         +" the kayak as it turns side on to the stream. You capsize!");
      capsize();
      return;
    }
  }
...
// Send a message about the direction the boat is facing on the river
  ...
  if (delay<1){
    tell_room(TO, "%^BOLD%^%^RED%^You lose control of the kayak as it hurtles down the river."
...
void launch(object ob){
  ...
  if (query_exit("out")==ROOMS + "river"){
    tell_object(ob, "The kayak is already launched, isn't it? If not, please make a bug report"); 
    return;
  }
