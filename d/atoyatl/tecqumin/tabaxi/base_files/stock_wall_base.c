...
void create() {
  ...
  set_items( ([ ({"fence", "wall", "stockade wall", "wooden fence"}) : "The %^ORANGE%^stockade fence"
...
string long_desc(){
  string desc;
  desc = "%^GREEN%^You are standing in the %^RESET%^gap %^GREEN%^between a"
        +" %^ORANGE%^stockade wall %^GREEN%^to the south and the"
        +" %^BOLD%^%^GREEN%^j%^RESET%^%^GREEN%^un%^BOLD%^%^GREEN%^gl"
        +"%^RESET%^%^GREEN%^e to the north."
        +" ";
  return desc;
}
