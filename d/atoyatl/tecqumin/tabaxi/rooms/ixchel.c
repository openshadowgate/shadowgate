...
void create() {
    ...
    set("aggressive", 0);
    set_level(49);
    set_long("This bright-eyed young tabaxi woman has soft brown fur, which is stained in %^RED%^r%^ORANGE%^a%^BOLD%^%^YELLOW%^i%^RESET%^%^GREEN%^n%^CYAN%^b%^BLUE%^o%^MAGENTA%^w c%^BLUE%^o%^CYAN%^l%^GREEN%^o%^YELLOW%^u%^RESET%^%^ORANGE%^r%^RED%^s from repeated exposure to ground herbs and powders, from her hands to her elbows. Her face is more deliberately decorated with a %^RED%^ba%^ORANGE%^n%^BOLD%^%^YELLOW%^d %^RESET%^%^GREEN%^o%^CYAN%^f %^BLUE%^ra%^MAGENTA%^in%^BLUE%^bo%^CYAN%^w %^RESET%^%^GREEN%^s%^BOLD%^%^YELLOW%^t%^RESET%^%^ORANGE%^ip%^RED%^es%^RESET%^ across her cheekbones");
    set_gender("female");
   set_alignment(2);
   set_race("tabaxi");
    set_hd(49,6);
