...
string true_long_desc(){
  string result;
  result = long_desc();
  result += " A fast %^BOLD%^%^BLUE%^r%^CYAN%^i%^BLUE%^ver %^RESET%^%^GREEN%^charges past here. It breaks out of the thick jungle with almost no warning, and disappears into more thick f%^BOLD%^o%^RESET%^%^GREEN%^li%^ORANGE%^a%^GREEN%^ge just as quickly. The bank on this side is relatively low and not too steep, meaning it probably gets used as a watering hole.";
  return result;
}
