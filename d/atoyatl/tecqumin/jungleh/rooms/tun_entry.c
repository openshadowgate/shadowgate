...
/*
int check_maze(){
  ::check_maze("/realms/lujke/tecqumin/jungleh/rooms/");
}
*/
...
void on_enter(){
  ...
  string *exits;
  report ("%^BOLD%^%^YELLOW%^Someone entered tun_entry");
  if(last_entered > time() - 50){
    ::on_enter();
    return;
  }
  if (catch(  mazeroom1 = find_object_or_load(JUNG_ROOM8 + "j_maze0.c") )){
    report("Error loading mazeroom1: " + JUNG_ROOM8 + "j_maze0.c");
  }
  set_basic_exits();
...
void place_exit(){
  if (last_entered > time()-50) return;
