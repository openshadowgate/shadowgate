...
void create() {
  ...
  add_item( ({"hut", "huts", "stone hut", "stone hunts", "ruin", "ruins"}), 
   ...
   +" are a number of them to your west. You think there might be one that is"
   +" still intact, but you will have to get closer to get a better look.");
  ...
  add_item( ({"break", "break in the jungle", "break in the jungle foliage"}), 
    ...
    +" west, and continuing to the southwest, the" 
    +" %^GREEN%^j%^BOLD%^%^GREEN%^u%^RESET%^%^GREEN%^ng%^ORANGE%^l%^GREEN%^e"
    +" f%^BOLD%^%^GREEN%^o%^RESET%^%^GREEN%^l%^BOLD%^%^GREEN%^i%^RESET%^%^GREEN%^a"
  ...
  set_exits(([
      "northeast" : CITY_ROOM + "city_jung09",
      "southwest" : CITY_ROOM + "canalbank04", 
      "west" : CITY_ROOM + "hut01"
   ]));
}
