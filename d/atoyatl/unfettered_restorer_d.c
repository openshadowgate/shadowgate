...
void checkunf(){
  object room;
  report("%^BOLD%^%^GREEN%^Checking on %^RESET%^%^MAGENTA%^Unfettered");
  ...
  if (COUNT >2) report ("%^BOLD%^%^GREEN%^Count has exceeded 2. Giving up on the poor %^RESET%^%^MAGENTA%^Unfettererd");
}
...
void restore(){
  ...
  unfettered->restore(unfettered, relay_room, clones, tentacle_hps, empress_moves, spirits_absorbed, countdown, invincible, last_summoned, targeters, seized, exits_used, victory, destinations_visited, path_trod, roomname, mucus, frozen_mucus, skin, tentacles_withdrawn, relaying, power_absorbed, last_move, absorbing, reporting, BEAT, heartcount);
  unfettered->move(roomname);
}
