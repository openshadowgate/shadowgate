...
void create(){
	::create();
	set_name("party table");
	set_id(({ "table", "party table", "tables", "party tables", " stool", "desk", "stools" }));
	set_short("%^RESET%^%^ORANGE%^The %^BOLD%^%^MAGENTA%^P%^CYAN%^a%^WHITE%^r%^BLUE%^t%^RED%^y %^RESET%^%^ORANGE%^Table%^RESET%^");
	set_obvious_short("%^RESET%^%^ORANGE%^The %^BOLD%^%^MAGENTA%^P%^CYAN%^a%^WHITE%^r%^BLUE%^t%^RED%^y %^RESET%^%^ORANGE%^Table%^RESET%^");
	set_long(
@AVATAR
