...
int wear_func()
{
    ...
    ETO->set_property("added short", ({ " %^BOLD%^%^WHITE%^(bathed in light)%^RESET%^" }));
    //ETO->set_property("magic resistance",10); removing this as it's a purely visual item now
    return 1;
}
...
int remove_func()
{
    ...
    ETO->remove_property("added short", ({ "%^WHITE%^ (bathed in light)%^RESET%^" }));
    //ETO->set_property("magic resistance",-10); //removing this as it's a purely visual item now
    return 1;
}

/*int strike_func(int damage, object what, object who){ removing struck function as it's a purely visual item now
    if(random(1000) < 1000){
