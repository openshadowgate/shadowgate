...
void create(){
	::create();
	set_name("Auppenser's Dagger");
	set_id(({ "dagger", "dirk" }));
	set_short("%^BOLD%^%^BLACK%^Dagger of %^RESET%^%^CYAN%^Enlightenment%^RESET%^");
	set_obvious_short("%^BOLD%^%^BLACK%^Obsidian Dagger%^RESET%^");
	set_long(
@AVATAR
%^BOLD%^%^BLACK%^The blade of this dagger is made from obsidian and is pitch black with a glassy sheen as a result.  The 4" grip is made of %^YELLOW%^bronze%^BLACK%^ wrapped in black leather. The pommel has a %^YELLOW%^bronze setting%^BLACK%^ in which an %^RESET%^%^CYAN%^eye%^BOLD%^%^BLACK%^ made from %^RESET%^diamonds%^BOLD%^%^BLACK%^ for the %^RESET%^white%^BOLD%^%^BLACK%^ of the eye, %^RESET%^%^CYAN%^ light blue topaz%^BOLD%^%^BLACK%^ for the iris, and %^CYAN%^aquamarine%^BLACK%^ for the pupil is set.  The 7" blade is exceptionally sharp and a deep %^RED%^blood groove%^BLACK%^ runs down the center of the blade almost to its point.  This weapon is %^RESET%^finely crafted%^BLUE%^, beautiful, %^BOLD%^%^BLACK%^and %^RED%^dangerous looking.%^RESET%^
