...
void create(){
	...
	set_id(({ "hammer", "soul hammer", "hammer of souls" }));
	set_short("%^BOLD%^%^BLACK%^Hammer of Souls%^RESET%^");
	set_obvious_short("%^BOLD%^%^BLACK%^blackened hammer%^RESET%^");
	set_long(
@AVATAR
%^RED%^This hammer is made from a metal that has been %^BOLD%^%^BLACK%^charred%^RESET%^%^RED%^ by an intense %^YELLOW%^h%^RED%^e%^YELLOW%^a%^RED%^t.%^RESET%^%^RED%^  Along the shaft of the hammer run several mysterious %^BLUE%^runes%^RED%^.  In the center of the %^BLUE%^runes%^RED%^ is an %^BLUE%^insignia%^RED%^ which depicts a human kneeling before an %^BOLD%^%^BLACK%^obscurred%^RESET%^%^RED%^ figure.  There is a very small %^BOLD%^%^WHITE%^inscription%^RESET%^%^RED%^ in the upper corner of the %^BLUE%^insignia%^RESET%^%^RED%^.  The head of the hammer is in the shape of an arched rectangle and has a small hole in the side of it, as if something was once %^YELLOW%^inset%^RESET%^%^RED%^ into it's side.%^RESET%^ 
