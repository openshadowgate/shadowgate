...
void init() {
    ::init();
    if(interactive(ETO) && !owner) owner = ETOQN;
}
...
int extra_wield() {
    ...
    tell_room(EETO,"%^BOLD%^%^BLACK%^Ripples spread along the length of "+
       ...
       "%^GREEN%^rg%^BOLD%^y %^BLACK%^b%^GREEN%^u%^BOLD%^%^BLACK%^rsts "+
       ...
       switch(random(80)){
          ...
          case 66..71:  tell_room(EETO,"%^RED%^Spinning quickly "+
                        ...
                        tell_object(ETO,"%^BOLD%^%^BLACK%^The shadow "+
                           "rises up before you!");
                        obj = new("/d/dagger/marsh/tower/mon/shadow");
	                  obj->move(EETO);
          ...
          case 72..76:  tell_room(EETO,"%^BLUE%^"+ETOQCN+" spins, "+
                        ...
                        ETO->set_property("magic",-1);
                        break;
          case 77..79:  if(!(int)vic->query_stats("constitution") > random(28)){
