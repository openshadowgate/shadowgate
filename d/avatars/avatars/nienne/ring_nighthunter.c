...
void create(){
   ...
   set_id(({"ring","carved ring","obsidian ring","carved obsidian ring","ring of the night hunter","Ring of the Night Hunter"}));
   set_short("%^BOLD%^%^BLACK%^Ri%^RESET%^%^MAGENTA%^n%^BOLD%^%^BLACK%^g of the N%^RESET%^i%^BOLD%^%^BLACK%^ght Hun%^RESET%^t%^BOLD%^%^BLACK%^er%^RESET%^");
...
"In the centre is a widened face, upon which is carved the delicate image of a %^RESET%^%^CYAN%^night hunter%^BOLD%^%^WHITE%^. A distinctive "
...
"The particular image upon this ring is of a night hunter, a creature equally admired and feared by the drow, both for its sharp "
"perception of its surroundings, and its ferocity in combat.\n");
...
int wear_fun() {
   if((string)ETO->query_name() != "bannic" && !avatarp(ETO)) {
     tell_object(ETO,"The ring doesn't seem to fit you.");
     return 0;
   }
   tell_object(ETO,"%^CYAN%^As the ring slides onto your finger, the knowledge touches your mind that you can <haste> yourself and your allies.%^RESET%^");
   return 1;
}
...
void init(){
   ::init();
   add_action("haste_fun","haste");
}

int haste_fun(string str) {
   if(!objectp(TO)) return 0;
   if(!objectp(ETO)) return 0;
   if(str) {
     notify_fail("You only need <haste> yourself and your allies.\n");
     return 0;
   }
   if(!TO->query_worn()) {
   ...
   new("/cmds/spells/h/_haste")->use_spell(ETO,0,25,100,"mage"); 
   return 1;
}
