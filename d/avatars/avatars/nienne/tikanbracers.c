...
"of the bracers being a gift of the Queen of the Air to a faithful follower, imbued with the breath of the "
"winds itself.  Rumours tell that the bracers can guide the very hands of the wearer, uplifting them to "
"strike with the fury and intensity of a whirlwind.%^RESET%^\n");
...
int check() {
   ::check();
   if((string)ETO->query_name() == "wererat") return 1;
   if((string)ETO->query_name() != "tikan" && !avatarp(ETO)) {
     tell_object(ETO,"A gust of wind pushes your hands aside, and stops you from wearing the bracers!");
     return 0;
   }
