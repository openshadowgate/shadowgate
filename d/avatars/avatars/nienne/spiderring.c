...
"particularly their priestesses. They summon nearby spiders to the "
"wearer, and compel them to protect their master. Such rings are usually "
"only weak in power though, and can be used only once each day.\n");
   set_property("lore difficulty",6);
   set_read("~ let the %^YELLOW%^spiders %^RESET%^guard you ~\n");
   set_language("drow");
...
int summon_me(string str) {
   ...
   j = random(4)+1;
   if(j == 1) tell_room(EETO,"%^CYAN%^A little spider scurries forth!%^RESET%^");
   else tell_room(EETO,"%^CYAN%^Little spiders scurry forth!%^RESET%^");
   for(i = 0; i < j; i++) {
     ob=new("/d/avatars/nienne/spider.c");
     ob->move(EETO);
