      ...
      switch(random(20)){
         case 0..4:
		  ...
		  return roll_dice(2,8)+3;
              break;
         case 5:
            tell_room(environment(ETO),"%^RESET%^%^ORANGE%^"+ETO->query_cap_name()+" strikes "+target->query_cap_name()+" with earthshaking force that knocks "+target->query_cap_objective()+" off of "+target->query_possessive()+" feet!%^RESET%^",({target,ETO}));
