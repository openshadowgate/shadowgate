...
"%^RESET%^\n");
   set_lore("%^BOLD%^In the year 323SG, the %^RESET%^%^GREEN%^elven priestess Alamerielle D'Allyan'ya"
...
void free(){
   ...
   if(!(string)ETO->query_deity()=="akadi"){
	tell_object(TP,"%^BOLD%^%^CYAN%^You do not follow Akadi child, this cape is of no use to you.%^RESET%^");
   return 1; 
   }
   if(!TO->query_worn()){ 
