...
void create()
{
  ...
  set_short("%^BOLD%^%^CYAN%^Imbued %^BLUE%^St%^WHITE%^a%^BLUE%^rlight Sw%^WHITE%^o%^BLUE%^rd%^RESET%^");
  set_id(({"sword","starlight sword","Starlight Sword"}));
  set_long(
	   "This sword is almost enchanting, crafted as it is from some opaque alloy that"+
	   ...
	   " and has a %^BOLD%^%^BLUE%^star sapphire%^RESET%^ set into the pommel. Mysteriously,"
         " a length of %^GREEN%^vine %^RESET%^seems to grow along the hilt, wrapping around the crossguard."
	   );
	set_lore("Seen as a holy blade in the church of Mystra, the Starlight "+
		...
		"has served the followers of The Lady of Mysteries well.  This "
            "particular sword has been further enchanted by some sort of "
            "druidic magic of unknown purpose.");
