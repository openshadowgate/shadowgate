...
void create(){
    ...
    set_long("This is a carved cube made of animal bone.  There are six sides, each with dots carved into it, they number one through six.");
...
int check(string str) {
	...
	if(str == "die") {
		if(loaded == 1) {
			write("You check your die and determine it will always land with %^CYAN%^" +number+ "%^RESET%^ up.");
			return 1;
		}
		if(loaded == 0) {
			write("You check your die and realize you can't tell which number will land up.");
			return 1;
		}
	}
	else {
...
int clear(string str) {
	...
	if(str == "die") {
		if(loaded == 1) {
			write("You fiddle with your die and even it out, letting fate once again determine the number on which it will land.");
   if(!ETO->query_invis()) {
   tell_room(EETO,ETO->QCN+" rolls "+ETO->QP+" die idly between his fingers.",ETO);
   }
			loaded = 0;
			return 1;
		}
  ...
  people -= ({ETO});
		if(!ETO->query_invis()) {
			for(x = 0;x < sizeof(people);x++) {
				if(random(41) + 1 <= people[x]->query_stats("wisdom")) {
     tell_object(people[x],"You notice " + ETO->query_cap_name() + " suspiciously touching " + ETO->query_possessive() + " die!%^RESET%^");
     continue;
    }
   }
  }
