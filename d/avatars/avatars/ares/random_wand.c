...
string CLASS, THE_SPELL;
mapping SPELL_MAP;


int use_wand(string str);
int calculate_charges(int level);
varargs void hard_setup(int max_level, string class, string spell, int charges);
varargs void setup_wand(int max_level, string class, string spell, int charges);
...
int use_wand(string str)
{
    ...
    if (CLASS == "mage")
    {
        if (ETO->is_class("mage") || ETO->is_class("sorcerer")) { roll = 100; } // auto-pass if they're the same class as the wand's spell
    }
    else
    {
        if (ETO->is_class(CLASS)) { roll = 100; } // auto-pass if they're the same class as the wand's spell
    }
    ...
    if (sizeof(args) == 1)
    {
        new(file)->use_spell(ETO, ETO, CASTER_LEVEL, 100, CLASS);
    ...
    if (argument == "me") { new(file)->use_spell(ETO, ETO, CASTER_LEVEL, 100, CLASS); }
    else { new(file)->use_spell(ETO, argument, CASTER_LEVEL, 100, CLASS); }
    if (CHARGES != -1) { CHARGES--; }
    calculate_value();
    return 1;
}
...
varargs void hard_setup(int max_level, string class, string spell, int charges)
{
    ...
    setup_wand(max_level, class, spell, charges);
    return;
}
...
varargs void setup_wand(int max_level, string class, string spell, int charges)
{
    ...
    if (!stringp(class) || class == "" || class == " " || (member_array(class, CASTING_CLASSES) == -1)) { class = CASTING_CLASSES[random(sizeof(CASTING_CLASSES))]; }
    ...
    if (!stringp(spell) || spell == "" || spell == " ")
    {
        ...
        SPELL_MAP = MAGIC_D->query_index(CLASS);

        if (!mapp(SPELL_MAP) || !sizeof(keys(SPELL_MAP)))
        {
            throw("ERROR, no spell exists for class " + CLASS);
            return;
        }
        ...
        if (!sizeof(spells))
        {
            throw("ERROR, no level " + max_level + " spells exist for " + class + ".");
            return;
        }
    }
