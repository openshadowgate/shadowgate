...
void create(){
	::create();
	set_name("nightfall sword");
	set_id(({ "sword", "longsword", "long sword", "nightfall's sword" }));
	set_short("%^BOLD%^%^BLACK%^Nightfall's Sword%^RESET%^");
	set_obvious_short("%^BOLD%^%^BLACK%^Nightfall's Sword%^RESET%^");
	set_long(
@AVATAR
