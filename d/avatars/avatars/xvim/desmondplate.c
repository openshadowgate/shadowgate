...
int wear_func(){
	tell_room(ETP,"%^WHITE%^%^BOLD%^"+ETO->query_cap_name()+" wears his buckles up his fullplate",TP);
	tell_object(TP,"%^WHITE%^%^BOLD%^You buckle up the heavy plate, and feel reinsured by its protection");
	return 1;
}
int remove_func(){
        tell_room(environment(ETO),"%^WHITE%^%^BOLD%^"+ETO->query_cap_name()+" removes his plate and lets out a sob",ETO);
        tell_object(ETO,"%^WHITE%^%^BOLD%^You remove the plate, and your body longs after its protection");
	return 1;
}
