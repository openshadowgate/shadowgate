...
void summon_me(string str){
   ...
   charges--;
   set_my_long();
   tell_object(TP,"%^BOLD%^%^WHITE%^You pull a pair of "
"%^GREEN%^ge%^MAGENTA%^ms%^GREEN%^t%^MAGENTA%^on%^GREEN%^es %^WHITE%^from "
"the base of the scepter, letting them fall to the ground.  At a touch "
"from the head of the scepter, they crack open to reveal a swarm of "
"hungry %^BLACK%^locusts%^WHITE%^!%^RESET%^");
   tell_room(EETO,"%^BOLD%^%^WHITE%^"+ETOQCN+" pulls a pair of "
"%^GREEN%^ge%^MAGENTA%^ms%^GREEN%^t%^MAGENTA%^on%^GREEN%^es %^WHITE%^from "
"the base of "+ETO->QP+" scepter, letting them fall to the ground.  At a "
"touch from the head of the scepter, they crack open to reveal a swarm of "
"hungry %^BLACK%^locusts%^WHITE%^!%^RESET%^",ETO);
   for(i = 0; i< j; i++) {
     ob=new("/d/avatars/lurue/locust");
     ob->move(EETO);
