...
#define INTERIOR "/d/avatars/lujke/rooms/tent_interior"
...
int empty_me(string str){
  object * critters, * stuff, interior;
  int i;
  if (!str || !id(str)){
    notify_fail("You could 'empty tent'");
    return 0;
  }
  interior = find_object_or_load(INTERIOR);
  if (!objectp(interior)){
    tell_object(TP, "There is a problem with the interior of the tent. Please make a bug report");
    return 1;
  }
  critters = all_living(interior);
  if (sizeof(critters)>0){
    tell_object(TP, "You can't empty the tent, there's someone inside");
    return 1;
  }
  stuff = all_inventory(interior);
  if (sizeof(stuff)<1){
    tell_object(TP, "The tent is already empty");
    return 1;
  }
  for (i=0;i<sizeof(stuff);i++){
    tell_object(TP, "You dump "+ stuff[i]->query_short() + " out of the tent and onto the ground");
    tell_room(ETP, TPQCN + "%^RESET%^ dumps " + stuff[i]->query_short() + " out of the tent and onto"
                    +" the ground", TP);
    stuff[i]->move(ETP);
  }
  return 1;
