...
//    for (i=0;i<num;i++){
 ...
 //   }
  num = sizeof(children("d/avatars/lujke/nic_hunter.c"));
  if (num>0){
    for (i=0;i<num;i++){
      children("/d/avatars/lujke/nic_hunter.c")[0]->remove();
    }
  }
//  }
  set_hidden(1);
...
//hide."); 
        ...
        if (sizeof(children("/d/avatars/lujke/nic_hunter.c"))>1){
 //       tell_object(wearer, "There are already too many bounty hunters"); 

          for (i=sizeof(children("/d/avatars/lujke/nic_hunter.c"))-1
                                           ;i>=0;i--){
            creature = present(children("/d/avatars/lujke/nic_hunter.c")[i], EETO);
            if (!objectp(creature)){
              children("/d/avatars/lujke/nic_hunter.c")[i]->off_duty();
//        tell_object(wearer, "sending one off duty"); 

            }
          }
        }
        if (sizeof(children("/d/avatars/lujke/nic_hunter.c"))<2){
  //      tell_object(wearer, "creating a bounty hunter to eat your hide."); 

          creature = new("/d/avatars/lujke/nic_hunter.c");
          if (!objectp(creature)){
    //  tell_object(wearer, "couldn't make one"); 
      ...
      if (sizeof(children("/d/avatars/lujke/nic_archer.c"))<5){
        groups = random(2)+1;
        for (j=0;j<groups;j++){
          num = random (3)+1;
          for(i=0;i<num;i++){
            creature = new("/d/avatars/lujke/nic_archer.c");
