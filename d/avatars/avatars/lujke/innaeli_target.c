...
int wear_func(){
  int i, num;
  tell_object(ETO, "You realise that you are really not wanted in Shadow OR Azha");
...
int remove_func(){
    tell_object(ETO, "You are reminded that going to Shadow or Azha might be a bad"
                    +" idea");
    set_hidden(0);
    return 1;
}
...
void heart_beat(){
  ...
  if (file_name(place)[0..strlen(SHADOW)-1] == SHADOW
     ...
     file_name(place)[0..strlen(AZHA)-1] == AZHA ){
