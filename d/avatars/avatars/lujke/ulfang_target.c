...
#define HUNTER "/d/avatars/lujke/ulfang_hunter.c"
#define ARCHER "/d/avatars/lujke/ulfang_archer.c"
#define MYTARG "Ulfang"
#include <std.h>
...
//    for (i=0;i<num;i++){
 ...
 //   }
  num = sizeof(children(HUNTER));
  if (num>0){
    for (i=0;i<num;i++){
      children(HUNTER)[0]->remove();
    }
  }
//  }
  set_hidden(1);
...
//    tell_object (wearer,"You are in Tabor!");
 ...
 //     tell_object(wearer, "You are indoors!");
      if ((string)wearer->QCN != MYTARG){ 
        tell_object(wearer, "This is not your feeling");
      }

      if (!random(10)){
//        tell_object(wearer, "seeing if we need a bounty hunter to eat your hide."); 

        if (sizeof(children(HUNTER))>1){
  //      tell_object(wearer, "There are already too many bounty hunters"); 

          for (i=sizeof(children(HUNTER))-1
                                           ;i>=0;i--){
            creature = present(children(HUNTER)[i], EETO);
            if (!objectp(creature)){
              children(HUNTER)[i]->off_duty();
//        tell_object(wearer, "sending one off duty"); 

            }
          }
        }
        if (sizeof(children(HUNTER))<2){
...
void place_archer(object * creatures){
  ...
  if (objectp(temproom)){
    tell_room(EETO, "%^BLUE%^Some movement to the "+direction + " catches"
                   +" your attention as a some %^ORANGE%^Tabor city archers"
