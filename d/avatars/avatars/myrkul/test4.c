...
void create(){
	...
	set_id(({ "sword", "long sword", "longsword", "skull cleaver", "skullcleaver" }));
	set_short("%^BOLD%^%^RED%^S%^CYAN%^k%^RED%^ull%^YELLOW%^C%^RED%^lea%^BLACK%^v%^RED%^er Longsword%^RESET%^");
...
int wield_func(){
	tell_room(environment(ETO),"As "+ETOQCN+" grips his "+query_short()+", he gets an evil %^RED%^glint%^RESET%^ in his eye!",ETO);
	tell_object(ETO,"As you grip "+query_short()+" you feel violent %^RED%^urges%^RESET%^ for revenge!");
	return 1;
}

int unwield_func(){
	tell_room(environment(ETO),"As "+ETOQCN+" release "+query_short()+", a look of piece crosses "+ETOQCN+"'s eyes.",ETO);
