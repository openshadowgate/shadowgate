      ...
      switch(random(6)){
 	   ...
 	   case 1..2:
         ...
         case 4:
            ...
            break;   
         case 5:
            tell_object(ETO,"%^BOLD%^%^BLACK%^Akra'Mashuls voice cackles with glee in your mind as it and strikes out at "+target->query_cap_name()+"!%^RESET%^");
...
void heart_beat(){
   object *livings,target,temp,obj,ob,sm;
   int i,j;
...
" at "+ETO->query_cap_name()+"'s side!",ETO);
   ...
   j = sizeof(livings); 
   for(i=0;i<j;i++){
      if((string)livings[i]->query_gender()=="male"){
         switch(random(4)){
            case 0:
               tell_object(livings[i],"%^BOLD%^%^BLACK%^The sword calls to you.. yes, just hold it, yes!  We can be great together!%^RESET%^");
            break;
            case 1:
               tell_object(livings[i],"%^BOLD%^%^BLACK%^Visions of you standing over a mountain of fallen enemies enter your mind.. At your side is the sword, Akra'Mashul!%^RESET%^");
           break;
           case 2:
               tell_object(livings[i],"%^BOLD%^%^BLACK%^The voice of Akra'Mashul enters your mind '%^RED%^Take me from this weak fool.  He is not worthy, only you and I can be great together.%^RESET%^");
           break;
           case 3:
               tell_object(livings[i],"%^BOLD%^%^BLACK%^The voice of Akra'Mashul enters your mind '%^RED%^Please, hold me.  Just hold me.. Take me now, for you know you are more powerful!%^RESET%^");
           break;
         }
      }
