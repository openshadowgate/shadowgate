...
"  Hidden within the folds of the garment are several strategically placed pockets.%^RESET%^\n");
 set_lore("%^BOLD%^%^BLACK%^It is rumored that these robes are worn only by those of the most secret of societies and may only be obtained by those that complete the tests of intelligence, subtlety, stealth, and strength, as deemed fit by the highest ranking members.  After successfully mastering the tasks given, the robes are presented as a gift to the newest inductee.%^RESET%^");
set_weight(4);
set_value(35000);
set_ac(0);
set("intbonus",1);
...
void makecolor(string str){
  ...
  TO->set_obvious_short("%^BOLD%^%^BLACK%^Long, dark flowing robes%^RESET%^");
