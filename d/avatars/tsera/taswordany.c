...
if((int)ETO->query_highest_level()>20&&(int)ETO->query_highest_level()<25&&(firstwield==1)){
...
if((int)ETO->query_highest_level()>=35&&(firstwield==1)){
            tell_object(ETO,"%^RESET%^%^MAGENTA%^The sword purrs happily as you take her into your hands.");
...
" at "+ETO->query_cap_name()+"'s side!",ETO);
   ...
   j = sizeof(livings); 
   for(i=0;i<j;i++){
      if((string)livings[i]->query_gender()=="male"&&!livings[i]==ETO){
         switch(random(400)){
            case 0..396:
            break;
            case 397:
