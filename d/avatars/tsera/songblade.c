...
int wield_func(){
   ...
   return 1;
      ...
      tell_room(environment(ETO),"%^BOLD%^%^WHITE%^The %^MAGENTA%^runes %^WHITE%^of "+ETO->query_cap_name()+"'s sword begin to %^RESET%^%^MAGENTA%^gl%^BOLD%^o%^RESET%^%^MAGENTA%^w s%^BOLD%^o%^RESET%^%^MAGENTA%^ftly %^BOLD%^%^WHITE%^as "+ETO->query_subjective()+" graps the magnificent hilt.%^RESET%^",ETO);
