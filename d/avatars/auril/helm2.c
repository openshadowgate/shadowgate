...
int strike_func(int damage, object what, object who){
	if(random(1000) < 50){
	tell_room(environment(query_worn()),"%^BOLD%^%^RED%^"+who->QCN+" is struck by a horrible wail emitting from "+ETOQCN+"'s helm!",({ETO,who}));
	tell_object(ETO,"%^BOLD%^%^RED%^A loud wail issues forth from your helm striking "+who->QCN+"!");
	tell_object(who,"%^BOLD%^%^RED%^You are struck with a horrible wail coming from "+ETOQCN+"'s helm!");
