...
void create(){
	...
	set_obvious_short("%^BOLD%^%^WHITE%^a %^MAGENTA%^m%^CYAN%^ys%^BLUE%^ti%^GREEN%^ca%^RESET%^%^YELLOW%^l %^WHITE%^silver ring%^RESET%^");
	set_short("%^BOLD%^%^MAGENTA%^Ma%^CYAN%^sq%^BLUE%^ue%^GREEN%^ra%^YELLOW%^de %^RESET%^%^ORANGE%^Ri%^BOLD%^%^RED%^ng%^RESET%^");
