...
void create(){
	...
	set_short("a steel bastardsword");
	set_obvious_short("a steel bastardsword");
	set_long(
@AVATAR
%^RESET%^%^CYAN%^The materials used to make this bastardsword are of the highest quality steel.  Made by a master weaponsmith, the bastardsword is crafted to be a fierce weapon in the hands of its wielder during combat.  Stamped with the insignia of a %^RESET%^steel war guantlet %^CYAN%^with a watching %^BLUE%^blue eye%^RESET%^%^CYAN%^, the holy symbol of Helm, this weapon was clearly made for those who serve and protect Helm's faith.
