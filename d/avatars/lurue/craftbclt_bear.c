...
void create(){
	...
	set_id(({ "bracelet", "woven bracelet", "multicolored bracelet", "bear bracelet" }));
	set_short("%^RESET%^%^CYAN%^Craft Bracelet of the %^RED%^Bear%^RESET%^");
	set_obvious_short("%^RESET%^%^CYAN%^a %^BLUE%^w%^GREEN%^o%^YELLOW%^v%^RESET%^%^GREEN%^e%^BLUE%^n %^CYAN%^bracelet%^RESET%^");
	set_long(
@AVATAR
%^RESET%^%^CYAN%^This simple hand braided bracelet appears to be little more then a fanciful piece of country craft.  %^BLUE%^Blue%^CYAN%^, %^GREEN%^green %^CYAN%^and %^YELLOW%^yellow %^RESET%^%^CYAN%^threads have been tightly knotted together into a zig-zagging weave that alternates back and forth with perfect regularity.  Dotting its surface, in bright %^RED%^red thread%^CYAN%^, images of bears can be seen marching across the multicolored strands.  At either end of the bracelet, %^BOLD%^%^BLACK%^metal clasps %^RESET%^%^CYAN%^provide a means to both prevent fraying and to allow the bracelet to be closed around the wrist.  Though simple of design, it is clear that someone spent a great deal of time working on making this bracelet just right.%^RESET%^ 
