...
void create(){
	...
	set_short("a silver warhammer");
	set_obvious_short("a silver warhammer");
	set_long(
@AVATAR
%^RESET%^%^CYAN%^The materials used to make this warhammer are of the highest quality silver.  Made by a master weaponsmith, the warhammer is crafted to be a fierce weapon in the hands of its wielder during combat.  Stamped with the insignia of a %^BOLD%^%^WHITE%^white capped %^RESET%^%^CYAN%^cresting %^BLUE%^wave%^RESET%^%^CYAN%^, the holy symbol of Istishia, this weapon was clearly made for those who serve and protect Istishia's faith.
