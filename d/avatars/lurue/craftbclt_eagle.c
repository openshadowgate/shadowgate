...
void create(){
	...
	set_id(({ "bracelet", "woven bracelet", "multicolored bracelet", "eagle bracelet" }));
	set_short("%^CYAN%^Craft Bracelet of the %^BOLD%^%^MAGENTA%^Eagle%^RESET%^");
	set_obvious_short("%^RESET%^%^CYAN%^a %^BOLD%^w%^BLUE%^o%^WHITE%^v%^BLUE%^e%^CYAN%^n %^RESET%^%^CYAN%^bracelet");
	set_long(
@AVATAR
%^RESET%^%^CYAN%^This simple hand braided bracelet appears to be little more then a fanciful piece of country craft.  %^BOLD%^Cyan%^RESET%^%^CYAN%^, %^BOLD%^%^BLUE%^azure %^RESET%^%^CYAN%^ and %^BOLD%^%^WHITE%^white %^RESET%^%^CYAN%^threads have been tightly knotted together into a zig-zagging weave that alternates back and forth with perfect regularity.  Dotting its surface, in bright %^BOLD%^%^MAGENTA%^magenta thread%^RESET%^%^CYAN%^, images of eagles can be seen sailing across the multicolored strands.  At either end of the bracelet, %^BOLD%^%^BLACK%^metal clasps %^RESET%^%^CYAN%^provide a means to both prevent fraying and to allow the bracelet to be closed around the wrist.  Though simple of design, it is clear that someone spent a great deal of time working on making this bracelet just right.%^RESET%^
