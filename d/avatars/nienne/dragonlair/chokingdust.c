...
void init() {
   ...
   if(living(environment(TO)) && TP==environment(TO) ) {
      TP->set_static("spell interrupt","%^CYAN%^You're trying to get your breath back!%^RESET%^");
      TP->set_property("spell interrupt","%^CYAN%^You're trying to get your breath back!%^RESET%^");
