...
"%^WHITE%^h%^BOLD%^%^BLACK%^adow %^RESET%^%^BLUE%^spring forth over the top of "+ETO->QP+" hands!%^RESET%^",ETO);
   claw1 = new("/d/avatars/nienne/shadowclaws.c");
   claw1->move(ETO);
   while((int)claw1->query_property("enchantment") < 1) {
     claw1->remove_property("enchantment");
     claw1->set_property("enchantment",(int)TO->query_property("enchantment"));
   }
   ...
   claw2 = new("/d/avatars/nienne/shadowclaws.c");
   claw2->move(ETO);
   while((int)claw2->query_property("enchantment") < 1) {
     claw2->remove_property("enchantment");
     claw2->set_property("enchantment",(int)TO->query_property("enchantment"));
   }
   if (TO->query_identified()) {
     names = TO->query_identified();
     for (i=0;i<sizeof(names);i++) {
