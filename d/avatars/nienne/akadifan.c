...
int hit_func(object targ){
   ...
   if(!random(12)) {
     ETO->execute_attack();
      tell_object(ETO,"%^BOLD%^%^CYAN%^A gentle breeze picks up around you, guiding your hands to strike rapidly!%^RESET%^");
      tell_object(targ,"%^BOLD%^%^CYAN%^A gentle breeze picks up around "+ETO->QCN+" as "+ETO->QS+" slashes at "
"you with surprising speed!%^RESET%^");
      tell_room(EETO,"%^BOLD%^%^BLUE%^ A gentle breeze picks up around "+ETO->QCN+" as "+ETO->QS+" slashes at "
+targ->QCN+" with surprising speed!%^RESET%^",({ETO,targ}));
