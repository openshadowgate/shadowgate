...
void create(){
	...
	set_short("%^BOLD%^%^WHITE%^a %^MAGENTA%^m%^CYAN%^ys%^BLUE%^ti%^GREEN%^ca%^RESET%^%^YELLOW%^l %^WHITE%^silver bag%^RESET%^");
	set_obvious_short("%^BOLD%^%^MAGENTA%^Ma%^CYAN%^sq%^BLUE%^ue%^GREEN%^ra%^YELLOW%^de %^RESET%^%^ORANGE%^Ba%^BOLD%^%^RED%^g%^RESET%^");
...
void heart_beat() {
   ...
   if(strsrch(locstring,"/d/avatars/nienne/party/") == -1) {
