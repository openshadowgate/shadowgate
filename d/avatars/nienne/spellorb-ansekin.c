...
void create() {
    ::create();
    set_name("orb");
    set_id(({"orb","glass orb","glossy glass orb","glossy orb","sphere of elemental wrath","Sphere of Elemental Wrath"}));
    set_element("fire");
...
int wield_fun(){
    ...
    if((string)ETO->query_name() != "ansekin") {
...
"ly within its glassy prison.%^RESET%^");
    if(TP->is_class("mage") || TP->is_class("psion")) {
      set_item_bonus("intelligence",1);
      set_item_bonus("charisma",0);
      set_item_bonus("wisdom",0);
      return 1;
    }
    if(TP->is_class("sorcerer") || TP->is_class("bard")) {
