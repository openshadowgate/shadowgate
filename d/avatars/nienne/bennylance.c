...
int wield_func(){
   if(!ETO->is_class("cavalier") && !ETO->is_class("paladin") && !ETO->is_class("antipaladin") && !avatarp(ETO)) {
     tell_object(ETO,"%^BOLD%^%^WHITE%^The lance fails to respond to your touch.%^RESET%^");
     return 0;
   }
   if((OWNED && (string)ETO->query_name() == owner) || !OWNED || avatarp(ETO)) {
     if(interactive(ETO) && !OWNED && !avatarp(ETO)) {
       ...
       OWNED = 1;
       tell_object(ETO,"%^BOLD%^%^WHITE%^A strange feeling of acceptance runs through you.%^RESET%^");
     }
