...
create() {
    ::create();
     ...
     set_obvious_short("a blackened dagger");
     ...
     set_lore("These unusual daggers are said to be forged "+
        ...
        "assassins, much like the pernicious souls that now guard "+
        "the castle.  The daggers seem to hold a mystical power, "+
...
int extra_wield() {
    ...
    if((ETO->query_level() < 25) && ((string)ETO->query_name() != "Assassin")){
