...
void create(){
    ...
    "appears to be an almost %^BOLD%^%^WHITE%^ghostly%^CYAN%^ "+
    ...
    "glowing %^BOLD%^%^GREEN%^green light%^CYAN%^ that is "+
...
int wield_func(){
	tell_room(EETO,"%^CYAN%^The %^YELLOW%^light%^CYAN%^ "+
    "emitted by "+ETOQCN+"%^CYAN%^"+
    "'s sword suddenly %^BOLD%^%^BLACK%^dims!%^RESET%^",ETO);
	
    tell_object(ETO,"%^CYAN%^The %^YELLOW%^light%^CYAN%^ "+
    "emitted by your sword suddenly %^BOLD%^%^BLACK%^dims!%^RESET%^");
	return 1;
}
int unwield_func(){
	tell_room(EETO,"%^CYAN%^The %^YELLOW%^light%^CYAN%^ emitted by "+
    ETOQCN+"%^CYAN%^'s sword suddenly %^BOLD%^%^WHITE%^"+
    "intensifies!%^RESET%^",ETO);
	tell_object(ETO,"%^CYAN%^The %^YELLOW%^light%^CYAN%^ emitted by "+
	...
	if(!random(10)){
        switch(random(3)) {
            case 0:
                ...
                targ->do_damage(targ->return_target_limb(),roll_dice(1,2));
                ETO->set_property("magic",-1);
                return 0;
                break;
            case 1:
