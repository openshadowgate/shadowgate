...
void create (){
  ...
  set_property("magic",1);
   set_short("%^BOLD%^%^BLUE%^Antioch guardsman%^RESET%^");
  set_property("bounty hunter",1);
  add_damage_bonus(7);
  set_long(
   "A disciplined looking guard in the livery of %^BOLD%^%^YELLOW%^A%^BLUE%^ntioch"
  +" %^YELLOW%^c%^BLUE%^ity.%^RESET%^  His stern"
  +"visage gives him the look of a man dedicated to his work and the protection"
  +"of the city above all else.  His armor and shield are brightly polished,"
  +"and amongst his equipment he carries a sharply honed longsword with which"
  +"he looks quite competent."
  );
set_property("swarm",1);
set_gender("male");
  set_race("human");
...
void off_duty(){
  ...
  if (objectp(place)){
    tell_room(place, "%^BLUE%Shadow %^ORANGE%^bounty hunter%^BLUE%^ goes off in search of profit");
  }
