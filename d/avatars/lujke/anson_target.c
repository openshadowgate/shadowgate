...
int wear_func(){
  ...
  num = sizeof(children("d/avatars/lujke/anson_hunter.c"));
  if (num>0){
    for (i=0;i<num;i++){
      children("/d/avatars/lujke/anson_hunter.c")[0]->remove();
    }
  }
  set_hidden(1);
  return 1;
...
void heart_beat(){
  ...
  if (file_name(place)[0..strlen(SHADOW)-1] == SHADOW
    ...
    if ((int)place->query_property("indoors")==1){
      ...
      if (!random(10)){
        if (sizeof(children("/d/avatars/lujke/anson_hunter.c"))>1){

          for (i=sizeof(children("/d/avatars/lujke/anson_hunter.c"))-1
                                           ;i>=0;i--){
            creature = present(children("/d/avatars/lujke/anson_hunter.c")[i], EETO);
            if (!objectp(creature)){
              children("/d/avatars/lujke/anson_hunter.c")[i]->off_duty();


            }
          }
        }
        if (sizeof(children("/d/avatars/lujke/anson_hunter.c"))<2){

          creature = new("/d/avatars/lujke/anson_hunter.c");
          if (!objectp(creature)){
      ...
      if (sizeof(children("/d/avatars/lujke/anson_archer.c"))<5){
        groups = random(2)+1;
        for (j=0;j<groups;j++){
          num = random (3)+1;
          for(i=0;i<num;i++){
            creature = new("/d/avatars/lujke/anson_archer.c");
