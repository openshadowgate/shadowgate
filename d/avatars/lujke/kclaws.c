...
void create(){
    ::create();
    set_name("claws");
    set_id(({"claws","scaled claws","black claws","blackscale claws","kyjahs claws","kyjah's claws"}));
    set_short("%^BOLD%^%^BLACK%^K%^BLUE%^y%^BLACK%^jah%^BLUE%^'%^BLACK%^s Bl%^BLUE%^a%^BLACK%^cksc%^BLUE%^a%^BLACK%^le Cl%^BLUE%^a%^BLACK%^ws%^RESET%^");
    set_obvious_short("%^BOLD%^%^BLACK%^sc%^BLUE%^a%^BLACK%^le ed%^BLUE%^g%^BLACK%^ed bl%^BLUE%^a%^BLACK%^ck cl%^BLUE%^a%^BLACK%^ws%^RESET%^");
    set_long("%^BOLD%^%^BLUE%^Gloves made of very thin leather form the basis "
