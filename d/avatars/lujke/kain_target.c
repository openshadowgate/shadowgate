...
#define SHADOW2 "/d/shadow/room/main"
...
//  num = sizeof(children("d/avatars/lujke/kain_hunter.c"));
//  if (num>0){
//    for (i=0;i<num;i++){
 ...
 //   }
  ...
  if (num>0){
    for (i=0;i<num;i++){
      children("/d/avatars/lujke/kain_hunter.c")[0]->remove();
    }
  }
//  }
  set_hidden(1);
...
void heart_beat(){
  ...
  if (file_name(place)[0..strlen(SHADOW)-1] == SHADOW || 
         file_name(place)[0..strlen(SHADOW2)-1] == SHADOW2 ){
//    tell_object (wearer,"You are in Shadow!");
    if ((int)place->query_property("indoors")==1){
//     tell_object(wearer, "You are indoors!");

      if (!random(10)){
 //   tell_object(wearer, "seeing if we need a bounty hunter to eat your hide."); 

        if (sizeof(children("/d/avatars/lujke/kain_hunter.c"))>1){
  //      tell_object(wearer, "There are already too many bounty hunters"); 

          for (i=sizeof(children("/d/avatars/lujke/kain_hunter.c"))-1
                                           ;i>=0;i--){
            creature = present(children("/d/avatars/lujke/kain_hunter.c")[i], EETO);
            if (!objectp(creature)){
              children("/d/avatars/lujke/kain_hunter.c")[i]->off_duty();
 //       tell_object(wearer, "sending one off duty"); 

            }
          }
        }
        if (sizeof(children("/d/avatars/lujke/kain_hunter.c"))<2){
//      tell_object(wearer, "creating a bounty hunter to eat your hide."); 

          creature = new("/d/avatars/lujke/kain_hunter.c");
          if (!objectp(creature)){
  //    tell_object(wearer, "couldn't make one"); 
      ...
      if (sizeof(children("/d/avatars/lujke/kain_archer.c"))<5){
        groups = random(2)+1;
        for (j=0;j<groups;j++){
          num = random (3)+1;
          for(i=0;i<num;i++){
            creature = new("/d/avatars/lujke/kain_archer.c");
...
//  tell_object (wearer,"You are %^BOLD%^%^RED%^NOT%^RESET%^ in Shadow!");
  }
...
((string)temproom->query_exit(direction))[0..strlen(SHADOW2)-1] != SHADOW2)
    {
      break;
    }
  ...
  if (objectp(temproom)){
    tell_room(EETO, "%^BLUE%^Some movement to the "+direction + " catches"
                   +" your attention as some %^ORANGE%^Shadow archers"
