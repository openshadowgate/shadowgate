...
#define DAGGERDALE "/d/dagger/Daggerdale/"
#include <std.h>
...
create() {
  ::create();
  set_name("feeling of being unwelcome in Daggerdale");   
  ...
  set_long("%^BLUE%^As you reflect inwards into your emotions, you are"
          ...
          +" city of Daggerdale, and that they might shoot arrows at you if"
          +" you go there.");
  set_type("ring"); 
...
int wear_func(){
  int i, num;
  tell_object(ETO, "You realise that you are really not wanted in"
                  +" Daggerdale");
...
int remove_func(){
    tell_object(ETO, "You are reminded that going to Daggerdale might be a"
                     +" bad idea");
    set_hidden(0);
    return 1;
}
...
void heart_beat(){
  ...
  if (file_name(place)[0..strlen(DAGGERDALE)-1] == DAGGERDALE){
//    tell_object (wearer,"You are in Tabor!");
    if ((int)place->query_property("indoors")==1){
 //     tell_object(wearer, "You are indoors!");
//      tell_object(wearer, "beat...");
      if (!random(10)){
 //       tell_object(wearer, "seeing if we need a bounty hunter to eat your 
//hide."); 

        if (sizeof(children("/d/avatars/lujke/cathy_hunter.c"))>2){
 //       tell_object(wearer, "There are already too many bounty hunters"); 

          for (i=sizeof(children("/d/avatars/lujke/cathy_hunter.c"))-1
                                           ;i>=0;i--){
            creature = present(children("/d/avatars/lujke/cathy_hunter.c")[i], EETO);
            if (!objectp(creature)){
              children("/d/avatars/lujke/cathy_hunter.c")[i]->off_duty();
//        tell_object(wearer, "sending one off duty"); 
  ...
  //      tell_object(wearer, "creating a bounty hunter to eat your hide."); 
          ...
          creature = new("/d/avatars/lujke/cathy_hunter.c");
          if (!objectp(creature)){
    //  tell_object(wearer, "couldn't make one"); 
         ...
         tell_room(place, "%^BLUE%^A vicious %^RED%^bounty hunter%^BLUE%^"
          ...
          creature->powder_em(wearer);
        }
      }
    }else{
      if (sizeof(children("/d/avatars/lujke/cathy_archer.c"))<5){
        groups = random(2)+1;
        for (j=0;j<groups;j++){
          num = random (3)+1;
          for(i=0;i<num;i++){
            creature = new("/d/avatars/lujke/cathy_archer.c");
            creature->set_target((string)ETO->query_name());
            creatures += ({creature});
          }
...
void place_archer(object * creatures){
  ...
  for (i=0;i<distance;i++){
    if (((string)temproom->query_exit(direction))[0..strlen(DAGGERDALE)-1] != DAGGERDALE){
      break;
    }
  ...
  if (objectp(temproom)){
    tell_room(EETO, "%^BLUE%^Some movement to the "+direction + " catches"
                   +" your attention as a some %^ORANGE%^Daggerdale archers"
