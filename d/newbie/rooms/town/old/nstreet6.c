...
"%^CYAN%^traffic %^RESET%^at most hours of the day.  To the northeast lies an odd %^MAGENTA%^dome%^RESET%^, obviously "
"some kind of building, but it is difficult to tell from here what purpose it serves.\n");
   set("night long", ""+TO->query_long()+"\n%^BOLD%^The street light glows brightly to light your way.");
   set_listen("default","It is unusually quiet here.");
...
"odd domed building.%^RESET%^",
     "dome":"%^MAGENTA%^Just off the side of the road lies a squat, domed building.  Its odd appearance is apparent even "
...
"before it seems to shimmer, much like a haze of heat.%^RESET%^"
   ...
   set_exits(([
      "northwest":MAINTOWN"nstreet5",
      "south":MAINTOWN"nstreet7",
      "northeast":MAINTOWN"psifoyer",
   ]));
...
int GoThroughDoor(){
   ...
   if(TP->is_class("psion")) {
      tell_object(TP, "As you walk through the palisade, you feel a "+
         "slight prickling sensation pass over your skin.");
      tell_room(ETP, "The air within the archway shimmers briefly "+
         "as "+TPQCN+" passes through it and out of sight.", TP);
        return 1;
   }
   tell_object(TP,"As you step through the archway, your head aches "+
      "as an unseen force seems to let you know you don't belong.\n");
   tell_room(TO,TPQCN+" winces and blinks in pain before proceeding inside.",TP);
   return 1;
}