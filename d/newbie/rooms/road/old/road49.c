//road49c Road from Offestry west gate to ruins
...
NEWBIE
    ...
    set_exits(([
       "southwest":ROAD"road48",
       "east":ROAD"road50",
       "northwest":"/d/newbie/rooms/keep/path1",
   ]));
}

void reset() {
	::reset();
   if( !present("bat") && !present("snake") && (query_night() == 1) && (random(10) < 3) ) {
	    switch(random(3)) {
			case 0:       
	...
	if( !present("snake") && !present("bat") && (query_night() == 0) && (random(10) < 3) ) {
	    switch(random(3)) {
			case 0:       
				new("/d/newbie/mon/rat")->move(TO);
				break;
