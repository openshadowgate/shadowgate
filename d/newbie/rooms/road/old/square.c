...
"citizens, businesses and adventure of this small city.",
      ...
      "down":MAINTOWN"boards1",
        ...
        ob->set_max_posts(50);
        ob->set_location("/d/newbie/rooms/town/square");
      ob->set_ooc_board(1);
        find_object_or_load("/d/npc/sarlen");
        "daemon/place_d"->place_in("/d/newbie/rooms/town/",new("/d/npc/sarlen"));
        find_object_or_load(MON"nerussa");
        "daemon/place_d"->place_in("/d/newbie/rooms/forest/",new(MON"nerussa"));
        find_object_or_load(MON"musond");
        "daemon/place_d"->place_in("/d/newbie/rooms/road/",new(MON"musond"));
        find_object_or_load(MON"doran");
        switch(random(3)) {
           case 0:
             "daemon/place_d"->place_in("/d/newbie/rooms/forest/",new(MON"doran"));
             break;
           case 1:
             "daemon/place_d"->place_in("/d/newbie/rooms/hill/",new(MON"doran"));
             break;
           case 2:  
             "daemon/place_d"->place_in("/d/newbie/rooms/road/",new(MON"doran"));
             break;
        }
        find_object_or_load(MON"trainer");  // this one has place_d in it's file
}
