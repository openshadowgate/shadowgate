...
"tied at the waist, and carries with him some sort of %^ORANGE%^old tome%^CYAN%^.%^RESET%^");
   ...
   set_emotes(1,({
     ...
     "Sirith rubs his hands together, looking down the trail leading west.",
...
//a repeat command, just in case someone forgets the instructions
   ...
   if((strsrch(msg,"help") != -1 || strsrch(msg,"yes") != -1 || 
    ...
    strsrch(msg,"okay") != -1)&& step1 && !step2)        {
      ...
      force_me("say As the town's story goes, a hero of theirs was buried in a tomb just west of here, after he died in an orc raid.");
   ...
   strsrch(msg,"okay") != -1) && step2 == 1) {
      ...
      force_me("emote peers down the road leading to the west.");
...
void receive_given_item(object obj){
   ...
   if(member_array("myringz",(string)obj->query_id()) != -1) {
      ...
      obj->remove();
      if(objectp(obj = present("newbiemassob", TP))) 
      {
