...
void create()
{
   ...
   grid_x = 20;
   grid_y = 20;
   MAP_GRID = ([ "9-5": (["file" : MAINTOWN"nstreet3", "symbol" : " # ", "color" : "%^BOLD%^%^YELLOW%^"]),
...
void build_grid(int x, int y)
{
    ...
    {
        which = myRooms[z];
        MY_GRID += ([ which : (["file" : MAP_GRID[which]["file"], "symbol" : MAP_GRID[which]["symbol"], "color" : MAP_GRID[which]["color"]]), ]);
        continue;
    }
    for(z = 1;z < x;z++)
    {
...
int read_func(string str)
{
    ...
    if(!mapp(MY_GRID)) build_grid(grid_x, grid_y);
    tell_object(TP, "\n%^BOLD%^%^GREEN%^       N\n\n   W - | - E\n\n       S%^RESET%^\n");
    tmp = ({});
    for(y = 1;y < grid_y;y++)
    {
        ...
        for(x = 1;x < grid_x;x++)
        {
            ...
            if(objectp(ETO))
            {
                if(objectp(EETO))
                {
                    ...
                    {
                        tmp2 += "%^BOLD%^%^RED%^"+MY_GRID[which]["symbol"] +"%^RESET%^";
                        loc = "%^RESET%^%^ORANGE%^"+MY_GRID[which]["symbol"] +"%^RESET%^";
                        continue;
                    }
                }
            }
