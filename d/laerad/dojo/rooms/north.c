...
void create() {
    ...
    set_name("The northern chamber of a wooden dojo hall");
    set_short("%^RESET%^%^RED%^The northern chamber of a wooden dojo hall%^RESET%^");
...
"lig%^WHITE%^h%^YELLOW%^t %^RESET%^%^CYAN%^into the room.  At the southern end of the room lies a single solidly closed "
"%^ORANGE%^door%^CYAN%^.  The rest of the room is bare except for two decorations, one of which is a long tapestry upon "
"the southern wall that depicts an %^GREEN%^op%^BOLD%^e%^RESET%^%^GREEN%^n f%^YELLOW%^i%^RESET%^%^GREEN%^eld%^CYAN%^, "
"covered in thick grass and dotted with flowers.  The other sits directly before it upon the floor; a squat pot holding "
"a spreading %^GREEN%^bonsai %^CYAN%^shrub, its trunk and soil coated in a fine layer of moss.%^RESET%^\n");
...
"southern wall, while most of the north is covered by a single long tapestry.%^RESET%^",
...
"illumination rather than a harsh glare.%^RESET%^",
      ({"door","doorframe"}) : "%^ORANGE%^To the south lies a sturdy wooden door, set within a frame as dark as the walls "
...
"pushed.  You feel a sudden desire to touch the door.%^RESET%^",
      ({"tapestry"}) : "%^GREEN%^A single tapestry takes up most of the northern wall.  It is simple in design, depicting "
"a broad field covered in thick grass and dotted with flowers.%^RESET%^",
      ({"bonsai","shrub","bonsai shrub"}) : "%^GREEN%^Sitting before the tapestry is a pot bearing a squat bonsai shrub, "
"its branches spreading wide to either side.  A fine layer of soft moss covers its trunk and the soil from which it "
"grows.%^RESET%^",
    ]));
}
...
void init() {
   ::init();
   if(!present("bhumi")) find_object_or_load(MON"bhumi")->move(TO);
...
int touch_fun(string str) {
   ...
   tell_object(TP,"%^GREEN%^You reach out to touch the door, and you suddenly find yourself in the main chamber"
"!%^RESET%^");
   tell_room(TO,"%^GREEN%^"+TP->QCN+" reaches out to touch the door, and suddenly vanishes!%^RESET%^",TP);
