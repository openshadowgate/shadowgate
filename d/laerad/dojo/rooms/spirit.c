...
void create() {
    ...
    set_name("The upper chamber of a wooden dojo hall");
    set_short("%^BOLD%^%^WHITE%^The upper chamber of a wooden dojo hall%^RESET%^");
    set_long("%^CYAN%^This chamber is almost as austere as those in the rest of the building. %^ORANGE%^Poli%^RED%^sh"
"%^ORANGE%^ed wooden floorboards %^CYAN%^run the length and breadth of the room, bordered upon all sides with "
...
"%^ORANGE%^door%^CYAN%^.  The rest of the room is bare except for one decoration, a long tapestry upon the northern wall "
"that depicts a %^BOLD%^%^BLACK%^jet-black %^RESET%^%^CYAN%^skyfield of night, scattered with tiny white %^BOLD%^%^WHITE%^s"
"%^RESET%^t%^BOLD%^%^WHITE%^a%^RESET%^r%^BOLD%^%^WHITE%^s%^RESET%^%^CYAN%^.%^RESET%^\n");
...
"pushed.  You feel a sudden desire to touch the door.%^RESET%^",
      ({"tapestry"}) : "%^ORANGE%^A single tapestry takes up most of the northern wall.  It is simple in design, depicting "
"a jet-black skyfield of night, scattered with tiny white stars.%^RESET%^",
    ]));
}

void init() {
   ::init();
   if(!present("akasha")) find_object_or_load(MON"akasha")->move(TO);
...
int touch_fun(string str) {
   ...
   tell_object(TP,"%^ORANGE%^You reach out to touch the door, and you suddenly find yourself back on the landing"
"!%^RESET%^");
