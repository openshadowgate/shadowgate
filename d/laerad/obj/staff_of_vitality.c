...
void create()
{
   ::create();
    ...
    set_lore("The Staff of Vitality has been mentioned in many holy texts of every "
        ...
        "of legends have surfaced that when in time of needs a benevolent diety "
        ...
        "nearly every faith. Though some clerics of Loviatar seem to hold a deep "
        "dislike of this staff, as they feel it encroaches upon the power of their "
        "Goddess.");
...
int hitme(object targ)
{
    ...
    if(random(1000) > 700 && is_straight_cleric(ETO))
    {
        ...
        targ->do_damage(targ->return_limb(), (roll_dice(1,4)+1));
        targ->do_damage(targ->return_limb(), (roll_dice(1,4)+1));
        targ->do_damage(targ->return_limb(), (roll_dice(1,4)+1));
        TO->set_property("magic", -1);
        return 1;
...
void cure(string str) 
{
    ...
    if(!max_uses) { max_uses = roll_dice(3,10) + 10; }
    ...
    if(uses > max_uses) return notify_fail("The staff is out of charges.\n");
    ...
    if(sscanf(str,"poison on %s",who)) 
    {
        ...
        tell_object(TP,"%^BOLD%^%^WHITE%^You shake the staff around "+ob->QCN+""
            ...
            "%^RESET%^%^CYAN%^blue-green%^BOLD%^%^WHITE%^ ring.");
        tell_object(ob,"%^BOLD%^%^WHITE%^"+TP->QCN+" shakes "+TP->QP+" staff "
            "around you and the %^RESET%^%^GREEN%^blue-green%^BOLD%^%^WHITE%^ "
            "ring beings to glow!");
        tell_room(ETP,"%^BOLD%^%^WHITE%^"+TP->QCN+" shakes "+TP->QP+" yew wood "
            "staff around "+ob->QCN+", moving in a circle.  "
            "The %^RESET%^%^GREEN%^blue-green%^BOLD%^%^WHITE%^ ring begins "
            "to glow!",({TP,ob}) );
