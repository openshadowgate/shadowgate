// Coded by Bane
// changed by *Styx* to use CROOM for mob control & mixing things up all across the wasteland and plains 12/14/03

#include <std.h>

#define MOB "/d/laerad/mon/"
#define EXTRAS ({ MOB+"tlin1", MOB+"tlin2", MOB+"tlin3", MOB+"tlin4", MOB+"tlin5", MOB+"cildabrin", MOB+"tshala", MOB+"flailer", MOB+"gearth" })
...
void set_mobs() {
  ...
  switch(random(120)) {
    ...
    case 3..4:	  set_monsters( ({MOB+"edrake"}), ({1}) );
			break;
    case 5..8:	  set_monsters( ({MOB+"edrake", EXTRAS[random(num)] }), ({1, 1}) );
			break;
    case 9..14:	  set_monsters( ({MOB+"gearth"}), ({1}) );
			break;
