...
create() {
    ::create();
    set_id(({"blade","flametongue","sword","longsword","long sword","flamberge longsword","flame"}));
    set_name("flaming long sword");
    set_obvious_short("%^RESET%^%^RED%^A flamberge longsword%^RESET%^");
    set_short("%^RESET%^%^RED%^F%^BOLD%^l%^RESET%^%^RED%^a%^ORANGE%^m%^BOLD%^"+
      "%^RED%^e%^RESET%^%^RED%^t%^ORANGE%^o%^BOLD%^n%^RESET%^%^RED%^g%^BOLD%^"+
      "u%^RESET%^%^RED%^e%^RESET%^");
    set_long("%^RED%^Forged from fever iron, this longsword has a "+
    ...
    set_lore("Forged with the heat of the fire and inscribed with ancient"+
            " runes of power, Flametongue was once a fiersome weapon used to "+
