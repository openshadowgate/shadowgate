...
void read(){
    ...
    write("%^GREEN%^-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=%^RESET%^");
    write("\n<ride> to Parnelli Forest: 10 Gold\n");
    write("%^GREEN%^-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=%^RESET%^");
...
void ride(){
    ...
    tell_object(TP,"The man shows you to a carriage.");
    tell_room(ETP,TPQCN+" pays the fee and steps onto the carriage.",TP);
    TP->move("/d/laerad/parnelli/forest/carriage_ride/carr2");
