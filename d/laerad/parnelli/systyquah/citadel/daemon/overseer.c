// overseer.c - daemon for systyquah citadel by Bane //
#include <std.h>
...
int do_orders(object room){
	...
	switch(random(3)){
	    case 1:
		tell_room(room,"You hear the shrieking of trolls screaming orders.");
		break;
	    case 2:
		tell_room(room,"You hear the pounding footsteps of bugbears in a close by room or hallway.");
		break;
	    case 3:
		tell_room(room,"You hear screams and swears being shouting by troll captains.");
		break;
	}
    }
...
int sound_alarm(object room){
    if(alarm > 0) return 1;
   tell_room(room,"%^BOLD%^An ear piercing shriek is let loose from near by!");
    alarm = 1;
    count = 500;
    count_down();
}
