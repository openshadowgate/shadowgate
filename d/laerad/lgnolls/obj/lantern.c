...
void create() {
  ::create();
    set_name("asgard lantern");
    set_id( ({ "lantern", "hooded", "hooded lantern" }) );
    set_short("An Asgardian lantern");
    set_long("This hooded lantern could be a collector's item these days. It "+
	"looks like a ordinary hooded lantern, except for some %^CYAN%^odd-looking "+
	"r%^BOLD%^u%^RESET%^%^CYAN%^n%^BOLD%^e%^RESET%^%^CYAN%^s "+
	"%^WHITE%^encircling the %^YELLOW%^sigil of Asgard.%^RESET%^\n");
    set_weight(2);
    set_value(2500);
    full_fuel = 2000;
    fuel = 2000;
...
int is_asgaard_lantern(){ return 1; }
