...
create (){
  ::create ();
   set_moving(0);
  set_speed(0);
  set_name("Torm outpost guard");
  set_id(({"private","guard","torm guard","soldier","Torm regular","infantry",
  "torm guard","torm guardsman","gateguard"}));
  set_short("Torm city guard");
  set_long("This guard is one of the soldiers required to protect the new Outpost on Laerad. "+
  "He stands vigilent in his duties and is obviously not just a fresh recruit");
...
void slice(object targ){
  ...
  tell_object(targ, "%^RED%^The city guards sword slips past your guard and slices you deeply in the arm!");
     targ->do_damage("left arm", random(10)+9);
     return 1;
