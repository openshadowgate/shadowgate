// Wadi al-Emir River Dock
// for Verhedin
// Thorn@ShadowGate
// 12 June 2000
// Shamelessly ripped off from Tristan's
// Shadow Dock and altered.
// dock.c
...
int __Read_me(string str){
   ...
   write(
@THORN
This is the river boatyard. From here you can do any number of things
...
int rent(string str){
   ...
   if((int)TP->query_money("gold") < 1000)
...
int buy(string str){
   ...
   if((int)TP->query_money("gold") < 20000)
   ...
   SAVE_D->set_value("tsarven_boat_owners",TPQN);
...
"+ctime(time())+"\n");
   ...
   SAVE_D->remove_name_from_array("tsarven_boat_owners",TPQN);
   TP->add_money("gold",money);
   return 1;
}
