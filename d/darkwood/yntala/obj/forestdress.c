...
int wearme(string str){
   ...
   if((string)ETO->query_gender() != "female") {
      tell_room(environment(ETO),"%^MAGENTA%^"+ETOQCN+" bats his eyes seductively as he slithers into the gown and gets in touch with his more feminine side.%^RESET%^",ETO);
      ...
      tell_room(environment(ETO),"%^GREEN%^"+ETOQCN+" smiles softly as she slips into the gown.%^RESET%^",ETO);
      tell_object(ETO,"%^GREEN%^You smile softly as you slip into the gown.%^RESET%^");
      return 1;
}
int unwearme(string str){
   if((string)ETO->query_gender() != "female") {
      tell_room(environment(ETO),"%^MAGENTA%^"+ETOQCN+" looks dissapointed as he removes the dress and becomes manly once again.%^RESET%^",ETO);
   ...
   tell_room(environment(ETO),"%^GREEN%^"+ETOQCN+" reluctantly removes the gown.%^RESET%^",ETO);
