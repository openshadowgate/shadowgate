...
void create() {
   ...
   set_name("%^BOLD%^%^BLUE%^A%^CYAN%^f%^MAGENTA%^a%^RESET%^g%^BOLD%^a%^CYAN%^l%^RESET%^i%^BLUE%^c%^RESET%^");
   set_obvious_short("%^MAGENTA%^A large %^BOLD%^c%^WHITE%^o%^RESET%^CYAN%^r%^RESET%^a%^BOLD%^%^MAGENTA%^l %^RESET%^%^MAGENTA%^spear%^RESET%^");
   set_short("%^BOLD%^%^CYAN%^F%^MAGENTA%^a%^RESET%^g%^BOLD%^a%^CYAN%^l%^RESET%^i%^BLUE%^c%^RESET%^");
   set_long("%^BOLD%^%^WHITE%^The extra long shaft of this exquisite spear is made entirely of knotty white"
