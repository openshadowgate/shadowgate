...
::create();
   ...
   set_long("%^BOLD%^%^BLACK%^This is a north-south tunnel that is quite large and easy to move in."
"  No slime covers the walls here, instead, they actually"
...
" still hang down from near the top of the walls, and bits of old carpet line the floor.%^RESET%^");
   ...
   set_exits((["north":INRMS+"hall4","south":INRMS+"sroom"]));
}
