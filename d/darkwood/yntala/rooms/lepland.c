...
::create();
   ...
   set_items(([
      "rainbow":"%^YELLOW%^This is a very pretty and bright %^MAGENTA%^r%^RED%^a%^BLUE%^i%^YELLOW%^n%^GREEN%^b%^RED%^o%^CYAN%^w.    %^YELLOW%^It is quite a bit larger than any other you've seen before, and its %^MAGENTA%^radiance %^YELLOW%^encompasses the entire area.  Peering more closely, it looks as though there may be a pot of some sort inside the rainbow itself.  Perhaps you could go there.%^RESET%^",
   ...
   set_exits(([
"rainbow":INRMS+"eor","north":INRMS+"leplandn","south":INRMS+"leplands","east":INRMS+"leplande","west":INRMS+"leplandw"
]));
}
...
" green hills covered in fresh spring grass and clovers.  A large rainbow ends here and the atmosphere is one of "
"pure bliss.  Playful little bunnies happily bounce around the area without a care in the world.%^RESET%^\n");
} 
