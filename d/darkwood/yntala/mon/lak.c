...
::create ();
   ...
   set_short("%^RESET%^%^GREEN%^A portly little leprechaun%^RESET%^");
...
" %^RESET%^%^GREEN%^eyes twinkle mischieviously.%^RESET%^");
  ...
  set_exp(5500);
   ...
   set_spell_chance(95);
   set_spells(({"lightning bolt","fireball","hideous laughter","dispel magic","fireball"}));
   set_property("magic resistance",30);
   if(random(10) < 1){
      obj = new("/d/magic/scroll");
      obj->set_av_spell(7-random(2));
      obj->move(TO);
   }
   obj = new("/d/magic/scroll");
   obj->set_av_spell(5-random(2));
   obj->move(TO);
