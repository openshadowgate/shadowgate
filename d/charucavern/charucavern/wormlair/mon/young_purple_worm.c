...
{
    ::create();
    set_name("a young purple worm");

    set_id(({"worm", "purple worm", "young purple worm", "wormlairsaideguardxx", "magical beast"}));

    set_short("%^RESET%^%^MAGENTA%^young p%^BOLD%^u%^RESET%^%^MAGENTA%^rpl%^BOLD%^"+
...
void swallow(object targ)
{
    ...
    if(targ->reflex_save(16) && !targ->id("saidewormkeeperhowne"))
    {
...
void heart_beat()
{
    ...
    if((int)TO->query_hp() < (int)TO->query_max_hp()) TO->set_hp((int)TO->query_hp() + 20 + random(20));
