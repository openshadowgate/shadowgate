...
void create() {
  ::create();
  set_name("behir");
  set_id(({"behir","magical beast","snake-like reptile"}));
  set_race("behir");
  set_gender("male");
...
void stroke(object targ) {
  ...
  if(((string)targ->query_property("strength") == "breath weapon")) {
    ...
    tell_object(targ, "%^RED%^You begen to shake radically and fall upon the ground.");
    targ->set_paralyzed(7);
...
void die(object ob) {
  message("environment","%^GREEN%^As the Behir begins to die he releases a long strong errie sound!",environment(ob));
  environment(this_player())->do_die();
  ::die(ob);
}
