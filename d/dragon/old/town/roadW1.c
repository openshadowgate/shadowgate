...
%^CYAN%^%^BOLD%^sign over the north door reads %^RED%^Sanctuary Hotel%^CYAN%^.
GRAY
    );
    set_exits(([
        "west" : "/d/dragon/town/roadW2",
       "north" : "/d/dragon/town/hotel",
       "east" : "/d/dragon/town/square"
    ] ));
    set_items(([
    ] ));
}
...
void reset() {
  ::reset();
  if(!present("beggar")) {
    new("/d/dragon/mon/beggar")->move(this_object());
  }
}
