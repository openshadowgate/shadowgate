...
void create(){
   ::create();
   set_name("blue firedoom staff");
   set_id(({"staff","firedoom","firedoom staff","blue firedoom staff"}));
   set_short("%^RESET%^%^BLUE%^Firedoom staff%^RESET%^");
   set_long("%^BLUE%^This is a heavy quarter staff. It is forged from "+
...
int more_wield() {
   if( !interactive(ETO) || wizardp(ETO) ) return 1;
   if( (int)ETO->query_level() < 30 ) {
      ...
      if( (string)ETOQN != owner ) {
         write("%^BOLD%^%^BLUE%^The %^RESET%^BLUE%^Firedoom staff "+
...
int more_hit(object victim) {
   ...
   switch(choose) {
      case 1:
         if(random(10))
            return 0;
         message("my_action","The %^RESET%^BLUE%^Firedoom staff %^RESET%^"+
            "ejects a large %^BLUE%^blue %^RESET%^ball of fire at "+
            victim->query_cap_name()+"'s body.",ETO);
         message("other_action","The %^RESET%^BLUE%^Firedoom staff "+
            "%^RESET%^wielded by "+ETO->query_cap_name()+" ejects a large "+
            "%^BLUE%^blue %^RESET%^ball of fire at "+victim->query_cap_name()+
            "'s body.",environment(ETO),( ({ETO,victim}) ));
         message("my_action","The %^RESET%^BLUE%^Firedoom staff %^RESET%^"+
            "wielded by "+ETO->query_cap_name()+" ejects a large %^BLUE%^"+
            "blue %^RESET%^ball of fire at you!",victim);
