...
void create()
{
	...
	set_obvious_short("%^BOLD%^%^BLACK%^A scythe made of jet%^RESET%^");
   	...
   	set_long("%^BOLD%^%^BLACK%^This handle of the scythe is made from a long and polished piece of solid jet, it is so black it seems almost to absorb the light that shines on it. The blade is joined at the top by a %^CYAN%^steel%^BLACK%^ setting, it is curved and very sharp. A large %^BOLD%^%^BLUE%^b%^GREEN%^l%^BLUE%^a%^BLACK%^c%^BLUE%^k %^BLACK%^o%^BLUE%^p%^GREEN%^a%^BLACK%^l has been imbedded at the bottom of the long handle.%^RESET%^");
	set_lore("The Night Reaver was a fearsome weapon that was once used by a half-drow warrior only known as The Hope Reaper.  This fearsome figure dressed in long flowing black robes that were tattered and worn.  His white hair was always dishelven.  He was said to have worn a darksteel mask over his face, that took the shape of a skull.  The Hope Reaper preyed upon families and young lovers in Heartlands, fertile farmlands that supply food for all the cities on the main land.  The Hope Reaper with his jet scythe broke into homes, slaying newborn babes and newlywed couples.  He struck at random, which only made it more difficult to catch him.  On a warm spring night in 438, the Hope Reaper met his end at the hands of a priestess of Lysara by the name of Vionetta, who disguised herself as a simple midwife. Those who saw the battle said that the Hope Reaper melted away in a black mist under the power of Vionetta's spells, but all anyone found were his tattered robes.  Some claim that the Hope Reaper is still out there, preying on the newborns and newlyweds to spread loss to the world.");
...
int wield_func(string str) {
     if((int)ETO->query_lowest_level() < 30) {
      	tell_object(ETO,"%^BOLD%^%^BLACK%^The scythe refuses you!");
     	return 0;
	}
      	...
      	tell_room(environment(ETO),"%^BOLD%^%^BLACK%^Dark flames coat"+
			...
			"slices open "+targ->QCN+".",({ETO,targ}));
      			targ->do_damage("torso",random(10)+5);
                        targ->set_paralyzed(random(4)+2,"%^BOLD%^%^BLACK%^The dark flames burn your very soul!");
