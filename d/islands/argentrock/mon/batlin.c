...
int set_paralyzed(int num, string str) { return 0; }
int set_tripped(int num, string str) { return 0; }
...
void check_them()
{
    ...
    else
    {
        if( base_name(ETO) == PATH+"arghouse11" )
        {
            ...
            {
                if(!objectp(vortices[i])) { continue; }
                vortices[i]->remove();
            }
            vortices -= ({0});
            return;
...
void do_it()
{
    ...
    for(i = 0;i < all;i++)
    {
        ...
        if( !userp(living[i]) && !interactive(living[i]) && !living[i]->is_merc() ) { living[i]->die(); }
        if( wizardp(living[i]) ) continue;
        if( avatarp(living[i])) continue;
