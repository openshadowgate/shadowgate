...
void create(){
	...
	set_long(
         ...
         "coat will not make you a popular individual.%^RESET%^");
      set_weight(8);
	set_value(75);
	set_lore(
...
int strike_func(int damage, object what, object who){
	...
	tell_room(environment(query_worn()),"%^BOLD%^%^RED%^"+who->QCN+" "+
         "looks as if "+who->QS+" is ready to vomit as "+who->QS+" gags on the "+
	...
	tell_object(who,"%^BOLD%^%^RED%^What the hell is that smell?!  "+
         ...
         "calm your stomach before you vomit.");
		who->set_paralyzed(random(2)+1,"You are recovering from that foul smell!");
return damage;	}
}
