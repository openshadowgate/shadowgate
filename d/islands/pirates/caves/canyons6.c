...
"from the outside is very dim.  The canyon wall has fallen here, blocking the canyon with rubble.\n");
...
"%^BOLD%^%^CYAN%^water%^RESET%^ trickle from them.",
     ...
     "rubble" : "Huge chunks of granite block your way."
   ]));

   set_exits(([
     "southwest":CAVES+"canyons5",
   ]));
}
