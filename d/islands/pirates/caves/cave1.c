...
void create(){
   ...
   set_travel(RUBBLE);
   set_name("Cave Entrance");
   set_short("Cave Entrance");
   set_long("%^BOLD%^A Cave Entrance%^RESET%^\n"
"You have entered a cave.  It is quite dark.  The cave looks naturally formed.  Water drips from the top of "
"the cave.  You can see stalagmites jutting up from the cave floor and stalactites hanging down from the "
"ceiling.  You have the uneasy suspicion that you are being watched.\n");
...
void init(){
   ::init();
   do_random_encounters(({MON"piercer"}),80,6);
}
