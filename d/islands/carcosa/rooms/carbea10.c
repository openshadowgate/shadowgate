inherit "/std/room.c";
string LUCIFER;
void create() {
  ::create();
  set_property("indoors",0);
  set_property("light",3);
  set("short","The Isle of Carcosa");
  set("long",
@LUCIFER
You are stading on the coast of Carcosa.
There appears to be some sort of footprints in the sand.
To the north, the island's %^BLUE%^river%^RESET%^ empties into the sea.
LUCIFER
);
  set_items((["footprints":"Webbed footprints lead up from the water.\n",
              "river":"The river is rather wide at this point and looks uncrossable.\n"]));
    set_listen("default","You hear the sound of rushing water to the north.");
    set_smell("default","The scent of the sea air pervades your senses.");

  set_exits((["south":"/d/islands/carcosa/rooms/carbea9.c"]));
  }
     
    void init() {
      ::init();
      do_random_encounters(({"/d/islands/carcosa/mon/crab2.c"}),60,6);
}
