...
int wield_func(){
   ...
   if((string)ETO->query_gender()=="male"){
      ...
      if(((int)ETO->query_alignment() % 3)==0 && !firstwield){ // evils can only be recognized as the master
         ...
         tell_room(EETO,"%^BOLD%^%^BLACK%^"+ETO->QCN+" is encompassed by a soothing dark shadow as "+ETO->QS+" takes command of the mighty scimitar!",ETO);
         firstwield = 1;
      }
      if((int)ETO->query_level()<24){
         ...
         while(TO->query_property("enchantment")< 3) set_property("enchantment",3);
         return 1;
      }
      if((int)ETO->query_level()<30){
         ...
         while(TO->query_property("enchantment")< 4) set_property("enchantment",4);
         return 1;
      }
      if((int)ETO->query_highest_level()<35){
         ...
         while(TO->query_property("enchantment")< 5) set_property("enchantment",5);
         return 1;
      }
      tell_object(ETO,"%^RESET%^%^MAGENTA%^The sword purrs happily as you take her into your hands.");
