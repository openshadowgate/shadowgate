...
void create(){
	...
	set_long(
"%^BOLD%^%^BLUE%^This armor is suprisingly light. The craftsmanship is obviously that of a master. The plates are shaped to lock perfectly with each other, while still leaving room for mobility. It would seem each plate is molded from an individual blue dragon scale. When in the light, the armor shines from dull blue to sapphire.");
	set_value(14000);
   while((int)TO->query_property("enchantment") != 5) {
     ...
     TO->set_property("enchantment",5);
      ...
      set_item_bonus("endurance",10);
      set_item_bonus("damage resistance",15);
	set_wear((:TO,"wear_func":));
	set_remove((:TO,"remove_func":));
   set_lore(
      "Who made this armor is not known, but what is known is that the dwarf Maros Battlefield was first seen with it. He led parties to kill several dragons, out of revenge, after his brother was killed by a red dragon. It was only fate that Maros would eventually suffer the same fate as his brother to the exact same dragon.");
}
...
int wear_func(){
   ...
   if((int)ETO->query_level() < 35) {
     ...
     return 0;
        ...
        tell_room(ETP,"%^WHITE%^%^BOLD%^"+ETO->query_cap_name()+" buckles up the fullplate",TP);
	tell_object(TP,"%^WHITE%^%^BOLD%^You buckle up the heavy plate, and feel reinsured by its protection");
	return 1;
}

int remove_func(){
        tell_room(ETP,"%^WHITE%^%^BOLD%^"+ETO->query_cap_name()+" removes the plate and lets out a sob",TP);
