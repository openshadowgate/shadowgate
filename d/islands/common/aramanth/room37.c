...
%^ORANGE%^You are on a sandy beach. Just north of here, the terrain becomes very rocky. The soft sand is very fine and your feet sink deeply into it leaving irrecognizable footprints behind. A natural wall of rock forms a cliff to the northwest that runs along the beach to the north. %^CYAN%^The o%^BOLD%^%^WHITE%^ce%^RESET%^%^CYAN%^a%^BOLD%^n %^RESET%^%^CYAN%^s%^BOLD%^%^WHITE%^p%^CYAN%^rout%^RESET%^%^CYAN%^s en%^WHITE%^d%^BOLD%^%^CYAN%^le%^RESET%^%^CYAN%^s%^BOLD%^%^WHITE%^s%^RESET%^%^CYAN%^ly %^BOLD%^%^WHITE%^b%^RESET%^%^CYAN%^e%^BOLD%^fo%^RESET%^%^CYAN%^r%^BOLD%^%^WHITE%^e %^RESET%^%^CYAN%^y%^BOLD%^ou%^RESET%^%^CYAN%^.%^RESET%^
");
   set_listen("default", "Waves gently break against the beach.");
       set_exits(([
    "north" : "/d/islands/common/aramanth/room33",
    "west" : "/d/islands/common/aramanth/room36",
    "south" : "/d/islands/common/aramanth/moonwell"
]));
