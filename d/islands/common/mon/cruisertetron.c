...
object get_targ()
{
    ...
    if(!objectp(obj)) { return TO->query_current_attacker(); }
    return obj;
}
...
void impaler(object targ)
{
    ...
    if(fodder_check()) { fireball(targ); return; }

    if(targ->fort_save(35+random(10))) { dam = "hurts"; }
    else { dam = "impales"; }
...
void fire(object targ)
{
    ...
    if(targ->reflex_save(35+random(10))) { dam = "singes"; }
    else { dam = "incenerates"; }
...
int fodder_check()
{
    ...
    {
        if(!objectp(live[i])) { continue; }
        if(interactive(live[i])) { continue; }
        if(live[i]->is_merc()) { continue; }
        count++;
    }
    if(count > 5) { return 1; }
    return 0;
...
void heart_beat()
{
    ...
    {
        rcount++;
        if(rcount > 60) TO->set_hp((int)TO->query_max_hp());
    }
...
int set_paralyzed(int time,string message){return 1;}
int set_tripped(int x, string s) { return 0; }
