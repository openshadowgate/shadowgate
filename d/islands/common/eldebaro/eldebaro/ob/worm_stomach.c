...
void create() 
{
    ...
    set_name("Stomach of a desert crawler");
    set_short("Inside the stomach of a desert crawler");
    set_property("no sticks",1);
    set_property("no teleport", 1);
    
    set_long("%^BOLD%^%^GREEN%^You are inside the stomach of a %^RESET%^"+
    "%^ORANGE%^desert%^BOLD%^%^GREEN%^ crawler. There is little room to move "+
...
int claw_func(string str)
{
    ...
    health = damage_stomach(TP, roll_dice(4, 12));
    if(health <= 0) 
    {
        if(objectp(myBeast))
        {
...
void set_my_worm(object targ, object vic)
{
    ...
    if(!mapp(MyStomachHealth)) MyStomachHealth = ([vic : 300]);
    else if(MyStomachHealth[vic]) MyStomachHealth[vic] = 300;
    else MyStomachHealth += ([vic : 300]);
}
