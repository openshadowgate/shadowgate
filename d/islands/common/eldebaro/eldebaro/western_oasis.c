...
#define WEST_OCEAN_ROOMS ({"1", "2"})
#define EAST_OCEAN_ROOMS ({"13", "14"})
#define SOUTH_OCEAN_ROOMS ({"4", "9", "10"})
#define SOUTH_EAST_OCEAN_ROOMS ({"15"})
#define SOUTH_WEST_OCEAN_ROOMS ({"3"})
...
void create()
{
    ...
    set_short("%^RESET%^%^ORANGE%^West%^BOLD%^e%^RESET%^%^ORANGE%^rn o%^BOLD%^a%^RESET%^%^ORANGE%^sis%^RESET%^");
...
string base_long()
{
    ...
    room  = base_name(TO);
    room  = replace_string(room,ELROOMSW,"");
    ...
    if(member_array(room, SPRING_ROOMS) != -1)
    {
        ...
        "from accidentally falling into it. You quickly realize that it is %^BOLD%^%^RED%^blood%^RESET%^%^ORANGE%^ "+
        "spilling out of it, not water at all. The remnants of a tent are scattered about the area, it looks as if it has "+
        "been ripped into shreds. There are corpses piled up here, several feet high. They are several different races "+
        "and all of them were obviously mutilated before death.%^RESET%^";
        
        add_item(({"tent", "tent remnants", "remnants"}), "%^RESET%^%^ORANGE%^What was once a tent is now nothing more than "+
        "shredded remnants. They are scattered about the area.%^RESET%^");
        
        add_item(({"spring", "bubbling spring", "grate", "metal grate"}), "%^BOLD%^%^CYAN%^This large spring bubbles up to the surface "+
        "and spills out here before it is absorbed by the sand, you realize though, that it is of blood, not water. "+
        "A large metal grate has been fitted over the top of it. It looks "+
        "very heavy and very solid. You are not sure what purpose it serves but it is likely either intended to keep people out "+
        "or perhaps to prevent anyone from accidentally falling into it. You can see several corpses floating beneath the "+
        "grate, as if someone removed it and then shoved some bodies down into it.%^RESET%^");
        
        add_item(({"corpses", "mutilated corpses"}), "%^BOLD%^%^GREEN%^These corpses are piled several feet high. They are all of "+
        "different races and each of them has been mutilated viciously before death. It looks as if all of them were hacked to "+
        "death recently. You can see that there are some of them stuffed into the spring, under the metal grate.%^RESET%^");

    }
    else if(member_array(room, TENT_ROOMS) != -1) 
    {
        clong += "%^RESET%^%^ORANGE%^ The remnants of a heavy and thick tent are scattered throughout the area here. It looks as if "+
        "something violently shredded it. Corpses are piled up here, several feet high. They are several different races and "+
        "all of them were obviously mutilated before death.%^RESET%^";
        
        add_item(({"tent", "heavy tent", "thick tent", "thick and heavy tent"}), "%^RESET%^%^ORANGE%^The remants of what was "+
        "once a large tent are scattered throughout the area. It looks as if something violently shredded it.%^RESET%^");
        
        add_item(({"corpses", "mutilated corpses"}), "%^BOLD%^%^GREEN%^These corpses are piled several feet high. They are all of "+
        "different races and each of them has been mutilated viciously before death. It looks as if all of them were hacked to "+
        "death recently.%^RESET%^");
    }   
