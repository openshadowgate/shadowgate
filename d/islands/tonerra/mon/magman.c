...
void create(){
   ...
   set_long(
@OLI
   %^RED%^This small creature looks as if it is solid rock but it is on fire
The pointy webbed ears end in flames. Thick solid little legs end
with a butt ended 4 toed stump. Long arms end in 5 fingered talons that
drag along the ground. A mischievous grin crosses the creature's face.
He is the original pyromaniac.
OLI
   );
...
void init(){
   ...
   if(wizardp(TP) || TP->query_invis() || !(TP->is_player())) return;
