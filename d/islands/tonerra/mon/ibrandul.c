...
{
   object ob;
   if(present("ibrandlin")) return execute_attack();
   if(present("warrior")) return execute_attack();
...
void heart_beat(){
    if(!objectp(TO)) return;
    if(query_attackers() == ({}) && coreparty) coreparty = 0;
    ::heart_beat();
}
