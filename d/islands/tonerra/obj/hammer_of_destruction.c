...
void create(){
   ...
   //set_weapon_prof("exotic"); removed as part of the great weapon update ~Odin
   set_property("master weapon",1);
}

int wieldem(){
   if(base_name(ETO) == "/d/islands/tonerra/mon/tombt") return 1;
...
int hitem(object targ){
   ...
   if(random(100) > 37) return roll_dice(1,4); // same rate as ragnarok. N, 7/12.
   switch(random(10)){
      case 0..1:
...
void init(){
   ...
   if((string)TO->query_weapon_prof() != "exotic") TO->set_weapon_prof("exotic");
