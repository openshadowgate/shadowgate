...
void create(){
	::create();
		...
		set_long("%^BOLD%^%^MAGENTA%^Tharis Inn%^RESET%^\n"+
			...
			"the %^BOLD%^%^GREEN%^green %^RESET%^%^CYAN%^and "+
			...
			"cute. %^BOLD%^%^GREEN%^Leaf green %^RESET%^"+
		...
		({"walls","roses"}):"%^BOLD%^%^GREEN%^The walls are a "+
			"pale grass green color, over which someone has "+
		...
		set_exits(([
		"southeast":ROOMS"tharis_hotel2",
		]));
		
		set_door("green door",ROOMS"tharis_hotel2","southeast",0);
}
