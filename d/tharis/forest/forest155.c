...
OLI
    ...
    set_items((["floor":"The ground is cluttered  with twigs and leaves.",
                ...
                "ground":"You notice a small cob web across the paths to the southeast and southwest.",
                "cobweb":"You notice that it is connected to a bent tree. You would have to disarm the trap to go further."
                ]));
    set_exits(([
                "southeast":TFOREST"forest156",
                "southwest":TFOREST"forest154",
                "northwest":TFOREST"forest158",
                ]));
    ...
    set_trap_functions(({"southwest","southeast"}),({"kill_em","kill_em2"}),({"southwest","southeast"}));
}
...
int kill_em2(){
    write("%^BOLD%^As you walk southeast you casually brush away a small cob web, but....");
    tell_room(TO,"%^BOLD%^As "+TPQCN+" walks southeast "+TP->query_cap_name()+" cassually brushes away a cob web.",TP);
