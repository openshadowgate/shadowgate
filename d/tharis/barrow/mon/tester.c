...
inherit MONSTER;
#define SAVEROOM "/d/save/templedrow"
string *exp_grants;
int xx;
void init() {
    ::init();
...
void create() {
    ...
    set_long(
        ...
        "scaled winged demons is the use of a pentegram drawn in cleric "
    ...
    set_emotes(2, ({
        "%^RED%^The krykoth attempts to leave the pentegram and head "
        "towards you.%^RESET%^",
...
void die(object ob) {
    ...
    ,environment(ob));
         if(member_array("Krykoth",TP->
		query_mini_quests() == -1)) {
                TP->set_mini_quest(
			"Krykoth",20000,"%^BOLD%^%^RED%^You "
        	        "slayed the krykoth as requested by "
			"your gods.%^RESET%^"
	        );
                write("%^YELLOW%^"
                "The powers of good shine bright!%^RESET%^");
          }    
}
