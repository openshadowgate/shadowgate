...
string lng_fun(string str) {
	...
	str = 
	    ...
	    "burial field%^BLUE%^ southwest of a large burial mound. "
	    "To your "
	    "northeast is the top of the large burial mound. "
...
int exit_fun() {
    ...
    write("%^BOLD%^%^RED%^A low wail is heard as you move through the "
	  "barrow downs.%^RESET%^"
    );
    tell_room(TO,"%^BOLD%^%^BLUE%^A low wail is heard as "+TPQCN+" gets a glazed "
	"and strange look on "+TP->query_possessive()+" face as "+TP->query_subjective()+" wanders off.%^RESET%^"
    ,TP);
    return 1;
}	
