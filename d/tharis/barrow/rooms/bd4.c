...
string lng_fun(string str) {
	...
	str =
	    ...
	    "burial field%^BLUE%^ northwest of a large burial mound. "
	    "To the "
	    "southeast is the top of the large burial mound. "
...
int exit_fun() {
    ...
    write("%^BOLD%^%^BLUE%^A sleepy feeling comes over you while moving "
	"through the barrow downs.%^RESET%^"
    );
    tell_room(TO,"%^BOLD%^%^BLUE%^"+TPQCN+" gets a glazed "
	"and strange look on "+TPP+" face as "+TPSX+
	" wanders off.%^RESET%^"
    ,TP);
    return 1;
}	
