...
int FLAG;
object here = RPATH "inner4";

void create() {
    ::create();
    set_exits(([ "out": RPATH "bd6" ]));
}
...
int twist_func(string str) {
    ...
    write("%^BOLD%^%^RED%^You manage to pivot the statue a foot to the "
        "left.%^RESET%^");
    tell_room(TO,
        "%^BOLD%^%^RED%^"+TPQCN+" twists the statue a foot to the left.",TP);
    message("info",
        "%^BOLD%^%^RED%^Suddenly, for no apparent reason, a portal appears "
        "in the floor!%^RESET%^",here);
    here->add_exit( RPATH "inner5" , "portal" );
...
void reset() {
	...
	if(random(100) < 50) { 
            ...
            message("info",
                "%^BOLD%^%^RED%^Suddenly, for no apparent reason, "
		"the portal "
        	"vanishes from the floor!%^RESET%^"
    	    ,here);
	}
        FLAG = 0;
    }
