...
that the noise from above has grown less. Then you notice why.\n  The floor
has begun to slope downward. The water is deeper here. It gushes by you
forcefully. The water is still getting deeper.
OLI
	);
	set_exits(([
		"south":"/d/tharis/Tharis/sewer25",
		"north":"/d/tharis/Tharis/sewer23"
		
		]));
	set_listen("default","Your boots splash through the the water."+
				"You also hear murmurs from the town above");
        set_smell("default","The smell of decay and refuse reaches your nostrils");
	set_items(([
		"walls":"The walls are dark. They are covered with a surprisingly thin layer of slime.",
              "floor":"You notice that the floor ahead of you comes to an abrupt halt. Maybe you can <jump across pit>.",
               "water":"You notice that the floor ahead of you comes to an abrupt halt as the water rushes into a pit. Maybe you can <jump across pit> though.",
	]));
...
int leap(string str) {
	...
	if(str != "across pit" && str != "north" ) return notify_fail(query_verb()+" where?\n");
	...
	if((int)TP->query_stats("dexterity") > random(17)) {
		TP->move_player("/d/tharis/Tharis/sewer22");
		...
		tell_room(find_object_or_load("/d/tharis/Tharis/sewer22"),"%^BOLD%^You hear someone fall into the pit!");
	}
	return 1;
}
