...
void create() {
   ...
   set_exits(([
      "enter" : "/d/tharis/Tharis/tharis_mage_entrance",
      "northwest" : "/d/tharis/Tharis/market6"
   ]));
...
int GoThroughDoor(){
   ...
   tell_object(TP,"%^BOLD%^%^CYAN%^You are forcibly repelled by the door, "+
      "as though finding yourself on the wrong polar end of a "+
      "powerful magnet.");
   tell_room(TO,"%^BOLD%^%^CYAN%^"+TPQCN+" approaches the door but cannot "+
      "seem to get through.",TP);
   return 0;
}
