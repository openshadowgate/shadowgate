...
void create() {
    if(random(4))
      set_monsters(({"/d/tharis/monsters/citizen", "/d/tharis/monsters/thief", "/d/tharis/monsters/patrol"}),({ random(3), random(2), random(3) }));
 	::create();
	set_terrain(CITY);
...
Despite this you still wonder about the presence of thieves.
    To the north is the Healer of Tharis.
OLI
		 );
	  set_smell("default","Your senses are accosted by the normal smells of a city.");
   set_listen("default","The city bustles around you.");
	  set_exits(([
		      "north":"/d/tharis/Tharis/healer",
		      "east":"/d/tharis/Tharis/estreet3",
		      "west":"/d/tharis/Tharis/estreet1"
		      ]));
	}
