...
little alley ends here in a %^BOLD%^%^BLACK%^shadowy alcove%^RESET%^.
CIRCE
   );
   ...
   set_items(([
      ...
      ({"alcove","shadowy alcove","shadow","shadows"}):"%^BOLD%^%^BLACK%^"+
         "Peering into the shadows, you realize they must be "+
         "concealing a small doorway to the north."
   ]));
   set_exits(([
      "east":ROOMS"a2",
      "north":ROOMS"thief_tailor",
      "doorway":ROOMS"thief_tailor"
   ]));
   set_invis_exits(({"north","doorway"}));
   set_search("default","Searching around, you realize there's a "+
      "doorway to the north, almost completely hidden by the shadows.");
}
