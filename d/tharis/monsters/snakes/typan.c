...
void create(){
	::create();
	set_name("typan");
	set_id(({"typan","Typan","snake"}));
	set_short("A typan snake");
	set_long(
@OLI
	This is a dangerous snake from a land unblemished by man. It has 
long fangs protruding from its mouth. Small in size, you are sure it 
can still hurt you.
OLI
	);
	set_hd(2,1);
