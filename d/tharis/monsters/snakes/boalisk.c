...
void create(){
	::create();
	set_name("boalisk");
	set_id(({"snake","boalisk","Boalisk"}));
   set_short("A Boalisk constrictor");
	set_long(
@OLI
   This large glassy eyed snake has the thick, strong body of all 
constrictor snakes. They have large glassy eyes. These scare you with
the eerie glint of the gaze.
OLI
	);
	set_body_type("snake");
	set_hd(8,8);
...
void gaze(object targ){
	...
	if("daemon/saving_d"->saving_throw(targ,"paralyzation_poison_death",0)){
		tell_object(targ,"%^BOLD%^%^GREEN%^You feel a sickness taking hold in your body!\n");
