...
void create() {
    ::create();
        set_name("recruiter");
        set_id( ({ "recruiter","halaquin"}) );
        set_race("human");
        set_gender("male");
       set_short("A recruiter in Tharis");
        set_long("This is a recruiter for the city of Tharis.");
        set_level(15);
        enable_commands();
        set_body_type("human");
        set("aggressive",0);
...
int catch_name(string str){
        ...
        write("You say "+str);
        if((strsrch(str,"greetings")!=-1)  ||  (strsrch(str,"hello")!=-1)
||(strsrch(str,"excuse")!=-1)){
        ...
        if(str == name){
           ...
           tell_room(ETO,"He says: take this to the securtiy building",TO);
           write("and he pushes you out");
        TP->move("/d/tharis/Tharis/market7.c");
        }
