...
void make_me(){
	...
	set_class(theclass);
       set_guild_level(theclass,8);
       set_mlevel(theclass,8);
	...
	case "cleric":
		...
		new("/d/common/obj/armour/chain")->move(TO);
		command("wear armor");
		set_spells(({ "cause light wounds","call lightning", "dispel magic", "hold person"}));
		set_spell_chance(60);
		break;
	case "mage":
		set_hp(35);
...
void heart_beat(){
	...
	if(objectp(ETO)){
		if(ob = present("elf",ETO)) { if(!ob->query_true_invis()) { kill_ob(ob,1); } }
		if(ob = present("elf 2",ETO)) { if(!ob->query_true_invis()) { kill_ob(ob,1); } }
		if(ob = present("elf 3",ETO)) { if(!ob->query_true_invis()) { kill_ob(ob,1); } }
		if(ob = present("elf 4",ETO)) { if(!ob->query_true_invis()) { kill_ob(ob,1); } }
	}
	::heart_beat();
