...
void create() {
   ...
   %^BLUE%^You are in a small boat floating in the reflection pool.  The waters lap against the hull almost melodically.
    %^CYAN%^The great wall of the tower rises out of view just to the west.%^RESET%^
...
set_listen("default", "%^BOLD%^%^MAGENTA%^A small boat is here.\n");
   set_exits( ([
       "north" : "/d/attaya/base/pool12",
       "east" : "/d/attaya/base/pool15",
       "south" : "/d/attaya/base/pool16"
]) );
    set_items( ([
...
.\n");
this_player()->move_player("/d/attaya/base/oubliette33",);
return 1;
}
