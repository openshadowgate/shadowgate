...
void create(){
    ...
    set_long("
    You have come to a strange clearing in the canopy.  A stone altar peeks out from beneath various vines and leaves.  A large rotting tree hass fallen across the altar.  
    A strange feeling of dread fills you suddenly!
    ");
    ...
    find_object_or_load("/d/attaya/jun92");
...
void skull(object targ){
    ...
    if(objectp(targ)){
        if(present(targ->query_name())){
			...
			targ->set_paralyzed(20,"%^RED%^You are unable to move!%^RESET%^");
