...
void create(){
   ...
   set_obvious_short("%^BOLD%^%^BLACK%^A helmet with horns");
   ...
   set_lore("Worn by the dread undead warriors Ashtar and Bishimon, "+
      ...
      "a power which lingers still.  Ashtar was once a demigod who has "+
      "been doomed to follow the will of Intruder ever since the "+
      "undead took over the island known as Attaya.  Both "+
...
int wearit() {
   ...
   if(base_name(ETO) != "/d/attaya/mon/bishimon"){
      if((int)ETO->query_highest_level() < 20 /*|| member_array("Gained entry to the Crystal Tower, Defeated Bishimon",ETO->query_mini_quests()) == -1)*/) {    
   ...
   if((ob = present("dread plate of the kinnesaruda",ETO)) && living(ETO) && ob != TO){
