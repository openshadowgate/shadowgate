...
void create(){
   ...
   set_property("no teleport",1);
       set_short("Northwest guard tower");
        set_long("%^BLUE%^Northwest guard tower
%^BLUE%^This is a round room with open doors to the south and east. It has been set up as a guard tower in the midst of the chamber. A %^BOLD%^%^BLACK%^w%^RESET%^%^ORANGE%^o%^BOLD%^%^BLACK%^o%^BLACK%^d%^BLACK%^e%^RESET%^%^ORANGE%^n %^ORANGE%^pla%^BOLD%^%^BLACK%^n%^RESET%^%^ORANGE%^k%^BLUE%^ is nailed to the %^BOLD%^%^BLACK%^f%^RESET%^%^CYAN%^l%^BOLD%^%^BLACK%^o%^RESET%^%^CYAN%^or%^BLUE%^ here and extends to the south to a %^CYAN%^m%^BOLD%^%^BLACK%^e%^BLACK%^t%^RESET%^%^CYAN%^al %^CYAN%^catw%^BOLD%^%^BLACK%^a%^RESET%^%^CYAN%^lk.");
     set_listen("default", "Water roars around far below this room.");
       set_exits(([
       "south" : "/d/attaya/tower/floor14",
       "east" : "/d/attaya/tower/floor12",
]));
       set_items(([
