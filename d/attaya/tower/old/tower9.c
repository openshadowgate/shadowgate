...
");
set_listen("default", "%^BOLD%^A remarkable white crystal floats here");
    set_exits(([
      "southwest" : "/d/attaya/tower/tower8",
   ] ));
   set_items(([
         "crystal" : "The crystal glistens and adds a spark of color to this otherwise transparent scene.  It shines a brilliant %^BOLD%^white%^RESET%^.",
...
say((string)TPQCN+" picks up the crystal.\n");
TP->move_player("/d/attaya/tower/dungeon.c");
write("%^BOLD%^%^BLUE%^You are forced to drop the crystal as it transforms into a towering creature and smashes your head in.\n");
say((string)TPQCN+" is suddenly pounded into the ground by a huge "
"towering creature.  It then vanishes with the limp corpse.\n");
TP->add_hp(-(random(100)));
return 1;
}

int touch(string str) {
write("You touch the crystal and feel an electric shock.\n");
say((string)TPQCN+" touches the crystal.\n");
TP->move_player("/d/attaya/tower/tower10",);
return 1;
}
