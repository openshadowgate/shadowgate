...
void create(){
   ...
   set_property("no teleport",1);
       set_short("Library");
        set_long("
    This is a great library!  The ceiling is just above you.  High up on a balcony that circles the room, you can look down past tow other levels to see the floor and rows of bookcases.  Two large chandeliers hang in here not far away and decend into the room on chains.  A ladder on the railing across from you links this floor to the ones below it!
");
       set_exits(([
       "north" : "/d/attaya/tower/hall22",
       "west" : "/d/attaya/tower/hall18",
       "east" : "/d/attaya/tower/hall20",
]));
       set_items(([
...
void reset(){
        ::reset();
    if(!present("bookcase"))
     new("d/attaya/obj/bookcase")->move(this_object());
}
void init() {
  ::init();
...
int jump(string str) {
   if(str=="chandelier" || str == "to chandelier") {
