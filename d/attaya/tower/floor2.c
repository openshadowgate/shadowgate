...
void create(){
   ...
   set_property("no teleport",1);
       set_short("Floor of the library");
        set_long("%^ORANGE%^Floor of %^ORANGE%^the %^ORANGE%^lib%^CYAN%^r%^ORANGE%^a%^CYAN%^r%^ORANGE%^y
%^ORANGE%^You are surrounded by long %^ORANGE%^book%^CYAN%^c%^ORANGE%^a%^CYAN%^s%^ORANGE%^e%^CYAN%^s%^ORANGE%^ filled with dusty %^CYAN%^b%^CYAN%^o%^ORANGE%^oks%^CYAN%^.%^ORANGE%^ Most are too old and worn to read. The carpet is torn here and a huge gash runs west. There is dust on everything and you get the feeling that people don't come down here often.The ceiling is far above you, and %^BOLD%^%^BLACK%^sh%^RESET%^%^BLUE%^a%^BOLD%^%^BLACK%^do%^RESET%^%^BLUE%^w%^BOLD%^%^BLACK%^y %^BLACK%^forms%^RESET%^%^ORANGE%^ slip around the railings of the next level up.");
       set_exits(([
