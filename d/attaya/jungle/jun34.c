...
//
void create(){
    set_monsters(({"/d/attaya/mon/solstice"}),({random(2)+1}));
	::create();
    set_terrain(JUNGLE);
    set_travel(DIRT_ROAD);
	set_name("A dark jungle altar");
	set_short("A dark jungle altar");
	set_property("indoors",0);
	set_property("light",1);
    set_long(
        "/daemon/crayon_d"->color_string("You have come to a strange clearing in the canopy.  A stone altar peeks out from beneath various vines and leaves. A strange feeling of dread fills you suddenly!","very black"));
	...
	set_exits(([
                   "east":"/d/attaya/jungle/jun35",
                   ]));
    set_listen("default","It is strangely quiet here.");
	find_object_or_load("/d/attaya/jungle/jun34");
	set_trap_functions(({"east"}),({"skullboss"}),({"east"}));
}

int skullboss(){
        write("%^BOLD%^%^RED%^As you exit the room, you step on a trigger block hidden beneath the leaves on the jungle floor",TP);
	tell_room(TO,"%^BOLD%^As "+TPQCN+" walks east "+TP->query_possessive()+" left foot steps upon a trigger block.",TP);
	toggle_trap("east");
     skull(TP);
	return 1;
}
