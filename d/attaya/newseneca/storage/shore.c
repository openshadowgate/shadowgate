//beach.c ~Circe~ 6/17/08 for new Seneca
#include <std.h>
#include "../seneca.h"
...
void create(){
   ...
   set_name("A Golden Shoreline, Attaya");
   set_short("%^BOLD%^%^WHITE%^A %^YELLOW%^G%^WHITE%^o%^RESET%^l"+
      "%^YELLOW%^d%^WHITE%^e%^RESET%^n %^CYAN%^S%^BOLD%^h%^WHITE%^"+
      "o%^RESET%^%^CYAN%^r%^BOLD%^e%^WHITE%^l%^RESET%^%^CYAN%^i"+
      "%^BOLD%^n%^WHITE%^e, Attaya%^RESET%^");
   set_long("%^BOLD%^%^WHITE%^A %^YELLOW%^G%^WHITE%^o%^RESET%^l"+
      ...
      "wind.  %^BOLD%^Seagulls %^RESET%^ride the wind "+
      ...
      "on shore, completing the idyllic scene.  This stretch of "+
      "beach is right in the %^BOLD%^surf%^RESET%^, cooling your "+
      "toes as the %^BOLD%^%^BLUE%^waves%^RESET%^ crash in.\n");
...
void reset(){
   ::reset();
   if(!present("wave_maker") && base_name(TO) != "/d/attaya/newseneca/storage/shore.c"){
      new(MON"wave_maker")->move(TO);
   }
}
