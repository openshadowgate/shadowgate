//sstreet ~Circe~ 11/23/07
//Southern thoroughfare around Seneca, runs E-W
#include <std.h>
#include "../seneca.h"
...
void create(){
   ...
   set_name("Verde Esplanade");
   set_short("Verde Esplanade");
   set_long("%^RESET%^%^GREEN%^Verde Esplanade, Seneca%^RESET%^\n"+
      "This broad thoroughfare runs east and west along the southern "+
      "edge of the city.  Not technically an esplanade, it is nonetheless "+
      "named so because it is the closest street to the ocean here.  "+
      "It is a single line wide enough for two "+
      "carts and is meticulously paved with %^ORANGE%^co%^RED%^b%^ORANGE%^"+
      "ble%^RESET%^s%^ORANGE%^to%^RED%^n%^ORANGE%^e%^RESET%^s "+
      "in a zig-zagging pattern.  Lining the northern side of the "+
      "street at intervals are ornate %^BOLD%^%^BLACK%^"+
      "street lamps%^RESET%^ to light the way at night, while part of the "+
      "southern edge of the street is bounded by the rough %^BOLD%^co"+
