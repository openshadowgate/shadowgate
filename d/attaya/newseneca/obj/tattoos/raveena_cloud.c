...
void create(){
	...
	set_value(10000);
	set_id(({ "tattoo", "cloud" }));
	set_short("A tiny tattoo of an iridescent %^BOLD%^%^WHITE%^silvery cloud %^RESET%^with flecks of %^BOLD%^%^BLUE%^b%^BOLD%^%^CYAN%^lu%^BOLD%^%^BLUE%^es %^RESET%^that, upon closer inspection, appears to %^BOLD%^%^CYAN%^s%^BOLD%^%^WHITE%^wi%^BOLD%^%^BLUE%^r%^BOLD%^%^WHITE%^l %^RESET%^as if being constantly blown by a slight %^BOLD%^%^WHITE%^b%^BOLD%^%^CYAN%^r%^BOLD%^%^WHITE%^ee%^BOLD%^%^CYAN%^z%^BOLD%^%^WHITE%^e %^RESET%^that only it seems aware of.");
	set_long(
@MYLONG
