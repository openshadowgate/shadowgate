...
int start_trip() {
    if(!this_player()) { return 1; }
     if(random(99) >= 50) {
       write( 
@TEXT
%^BOLD%^%^WHITE%^As you walk away your foot slips on the loose pebbles
You feel yourself slipping down the cliff
%^BOLD%^%^GREEN%^HELLLLL
%^BOLD%^%^BLUE%^      LLLLLLLLLL
%^BOLD%^%^YELLOW%^               LLLLLPPPPP
%^BOLD%^%^RED%^                        PPPPPPPPPP
%^WHITE%^%^BOLD%^You can only grab onto loose dirt and some scrub brush..
