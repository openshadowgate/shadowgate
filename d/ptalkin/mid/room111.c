...
set_short("A pathway");
set_long("%^GREEN%^White gauze clothes her, transparent and weightless, the pale green skin of her child's body glimmers beneath the covering.  Waist-length silver hair ripples and shimmers with each movement she makes, a trailer of white fire against the forest's dark.  Music she alone can hear sweeps her on.");
...
set_exits (([
"northwest" : PMID+"room112.c",
"southeast" : PMID+"room98.c"
]));
set_items(([
...
if(!present("forest nymph"));
new("/d/ptalkin/mon/f_nymph")->move(TO);
}
