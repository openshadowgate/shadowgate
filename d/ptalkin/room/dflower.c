...
void create() {
    ...
    set_exits((["east" : "/d/ptalkin/room62"]));
...
for example <buy roses>.  To send an item to another player, you do NOT need 
to <buy> an item.  Just type <send item to player> and the item will be sent.  
...
int send_fun(string str) {
	...
	"the only items we deliver here!\n");
	return 0;
}
      switch(type) {
      case "peach" : 
	...
	switch (ITEM) {
	  ...
	  default : PRESENT=new(PEACH); break;
		}
	    SENDER = TP;
            ITEM=0;
