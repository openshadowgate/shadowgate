...
void create()
{
	...
	"were a living being trying to %^YELLOW%^strangle%^BLACK%^ the last bits of %^WHITE%^life%^BLACK%^ from your body. To the north you see that the hallway continues into the darkness. South is the hallwa"
	"y you came from upon entering this %^RESET%^%^MAGENTA%^d%^CYAN%^e%^MAGENTA%^ath r%^CYAN%^i%^MAGENTA%^dden t%^CYAN%^o%^MAGENTA%^mb.%^RESET%^"
	);
...
set_items(([ 
    ...
    set_exits(([ 
		"north" : "/d/av_rooms/asmodeus/a_tomb2",
	]));
