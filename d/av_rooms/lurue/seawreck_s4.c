...
void create()
{
	...
	"To the west, an underwater structure, covered in %^ORANGE%^coral%^CYAN%^ causes the waves to break and splash, but you are far too distant to make out what it is.%^RESET%^"
	);
...
set_items(([ 
	({ "structure", "coral" }) : "%^RESET%^%^ORANGE%^From here all you can tell is that there is something under the water to the west.  What it is you're not sure, but at least out here you might be able to %^BLACK%^%^BOLD%^dive%^RESET%^%^ORANGE%^ without having to worry about being shoved into the sharp coral shelves.%^RESET%^",
	]));

    set_exits(([ 
		"dive" : "/d/av_rooms/lurue/seawreck_u3",
		"northwest" : "/d/av_rooms/lurue/seawreck_s1",
	]));
