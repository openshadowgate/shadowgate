...
void create()
{
	...
	"^BLACK%^tugs at your mind, toying with your vision, revealing hints of strangely shaped edifices prowled by even stranger beings. It is much as though looking through thickly clouded %^RESET%^%^CYAN%^"
       "water%^BOLD%^%^BLACK%^ that distorts everything into rippling fuzzy lines and shapes.  All until you feel yourself bumped into by one of the netherworld's inhabitants. Though their doing so causes you to notice one slightly more solid bit of landscape.  A small grate on the floor seems almost real.%^RESET%^"
	);
...
set_items(([ 
	...
	"water" : "%^RESET%^%^CYAN%^There's no water here, it just feels like you're looking through some very cloudy water when you look around.  Almost as if you can see the city and the inhabitants just on the edge of your vision, but as soon as you focus all you see is the murkiness of nothing.%^RESET%^",
     ...
     "grate":"%^CYAN%^The grate looks just a little more "+
"real.",
	]));

     set_exits(([
           "east":"/d/av_rooms/lurue/ts_01.c",
           "west":"/d/av_rooms/lurue/ts_03.c",
           "south":"/d/av_rooms/lurue/ts_02.c",
     ]));

set_search("default","You wanted to look at what now?");
set_search("grate",(:TO,"search_grate":));
}
...
int search_grate(string str){
if(str == "grate"){
tell_room(TO,"%^BOLD%^%^BLACK%^The grate lifts without "+
"a sound!%^RESET%^");
add_exit("/d/av_rooms/lurue/ts_05.c","hole");
return 1;
}
else{tell_object(TP,"That doesn't seem possible.");
return 1;}
}
