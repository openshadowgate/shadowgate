...
void create()
{
	...
	"eiling.  They illuminate frescos of celestial objects.  The moon, the stars and the ribbons of distant galaxies all peeked at through the soft edges of clouds.  To the north a stairway leads upward to"
	" the temple proper.  Ahead to the south a set of double doors waits, flanked by a pair of statues.%^RESET%^"
	);
...
set_items(([ 
	...
	({ "statues", "statue" }) : "%^BOLD%^%^WHITE%^They look like marble carvings standing to either side of the silver set of doors.  They're quiet lifelike, though made of stone.%^RESET%^",
	]));

    set_exits(([ 
		"west" : "/d/av_rooms/lurue/selune_under1",
		"east" : "/d/av_rooms/lurue/selune_under3",
	]));
