...
%^RESET%^%^ORANGE%^A %^BOLD%^%^WHITE%^sweet%^RESET%^%^ORANGE%^, %^RESET%^%^MAGENTA%^delicate %^ORANGE%^fragrance graces the air.%^RESET%^");
    ...
    set_exits(([ 
		"north" : "/d/av_rooms/lurue/masq_t4b",
		"west" : "/d/av_rooms/lurue/masq_t49",
		"south" : "/d/av_rooms/lurue/masq_t40",
		"east" : "/d/av_rooms/lurue/masq_t41",
...
int room_south(string str) {
   if(!present("masq_key_40",TP) && !avatarp(TP)) {
     tell_object(TP,"You can't seem to pass through the doorway. You will need key 40 to enter here.");
     return 0;
   }
...
int room_west(string str) {
   if(!present("masq_key_49",TP) && !avatarp(TP)) {
     tell_object(TP,"You can't seem to pass through the doorway. You will need key 49 to enter here.");
     return 0;
   }
   tell_object(TP,"The key in your hand hums as you pass through the doorway.");
   return 1;
...
int room_east(string str) {
   if(!present("masq_key_41",TP) && !avatarp(TP)) {
     tell_object(TP,"You can't seem to pass through the doorway. You will need key 41 to enter here.");
     return 0;
   }
   tell_object(TP,"The key in your hand hums as you pass through the doorway.");
   return 1;
