...
set_items(([ 
	({ "curtain", "curtain flap", "flap" }) : "%^RESET%^Made of heavy %^BOLD%^%^MAGENTA%^pink%^RESET%^ tent canvas, it is sewn along the top and one side to the tent panels, allowing it to drop back down into place easily.  A little clever dowel and ring system along the open side of the flap allow the curtain to be secured while the occupant is changing.",
    ...
    set_exits(([ 
		"north" : "/d/av_rooms/lurue/masq_t2",
	]));

    set_door("pink flap","/d/av_rooms/lurue/masq_t2","north");
    set_open("pink flap",0);
    set_string("pink flap","knock","knocks upon the wooden frame beside the");
    set_string("pink flap","open","You lift the heavy canvas flap.");
    set_string("pink flap","close","You lower the heavy canvas flap back into place.");
    set_door_description("pink flap","%^RESET%^Made of heavy %^BOLD%^%^MAGENTA%^pink %^RESET%^tent canvas, it is sewn along the top and one side to the tent panels, allowing it to drop back down into place easily.  A little clever dowel and ring system along the open side of the flap allow the curtain to be secured while the occupant is changing.%^RESET%^");
}