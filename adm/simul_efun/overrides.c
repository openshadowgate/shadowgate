...
object find_living(string str)
{
    ...
    if (sscanf(geteuid(previous_object()),"%sobj",whom) > 0)
    {
        ...
        if (objectp(who) && ((int)who->query_level() > (int)this_player()->query_level())) return 0;
    }
...
object find_player(string str){
    ...
    if (sscanf(geteuid(previous_object()),"%sobj",whom) > 0) {
        if (member_group(whom,"law")) return efun::find_player(str);
    ...
    if (!who) {
        ...
        for (i=0;i<sizeof(liv);i++) {
            if (((string)liv[i]->query_position() == "avatar" || wizardp(liv[i])) && liv[i]->query_disguised())
                if ((string)liv[i]->query_vis_name() == str)  who = liv[i];
        }
    }
    return who;
}
...
object *users() {
    ...
    if (sscanf(getuid(previous_object()),"%sobj",who) > 0) {
        if (member_group(who,"law")) return efun::users();
        List = ({});
        userList = efun::users();
        j = sizeof(userList);
        for (i=0;i<j;i++) {
            if (userList[i]->query_level() < this_player()->query_level()) List +=
                ({userList[i]});
        }
        return List;
    }
...
void destruct(object destructee) {
    ...
    if (environment(PO)== destructee)  return efun::destruct(destructee);
    ...
    if (!destee || dester==destee || dester==UID_ROOT ||
        ...
        efun::destruct(destructee);
    else write_file("dest.log",capitalize(dester)+" is not allowed to destruct "+
                    capitalize(destee)+"\n");
}
...
object find_object(string str){
    ...
    object who;
    int i,j;
    if (!geteuid(previous_object())) return efun::find_object(str);
    if (sscanf(geteuid(previous_object()),"%sobj",whom) > 0) {
        if (member_group(whom,"law")) return efun::find_object(str);
...
string query_ip_name(object ob){
    ...
    int i,j;
    if (sscanf(geteuid(previous_object()),"%sobj",whom) > 0) {
        if (member_group(whom,"superuser")) return efun::query_ip_name();
        return "";
    }
...
int living(object ob){
    ...
    if (ob->query("not living")) return 0;
...
int wizardp(object ob)
{
    ...
    {
        if(member_group((string)ob->query_true_name(), "assist")) return 1;
        return 0;
    }
...
{
    if(!stringp(str) || !strlen(str)) return str;
