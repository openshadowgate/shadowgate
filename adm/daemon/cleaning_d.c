...
int clean_filter(object obj)
{
    ...
    if(strsrch(base_name(obj),"/daemon") == 0) { return 0; }
    if(strsrch(base_name(obj),"/adm/daemon") == 0) { return 0; }
    if((clonep(obj) && !objectp(environment(obj))) && !obj->query_had_players()) { return 1; }
    if(!objectp(environment(obj)) && obj->is_monster()) { return 1; }
    if(obj->is_spell())          { return 1; }
    if(obj->is_feat())           { return 1; }
    if(!obj->is_room())          { return 0; }
    if(obj->query_had_players()) { return 0; }
    return 1;
}
...
void block_objects(object *objs)
{
    ...
    if(!pointerp(objs) || !sizeof(objs)) { begin_cleaning(); }
    ...
    if(remainder)
    {
        if(!blocks) { tmp = objs[0..remainder]; }
        else { tmp = objs[(BLOCK_SIZE * (blocks-1))..( ((BLOCK_SIZE * (blocks-1)) + remainder) -1)]; }
        obj_blocks += ({ tmp });
    }
...
{   
    ...
    if(!pointerp(objs) || !sizeof(objs)) { return; }
...
void debug_message(string str)
{
    if(!objectp(debug_object)) { return; }
