...
int add_member(object ob, string group)
{
    ...
    if (!party[group]) {
        tell_object(ob, "%^BOLD%^%^GREEN%^You are the leader of party " + group + ".\n");
        party[group] = ({});
    }
    party[group] += ({ ob });
    if (invited) {
        if (invited[group]) {
            if (member_array(ob, invited[group]) != -1) {
...
string party_member(object ob)
{
    ...
    for (i = 0; i < sizeof(groups); i++) {
        if (member_array(ob, party[groups[i]]) != -1) {
            return groups[i];
        }
    }
    return 0;
...
string *query_all_invited_now(object ob)
{
    int party;
    string * parties = ({});

    foreach(party in keys(invited)) {
        if (invited_now(ob, party)) {
            parties += ({party});
        }
    }
...
int invited_now(object ob, string str)
{
    ...
    if (member_array(ob, invited[str]) == -1) {
        return 0;
    }
...
void remove_invitation(mixed* bing)
{
    ...
    if (member_array(bing[0], invited[bing[1]]) == -1) {
        return;
    }
    invited[bing[1]] -= ({ bing[0] });
    if (!bing[2]) {
        tell_object(bing[0], "You are no longer invited to be a member of the party.\n");
    }
}
