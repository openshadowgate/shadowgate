...
inherit "/adm/failsafe/trap_data_failsafe.c";
inherit "/adm/failsafe/move_failsafe.c";
...
// check for raised resistance prop - spells & plat bands both give self-level MR & should NOT stack with drow racial. N, 9/15.
        if((string)TO->query_race() == "drow" && !TO->query_property("raised resistance")) {
