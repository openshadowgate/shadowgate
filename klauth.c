...
void cm(string str) { return CRAYON_D->color_string(str,"fire red"); }

int query_paralyzed() { return 0; }
int query_tripped() { return 0; }
...
object pick_target()
{
    ...
    if(!sizeof(ppl)) { return 0; }
    
    for(i=0;i<sizeof(hps);i++)
    {
        if(!objectp(lowest)) { lowest = ppl[i]; }
        if(!objectp(highest)) { highest = ppl[i]; }
        if(hps[i] < lowest->query_hp()) { lowest = ppl[i]; }
        if(hps[i] > highest->query_hp()) { highest = ppl[i]; }
    }
    
    if(!objectp(highest)) { return 0; }
    if(!objectp(lowest)) { return 0; }

    if(TO->query_hp_percent() > 15) { return highest; }
    return lowest;
}
...
object pick_spelled_target()
{
    ...
    {
        if(!objectp(attackers[i])) { continue; }
        if(!objectp(obj)) { obj = attackers[i]; }
        if(sizeof(attackers[i]->query_property("spelled")) > sizeof(obj->query_property("spelled"))) { obj = attackers[i]; }        
    }
...
int has_spell(object obj, string name)
{
    ...
    if(!name || !stringp(name) || !strlen(name)) { return 0; }
    ...
    for(i=0;sizeof(spells),i<sizeof(spells);i++)
    {
        ...
        if(spells[i]->query_spell_name() == name) { return 1; }
    }
...
void scale_me(int num)
{
    ...
    if(TO->query_property("scale") >= num) { return; }
...
void restore_exit()
{
    ...
    if(!objectp(TO)) { return; }
    if(TO->query_hp() > 0 && sizeof(TO->query_attackers())) { return; }
...
void heart_beat()
{
    ...
    if(!sizeof(TO->query_attackers())) { call_out("restore_exit",ROUND_LENGTH * 3); }
}
...
void single_target_spell() // always cast two
{
    ...
    case 5: // blindness
        ...
        if(!objectp(targ)) { targ = pick_target(); }
    ...
    case 7: // powerword stun
        ...
        if(objectp(obj)) { obj->dest_effect(); }
...
void do_inferno(int count)
{
    ...
    for(i=0;sizeof(attackers),i<sizeof(attackers);i++)
    {
        ...
        for(j=0;sizeof(inv),j<sizeof(inv);j++)
        {
            ...
            if(!inv[j]->query_short()) { continue; }
            if(inv[j]->query_property("no detect")) { continue; }
