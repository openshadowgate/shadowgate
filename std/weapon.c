...
//    if(wc_dice < original_wc_dice) str = query_broken();
   str = query_broken();
    if(objectp(TP) && member_array(TPQN, query_identified()) != -1 && PO->query_property("information")) {
        if(!wielded) return ::query_short()+enc+poi+str;
        else return ::query_short()+enc+poi+str+limbString;
    }
    else {
        if(!wielded) return ::query_short()+poi+str;
        else return ::query_short()+poi+str+limbString;
    }
}
...
string query_long(string str) {
   ...
   int myenchant;
   ret = ::query_long(str);
   if(!ret) { ret = "this weapon is broken"; }
...
int query_damage() {
    ...
    if(wepstatus > 100) wepstatus = 100;
...
{
    if(member_array(str,({"simple","martial","exotic"})) == -1) { return; }
    weapon_prof = str;
}
...
int drop(){
    if(query_property("enchantment") && query_property("enchantment") < 0)
        if(base_name(PO) != "/std/user")
            if(query_wielded() == TP) return 1;
   return ::drop();
}


void set_prof_level(int xxx){    maxAtLevel = xxx; }


void set_prof_type(string prof){    profType = prof; }
...
int query_level_max(){    return maxAtLevel; }


void set_weapon_speed(int x){   Weapon_speed = x; }
        ...
        if(interactive(ETO))
        {
            if(FEATS_D->usable_feat(ETO,"light weapon"))
            {
                if(query_size() > (int)ETO->query_size())
                {
                    ...
                    return Weapon_speed / 3;
                }
            }
        }
    }
    if(Weapon_speed == -1) return 3*query_size();
    return Weapon_speed;
}
...
int is_broken(){   return wc_dice < original_wc_dice; }


int query_size(){
    if(!objectp(ETO)) return ::query_size();
    if(query_property("lance") && living(ETO)) return ETO->query_size();
    return ::query_size();
}
...
int query_wc_num() { return wc_num; }
int query_wc_dice() { return wc_dice; }
int query_large_wc_num() { return large_wc_num; }
int query_large_wc_dice() { return large_wc_dice; }


// Critical hit stuff
void set_critical_threat_range(int num)
{
    if(!num || num < 1) { threat_range = 1; }
    else { threat_range = num; }
}
...
void set_critical_hit_multiplier(int num)
{
    if(!num || num < 1) { crit_multiplier = 1; }
    else { crit_multiplier = num; }
}
...
int query_critical_hit_multiplier()
{
    if(!crit_multiplier) { crit_multiplier = 2; }
...
{
    if(member_array(str, VALID_DAMAGE_TYPES) == -1) { damage_type = "untyped"; }
    damage_type = str;
}
...
{
    if(!damage_type || damage_type == "" || damage_type == " ") { damage_type = "untyped"; }
    return damage_type;
}
