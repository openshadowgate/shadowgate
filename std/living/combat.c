...
{
    return COMBAT_D->clean_up_attackers(TO);
}

string* query_atnames()
{
    return query_combat_mapps("arrays", "atnames");
}

void set_atnames(string* attNames)
{
    return adjust_combat_mapps("arrays", "atnames", attNames);
}

int kill_ob(object victim, int which)
{
    return COMBAT_D->kill_ob(TO, victim, which);
}
...
{
    return COMBAT_D->counter_attack(ob);
}

void spring_attack(object ob)
{
    return COMBAT_D->spring_attack(ob);
}

int do_casting(object targ)
{
    return COMBAT_D->do_casting(TO, targ);
}

mixed return_player_target(int flag)
{
    return COMBAT_D->return_player_target(TO, flag);
}

int do_functions()
{
    return COMBAT_D->do_functions(TO);
}
...
//called in battle.c to ensure bugs don't fuck up attacks -Ares
void prepare_attack()
{
    adjust_combat_mapps("static vars", "attacking", 0);
...
int attack_count()
{
    return query_combat_mapps("static vars", "attack count");
}

void set_for_attack()
{
    return adjust_combat_mapps("static vars", "attacking", 1);
}
...
void execute_attack()
{
    ...
    if (!query_combat_mapps("static vars", "attacking")) {
        ...
        for (i = 0; i < query_combat_mapps("static vars", "attack count"); i++) {
            adjust_combat_mapps("static vars", "attack loop", 1, 1);
            internal_execute_attack();
        }
...
protected void internal_execute_attack()
{
    return COMBAT_D->internal_execute_attack(TO);
}
...
object query_current_attacker()
{
    if (!clean_up_attackers()) {
        ...
        return query_combat_mapps("arrays", "attackers")[0];
    }
}

void run_away()
{
    return COMBAT_D->run_away(TO);
}
...
{
    adjust_combat_mapps("vars", "wimpy", x);
}
...
int query_wimpy()
{
    return query_combat_mapps("vars", "wimpy");
}

void set_wimped(int x)
{
    adjust_combat_mapps("vars", "wimped", x);
}

int query_wimped()
{
    return query_combat_mapps("vars", "wimped");
}

void set_casting(int x)
{
    adjust_combat_mapps("static vars", "casting", x);
}

int query_casting()
{
    return query_combat_mapps("static vars", "casting");
}

int valid_casting()
{
    if (query_casting() && objectp(TO->query_property("spell_casting"))) {
        return 1;
    }
    return 0;
}
...
object* query_hunted()
{
    return query_combat_mapps("arrays", "hunters");
}

object* query_attackers()
{
    if (!sizeof(query_combat_mapps("arrays", "attackers"))) {
        COMBAT_D->clean_attacker_flags(TO);
    }
...
{
    return COMBAT_D->sight_adjustment(TO);
}
...
{
    return COMBAT_D->ok_to_kill(TO, targ);
}

int light_armor_filter(object ob)
{
    return COMBAT_D->light_armor_filter(ob);
}

void ok_to_wield()
{
    return COMBAT_D->ok_to_wield(TO);
}

void add_attacker(object ob)
{
    return COMBAT_D->add_attacker(TO, ob);
}

int reaction_adj()
{
    return BONUS_D->query_stat_bonus(TO, "dexterity");
}

void thaco_messages(int thaco)
{
    return COMBAT_D->thaco_message(TO, thaco);
}
...
{
    return adjust_combat_mapps("arrays", "hunters", hunt);
}

int is_ok_armour(string myclass)
{
    return COMBAT_D->is_ok_armour(TO, myclass);
}

int is_ok_weapon(string myclass)
{
    return COMBAT_D->is_ok_weapon(TO, myclass);
}

void send_dodge(object att)
{
    return COMBAT_D->send_dodge(TO, att);
}

object* query_active_protectors(object obj)
{
    return COMBAT_D->query_active_protectors(obj);
}

object* doProtection(object player)
{
    return COMBAT_D->doProtection(player);
}

void add_reaction(int x)
{
    return adjust_combat_mapps("vars", "react", x, 1);
}

int query_reaction()
{
    return query_combat_mapps("vars", "react");
}

int query_dead()
{
    return query_combat_mapps("static vars", "dead");
}

int queryFuncing()
{
    return query_combat_mapps("static vars", "funcing");
}

void setFuncing(int i)
{
    return adjust_combat_mapps("static vars", "funcing", i);
}

int ok_combat_vars()
{
    return COMBAT_D->ok_combat_vars(TO);
}
...
void set_paralyzed(int x, string paralyze_msg)
{
    return COMBAT_D->set_paralyzed(TO, x, paralyze_msg);
}

void remove_paralyzed()
{
    return COMBAT_D->remove_paralyzed(TO);
}

void set_gagged(int x, string message)
{
    return COMBAT_D->set_gagged(TO, x, message);
}

void set_bound(int difficulty, string message)
{
    return COMBAT_D->set_bound(TO, difficulty, message);
}

varargs void set_temporary_blinded(int difficulty, string message)
{
...
varargs void set_blindfolded(int difficulty, string message)
{
    return COMBAT_D->set_blindfolded(TO, difficulty, message);
}

varargs void set_tripped(int severity, string message, int special)
{
    return COMBAT_D->set_tripped(TO, severity, message, special);
}

void set_unconscious(int xxx, string message)
{
    return COMBAT_D->set_unconscious(TO, xxx, message);
}

void set_asleep(int xxx, string message)
{
    return COMBAT_D->set_asleep(TO, xxx, message);
}

int query_unconscious()
{
    return (query_combat_mapps("vars", "unconscious") || (query_hp() < 1) || query_combat_mapps("vars", "asleep"));
}

int query_asleep()
{
    return query_combat_mapps("vars", "asleep");
}

int query_tripped()
{
    return query_combat_mapps("vars", "tripped");
}
...
{
    return query_combat_mapps("vars", "blindfolded");
}

int query_blind()
{
    return (query_combat_mapps("static vars", "blinded") + query_combat_mapps("vars", "blindfolded"));
}

int query_bound()
{
    return query_combat_mapps("vars", "bound");
}
...
int query_gagged()
{
    return query_combat_mapps("vars", "gagged");
}

string query_gagged_message()
{
    return query_combat_mapps("messages", "gagged");
}

string query_paralyze_message()
{
    return COMBAT_D->query_paralyze_message(TO);
}

string query_bound_message()
{
    return query_combat_mapps("messages", "bound");
}

string query_tripped_message()
{
    return query_combat_mapps("messages", "tripped");
}

string query_asleep_message()
{
    return query_combat_mapps("messages", "asleep");
}

string query_unconscious_message()
{
    return query_combat_mapps("messages", "unconscious");
}
...
void iterate_combat()
{
    return COMBAT_D->iterate_combat(TO);
}

void send_paralyzed_message(string type, object who)
{
    return COMBAT_D->send_paralyzed_message(type, who);
}

int reset_hunted()
{
    adjust_combat_mapps("arrays", "hunters", ({})); return 1;
}

int remove_hunted(object hunt)
{
    adjust_combat_mapps("arrays", "hunters", hunt, 1); return 1;
}
...
void reset_critical()
{
    adjust_combat_mapps("static vars", "critical hit", 0); adjust_combat_mapps("static vars", "critical message", 0);
}

varargs void adjust_combat_mapps(string type, string which, mixed val, int mod)
{
    if (!objectp(TO)) {
        return;
    }
    ...
    case "vars":
        ...
        if (!member_array(which, keys(combat_vars)) == -1) {
            return 0;
        }
        if (!mod) {
            combat_vars[which] = val;
        }else {
            if (which == "paralyzed" || which == "tripped") {
                if (val > combat_vars[which]) {
                    combat_vars[which] = val;
                }
            }else {
                combat_vars[which] += val;
    ...
    case "static vars":
        ...
        if (!member_array(which, keys(combat_static_vars)) == -1) {
            return 0;
        }
        if (!mod) {
            combat_static_vars[which] = val;
        }else {
            combat_static_vars[which] += val;
        }
        break;
    case "messages":
        if (!member_array(which, keys(combat_messages)) == -1) {
            return 0;
        }
        if (!stringp(val)) {
            if (DEFAULT_MESSAGES[val]) {
                val = DEFAULT_MESSAGES[val];
            }
        }
    ...
    case "counters":
        ...
        if (!member_array(which, keys(combat_counters)) == -1) {
            return 0;
        }
        if (!mod) {
            combat_counters[which] = val;
        }else {
            combat_counters[which] += val;
        }
        break;
    case "arrays":
        if (!combat_arrays[which] || !pointerp(combat_arrays[which])) {
            return ({});
        }
        if (pointerp(val)) {
            combat_arrays[which] = val;
        }else {
            if (mod) {
                combat_arrays[which] -= ({ val });
            }else {
...
mixed query_combat_mapps(string type, string which)
{
    ...
    if (!mapp(combat_vars) || !mapp(combat_static_vars)) {
        ok_combat_vars();
    }
    if (!stringp(type)) {
        return 0;
