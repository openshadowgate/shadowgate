...
{
    max_hp_bonus = hp>query_true_max_hp()?query_true_max_hp():hp;
}

void add_max_hp_bonus(int hp)
{
    max_hp_bonus += hp;
    max_hp_bonus = max_hp_bonus>query_true_max_hp()?query_true_max_hp():max_hp_bonus;
}
...
mixed is_wielding(string type)
{
    ...
    switch(type)
    {
        ...
        case "weapon":
            ...
            {
                if(weapons[i]->is_weapon()) { return 1; }
            }
            return 0;
        case "lrweapon":
            for(i=0;i<sizeof(weapons);i++)
        ...
        case "small":
            for(i=0;i<sizeof(weapons);i++)
            {
                if(weapons[i]->query_size() > 1) { return 0; }
            }
            return 1;
        case "medium":
            for(i=0;i<sizeof(weapons);i++)
            {
                if(weapons[i]->query_size() != 2) { return 0; }
            }
            return 1;
        case "large":
            for(i=0;i<sizeof(weapons);i++)
            {
                if(weapons[i]->query_size() < 3) { return 0; }
            }
            return 1;
        case "giant":
            for(i=0;i<sizeof(weapons);i++)
            {
                if(weapons[i]->query_size() < 4) { return 0; }
            }
            return 1;
    }
    return 0;
...
int check_on_limb(string limb) {
   ...
   if(body[limb]["damage"] > body[limb]["max_dam"]) {
      ...
      if(limb == "torso") tell_object(environment(this_object()),this_object()->QCN+" is dealt a mortal blow to the torso!", this_object());
      else tell_room(environment(this_object()),this_object()->QCN+" has "+this_object()->QP+" "+limb+" severed!", this_object());
