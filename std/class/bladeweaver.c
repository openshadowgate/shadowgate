...
{
    object class_ob;
    if(!objectp(ob) || !ob->query("bladeweaver_base_class")) { class_ob = find_object_or_load(DIR_CLASSES+"/fighter.c"); }
    else { class_ob = find_object_or_load(DIR_CLASSES+"/"+ob->query("bladeweaver_base_class")+".c"); }
...
string *query_base_classes(object obj) 
{
    ...
    base = obj->query("bladeweaver_base_class");
    if(!base) { return ({}); }
    return ({ base }); 
}
...
{
    if(!objectp(obj)) { return; }
    obj->delete("bladeweaver_base_class");
    return;
}
...
int set_base_class(object obj, string choice)
{
    ...
    obj->set("bladeweaver_base_class",choice);
    return 1;
}
...
string requirements() // string version, maybe we'll need this, maybe not, can remove later if not
{
    ...
    str = "Prerequisites:\n"
        ...
        "    40 Character Levels\n"
        "    20 Points Spent in Athletics Skill\n"
...
int prerequisites(object player)
{
    ...
    if(skills["athletics"] < 20) { return 0; }    
    base = player->query("bladeweaver_base_class");
    if(!base) { return 0; }
    if(!player->is_class(base)) { return 0; }
    if((player->query_class_level(base) + adj) < 20) { return 0; }    
    if(!FEATS_D->usable_feat(player,"improved two weapon fighting")) { return 0; }
