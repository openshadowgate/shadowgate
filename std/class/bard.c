...
mapping class_featmap(string myspec) {
    return ([ 1:({ "simple weapon proficiency", "martial weapon proficiency", "light armor proficiency","shield proficiency", "spell focus","inspire","inspire courage", "dodge", "countersong"}), 3:({"inspire competence"}), 8:({"dirge of doom"}), 9:({"inspire greatness"}), 10:({ "tools of the trade" }), 14:({"frightening tune"}), 15:({"inspire heroics"}), 20:({"deadly song"})]);
}
