...
mapping stat_requirements() {
    return ([ "strength" : 13, "constitution" : 13 ]);
}
...
mapping class_featmap(string myspec) {
    return ([ 1 : ({ "light armor proficiency", "medium armor proficiency", "shield proficiency", "simple weapon proficiency", "martial weapon proficiency", "rage", "powerattack", "unarmored defense" }), 4 : ({ "dodge" }), 7 : ({ "damage resistance" }), 11 : ({ "greater rage", "resistance" }), 14 : ({ "mobility" }), 17 : ({ "mighty rage"}) ]);
}
