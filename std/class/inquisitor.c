...
mapping class_featmap(string myspec) {
    return ([ 1 : class_feats()+({"judgement"}),3:({"force of personality"}), 6: ({"leadership"}), 8: ({"second judgement"}), 11: ({"stalwart"}), 14: ({"exploit weakness"}),16 : ({"third judgement"}), 17 : ({"slayer"}), 20 : ({"true judgement"})]);
}
