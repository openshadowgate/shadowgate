...
string *query_open_riddle_answers() { return open_riddle_answers; }

void set_open_riddle(mixed str)
{
    if(!str) return;
    if(!pointerp(str)) str = ({str});
    open_riddles = str;
}
...
void delayed_get(object ob, object who, object where)
{
    ...
    if(!objectp(pilfered_for) || !objectp(pilfered_by) || !objectp(pilfered_at)) return;
    if(ob != pilfered_for ||
    who != pilfered_by ||
    where != pilfered_at)
    {
