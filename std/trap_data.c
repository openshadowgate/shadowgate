...
varargs void set_trapped(string *actions, string *TrapLevels, string *TrapTypes, int PSetter)
{
	...
	mixed tmp;
	if(!sizeof(actions)) return;
	if(!sizeof(TrapLevels)) return;
	if(!sizeof(TrapTypes)) return;
	if(!mapp(trap_data)) trap_data = ([]);
	if(!intp(PSetter)) PSetter = 0;
	if(PSetter == 0 || PSetter == 2) 
	{
		if((tmp = TO->query_property("traps_known_to_monsters"))) 
		{
			if(stringp(tmp)) MobKnown += ({tmp});
            else if(pointerp(tmp)) MobKnown += tmp;
		}
	}
	if(PSetter == 1) 
	{
		...
		{
			if(stringp(tmp)) PlayerKnown += ({tmp});
			if(sizeof(tmp)) PlayerKnown = tmp;
		}
	}
	for(x = 0;x < sizeof(actions);x++)
	{		
		if(x >= sizeof(TrapLevels) || x >= sizeof(TrapTypes)) continue;
		...
		if(MyTrapLvl != "special")
		{
			...
			if(MyTrapLvl == "custom")
			{
				if(strsrch(TrapTypes[x], ".c") == -1) TrapTypes[x] += ".c";
				if(file_exists(TrapTypes[x])) 
				{
					TrapOb = new(TrapTypes[x]);
				}
			}
			if(!objectp(TrapOb)) continue;
...
string get_trap_object_auto_detect(string str, object who)
{
	...
	if(!objectp(who)) return str;
	if(!who->query("is_auto_detecting_traps")) return str;
	if(!sizeof(trap_data)) return str;
	if(!userp(who)) return str;
	MN = who->query_name();
	TrapKeys = keys(trap_data);
	MySkill = (int)who->query_skill("dungeoneering");
	for(x = 0;x < sizeof(TrapKeys);x++)
	{
		if(!trap_data[TrapKeys[x]]) continue;
		if(trap_data[TrapKeys[x]]["trap state"] != 1) continue;
		...
		trap_data[TrapKeys[x]]["failed_check"][MN] >= MySkill) continue;
	...
	if(flag == 2) return str +"%^RESET%^ (%^BOLD%^%^GREEN%^Trapped%^RESET%^)";
	if(flag == 1) return str + "%^RESET%^ (%^BOLD%^%^RED%^Trapped%^RESET%^)";
	return str;
}
...
string get_trap_room_auto_detect(string str, object who, int is_door)
{
	...
	{
		case 0:
			if(!trap_data[str]) return str;
			if(trap_data[str]["trap state"] != 1) return str;
			if(userp(who))
			{
			...
			{
				if(trap_data[str]["failed_check"][MN] >= MySkill) return str;
			}
		...
		case 1:
			...
			for(x = 0;x < sizeof(TrapKeys);x++)
			{
				...
				if(strsrch(TrapKeys[x], str) == -1) continue;
				if(trap_data[TrapKeys[x]]["trap state"] != 1) continue;
				if(userp(who)) 
				{	
			...
			if(flag == 2) return "%^BOLD%^%^GREEN%^"+str+"%^BOLD%^%^CYAN%^";
			if(flag == 1) return "%^BOLD%^%^RED%^"+str+"%^BOLD%^%^CYAN%^";
			else return str;
			break;
	}
	return str;
...
void remove_trap(string str)
{
	if(!mapp(trap_data)) return;
	if(!trap_data[str]) return;
...
void query_trap_data() { return trap_data; }
int is_this_trapped(string str)
{
	if(!stringp(str)) return 0;
	if(!mapp(trap_data)) return 0;
	if(trap_data[str]) return 1;
	return 0;
}
...
void check_random_traps()
{
	...
	if(TO->query_property("random traps")) 
	{
		...
		if(undefinedp(TO->query_property("random trap frequency"))) tc = random(3);
		else 
		{
			tf = TO->query_property("random trap frequency");
			if(tf >= random(100)) tc = 0;
		}
		if(!tc) 
		{
			TO->set_random_traps();
...
void set_random_traps()
{
	...
	if((int)TO->query_property("next_random_trap_time") > time()) return;
	if(!stringp(Tlvl = TO->query_property("random traps"))) return;
	RandNum = 1 + random(2);
	if(TO->is_vault()) 
	{
	...
	if(!sizeof(MyExits) && !sizeof(MyDoors)) return;
	while(RandNum)
	{
...
varargs int toggle_trap(string trap_loc, string trigger, string info) { return 0; }
...
void reactivate_traps()
{
	...
	string *TrapKeys;
	if(!mapp(trap_data)) return;
	if(!sizeof(TrapKeys = keys(trap_data))) return;
	for(x = 0;x < sizeof(TrapKeys);x++)
	{
		if(trap_data[TrapKeys[x]]["enable time"] == -1) continue;
...
{
	if(!sizeof(trap_data)) return;
	if(!mapp(trap_data)) return;
	if(!trap_data[ThisTrap]) return;
...
void add_passed_check(string ThisTrap, string MyName)
{
	...
	if(member_array(MyName, trap_data[ThisTrap]["passed_check"]) != -1) return;
...
{
	if(!mapp(trap_data)) return 0;
	if(!sizeof(trap_data)) return 0;
	if(!stringp(ThisTrap)) return 0;
	if(!trap_data[ThisTrap]) return 0;
	if(!trap_data[ThisTrap]["set by"]) return 0;
...
void add_known_to(string ThisTrap, string which, mixed name)
{
	...
	if(!mapp(trap_data)) return;
	if(!sizeof(trap_data)) return;
	if(!stringp(ThisTrap)) return;
	if(which != "player" && which != "monster") return;
	if(!trap_data[ThisTrap]) return;
	if(!stringp(name) && !sizeof(name)) return;
	if(stringp(name)) 
	{
		if(which == "player")
		{
			if(member_array(name, trap_data[ThisTrap]["KnownToPlayers"]) != -1) return;
			trap_data[ThisTrap]["KnownToPlayers"] += ({name});
			return;
		}
		if(which == "monster")
		{
			if(member_array(name, trap_data[ThisTrap]["KnownToMobs"]) != -1) return;
...
int query_failed_know(string ThisTrap, string MyName)
{
	...
	if(!mapp(trap_data)) return 0;
	if(!trap_data[ThisTrap]) return 0;
	if(member_array(MyName, keys(trap_data[ThisTrap]["failedknow"])) == -1) return 0;
...
void remove_failed_know(string ThisTrap, string MyName)
{
	...
	if(!mapp(trap_data)) return;
	if(!trap_data[ThisTrap]) return;
	if(member_array(MyName, keys(trap_data[ThisTrap]["failedknow"])) == -1) return;
	map_delete(trap_data[ThisTrap]["failedknow"], MyName);
}
void clear_all_traps() { trap_data = ([]); }