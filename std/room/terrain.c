...
{
    if (!travel_penalty) {
        return get_default_travel_penalty_multiplier();
    }
...
{
    if (!stamina_penalty) {
        return get_default_stamina_penalty();
    }
...
string query_terrain()
{
    ...
    if (TO->query_property("indoors")) {
        if (!isInside(terrain)) {
            ...
            if (who) {
                log_file("reports/" + who, "Travel error in " + base_name(TO) + "\n");
            }
            seteuid(getuid());
        ...
        if (!isOutside(terrain)) {
            ...
            if (who) {
                log_file("reports/" + who, "Terrain error in " + base_name(TO) +
...
string query_travel_penalty()
{
    ...
    if (!travel) {
        ...
        log_file("errors/travel", "Terrain error in: " + base_name(TO) + " " + ctime(time()) + "\n");
        if (who) {
            log_file("reports/" + who, "Travel Penalty error in " + base_name(TO) + "\n");
        }
        seteuid(getuid());
        return get_default_travel_penalty();
    }
