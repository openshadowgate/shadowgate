...
int magic_arsenal_feat(int num) {
    ...
    if(FEATS_D->usable_feat(TO,"magic arsenal")) num += 2;
    if(FEATS_D->usable_feat(TO,"greater arsenal")) num += 4;
    return num;
}
    ...
    if(TO->is_class("mage") || TO->is_class("sorcerer"))
    {
        if(FEATS_D->usable_feat(TO,"gift of the shadows"))
        {
            ...
            if(!arrayp(tmp[baseclass]))
                tmp[baseclass]=({});
            tmp[baseclass]+=({"umbral sight","shield of shadows","shadow vortex","night armor","darkbolt","shadow blast","shadow double","shadow nova"});
        }
    }
