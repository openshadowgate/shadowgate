...
int *age_brackets() { return ({ 3, 100, 150, 300 }); }
...
int *restricted_alignments(string subrace) { return ({ 1, 2, 3, 5, 7, 9}); }

string *restricted_classes(string subrace) {
    return ({  "paladin", "fighter", "barbarian", "psion", "psywarrior", "monk","inquisitor", "cleric"});
}
...
string *query_hair_colors(string subrace) {
    ...
    choices += ({"pink","green", "emerald", "red", "magenta", "crimson"});
    choices += ({"blue"});
    choices += ({"black","white"});
    return choices;
}
