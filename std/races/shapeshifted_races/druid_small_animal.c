...
void create()
{
    ...
    set_shape_profile("druid_small_animal_999"); // needs to be something the player is unlikely to name one of their profiles when disguise goes in
...
int default_descriptions(object obj)
{
    ...
    obj->set_description("is a cute, furry little creature.");
    ...
    obj->force_me("message in darts in");
...
int change_into_message(object obj)
{
    ...
    tell_object(obj,"%^RESET%^%^RED%^Your senses heighten, you can feel the pulse of the world, smell countless scents, "
        "you can taste the very air.  You feel yourself shrinking, you are now a small animal.");
    ...
    tell_room(environment(obj),"%^YELLOW%^Where "+obj->QCN+" once stood, is now a small animal!",obj);
