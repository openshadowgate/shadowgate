...
{ 
    if(TO->query("alternative world")) return; //to prevent alternative "versions" of rooms from saving - hopefully none of the rooms that set_storage() are alternative
    storage = i; 
}


int query_storage() { return storage; }


int is_room() { return 1; }


void set_enter_room(string str) { return; }
...
string query_climate() { return climate; }
...
void set_random_frequency(int x) { only_on_reset = x; }


int query_random_frequency() { return only_on_reset; }
...
void do_random_encounters(string *monsters, int percentile, int num_mons) 
{
  ...
  //   Check for other monsters in the room.     //
    ...
    if(random(100) < percentile) 
    {
        ...
        if(actual_num > 0) { already_encountered = 1; }
    }
}


int query_only_on_reset() { return only_on_reset; }


int query_already_encountered() { return already_encountered; }
...
void set_light(int d) { set_property("light", d); }


void set_indoors(int d) { set_property("indoors", d); }


void set_no_castle(int d) { set_property("no_caste", d); }


void set_no_steal(int d) { set_property("no_steal", d); }
...
int has_grafitti(string which) 
{
    ...
    if(query("_nograph") || (!mapp(graphmap = query_property("_grafitti"))) ) { return 0; }
    else 
    {
        if (stringp(graphmap[which])) { return 1; }
        else { return 0; }
    }
}
...
string query_short() 
{
    if(has_grafitti("short")) { return query_grafitti("short"); }
...
void set_peer_message(string str) { peer_message = str; }


string query_peer_message() { return peer_message; }


void set_mcharge_message(string str) { mcharge_message = str; }


string query_mcharge_message() { return mcharge_message; }


void set_charge_message(string str) { charge_message = str; }


string query_charge_message() { return charge_message; }
...
int set_room_dependencies(object * obs) { dependencies = copy(obs); }
...
mapping query_coords() { return coords; }
...
void keep_rooms(mixed *rooms)
{
    ...
    if(!pointerp(rooms) || !sizeof(rooms)) { return; }
    
    for(i=0;i<sizeof(rooms);i++)
    {
        if(!objectp(obj = find_object_or_load(rooms[i]))) { continue; }
