...
// Checking to see how much damage this does... I really need HB to keep going
    ...
    if(!sizeof(query_attackers()) && query("protecting")) set("protecting",0);
...
void die(object ob)
{
    ...
    if(TO->query_property("death effects")) { "/daemon/death_effects_d"->get_death_effect(TO); }
    TO->move(ROOM_VOID);
    remove();
}
...
string *query_mini_quests() { return CHAMPION_D->query_monster_quests(); }
string *query_quests() { return CHAMPION_D->query_monster_quests(); }
