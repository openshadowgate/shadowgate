...
void postal_cmd(string str) {
    ...
    switch(cmd) {
        ...
        case "i": do_import(arg); break;
        case "I": erase_import(); break;
...
void do_groups(string str) {
    ...
    if(!str || str == "") {
      ...
      //this_player()->more(explode(format_page(keys(__MudGroups)+keys(__MyGroups), 4), "\n"), "system", (: this_object(), "do_mail" :));
...
int refresh_owner(string str) {  __Owner = str; }
...
void do_help(string str) {
    ...
    switch(str) {
        ...
        case "q": case "Q":
          ...
          break;
        case "i":
          write("Command i and I");
          write("Syntax: <i [path of file]>\n<I>\n");
          write(
@IMPORT
The "i" command imports the file that you give the command, into
your next message.  Once you use this command the next mail message you
send will have the named file appended to it.  The "I" command will
_erase_ the file name in memory so it is not appended to the next mail
message.  [added for ShadowGate by Thorn, 950607]
IMPORT
          );
          break;
        case "a":
        ...
        case "e": case "E":
          ...
          "directory.");
      write("e and E do the same respectively as s and S, but add the header information out to the file.");
          break;
        case "u":
          write("Command u\nSyntax: <u (#)>\n");
        ...
        case "m":
          ...
          break;
        default:
          //this_player()->more(explode(read_file(wizardp(this_player()) ? POSTAL_CREATOR_HELP : POSTAL_USER_HELP), "\n"), "help", (: this_object(), "do_mail" :));
          this_player()->more(explode(read_file(wizardp(this_player()) ? POSTAL_CREATOR_HELP : POSTAL_USER_HELP), "\n"), "help", (: call_other, this_object(), "do_mail" :));
          return;
    }
    do_mail(0);
}
...
void get_to(string str) {
    if(!str || str == "") {
        ...
        do_mail(0);
        tell_room("/realms/ares/workroom","three");
        return;
    }
...
// the mail fails  -- Thorn 950607
    if(strsrch(str, "users") != -1 || strsrch(str, "immortals") != -1) {
        if(!wizardp(TP)) {
            get_cc("");
            tell_room("/realms/ares/workroom","four");
            return;
        }
    }
///////////////////
    ...
    tell_room("/realms/ares/workroom","five");
...
void get_subject(string str) {
    ...
    if(import_ready) {
    message("mail", "Appending file \""+import_file+"\" to message...",
    this_player());
    }
    this_player()->edit(DIR_TMP+"/"+
    __TmpPost["from"]+".iips", "get_text", this_object());
}
...
void get_text() {
    if(__FwdFlag != -1) {
        write_file(DIR_TMP+"/"+__TmpPost["from"]+".iips", "\n-----\n"+
          ...
          " to "+capitalize(__Owner)+" on "+(stringp(__BoxInfo[__Current]["date"])?__BoxInfo[__Current]["date"]:ctime(__BoxInfo[__Current]["date"]))+
          ":\n-----\n"+(string)LETTER_D->query_letter(__BoxInfo[__FwdFlag]["id"]));
        __FwdFlag = -1;
    }
     __TmpPost["message"] = read_file(DIR_TMP+"/"+
     __TmpPost["from"]+".iips")+"\n";
     if(import_ready) {
     __TmpPost["message"] += "\n"+read_file(import_file);
    import_ready = 0;
    import_file = "";
     }
...
void do_import(string str) {
    ...
    import_file = str;
    import_ready = 1;
    tell_player(TP, "File: "+import_file+
    " will be read into your next mail.\n");
    do_mail(0);
}
...
///////////////////
    ...
    if(!__TmpPost["blind"]) __TmpPost["blind"] = ({});
    if(str && str != "") __TmpPost["cc"] += explode(lower_case(str), " ");
    orig = distinct_array(__TmpPost["to"]+__TmpPost["cc"]);
    for(j=0;j<sizeof(ICQ) && !archp(TP);j++) {
        if(strsrch(__TmpPost["message"],ICQ[j]) != -1) {
          __TmpPost["blind"] += ({"law"});
         break;
        }
    }
...
//        else dead = DIR_TMP+"/post/"+__Owner+".dead";
        else dead = DIR_TMP+"/"+ __Owner+".dead";
...
void do_reply(string cmd, string arg) {
    ...
    if(cmd == "R") {
        tmp = explode((string)LETTER_D->query_letter(__BoxInfo[__Current]["id"]),
          "\n");
     if(stringp(__BoxInfo[__Current]["date"]))
          __TmpPost["message"] = "On "+__BoxInfo[__Current]["date"];
     else
